...
graph TD;
    ...
    H_though2[H]-->T_thought[T];

    H[H]-->R_through[R];
    R_through[R]-->O_through[O];
    O_through[O]-->U_through[U];
    U_through[U]-->G_through[G];
    G_through[G]-->H_through2[H];
```

### Code analysis

Iterates through each word in the document, creating new child nodes as necessary. With similar prefixes, branches off them and marks the last character node of each word as a node to end the word.
...
BFS used to traverse the tree from the end of the prefix where child nodes are added to a queue. Uses nodes marked as is_word to find valid words. 

### Your output

For the prefix, "th": the, thee, thou, that, thag, there, their, though, thought, through
...
DFS used to explore deeper paths first with the same prefix, pushing child nodes onto a stack. Uses nodes to find valid words until is_word marked nodes are reached.

### Your output

For the prefix, "th": the, there, their, thee, thou, though. thought, that, that, through

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.

Recursive DFS will eventually lead to max recursion depth but is better for readability. I used stakc-based DFS in the suggest_dfs function, which manages a stack more easily and allows for more control while traversing the tree. By iteratively exploring child nodes, I am able to avoid a potential depth limit and track words orderly. 

## `UCS`

### Code analysis

UCS used to prioritize paths based on frequency of characters. Priority queue used to expand nodes based on cost function. 

### Your output

For the prefix, "th": the, thou, thee, that, thag, though, there, their, thought, through 

## Experimental
Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS:
In the suggestions generated using BFS, it showed the shorter words first before going the longer words. Since DFS goes deeper into the tree first, some longer words appeared before some shorter words because they have the same prefix. When using UCS, it was based on frequency so that more common words appeared earlier like "the", allowing a more practical autocompletion feature. 
