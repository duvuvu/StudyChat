...
![Tree Diagram](tree_diagram_image.jpg)
...
of the queue it checks if that node is the end of a word and if it is adds the string in the tuple with that node to the suggestions array that is returned. It then adds all of the tuples for the explored node's children to the queue, appending their letter to the substring in their tuple.
...
of the stack it checks if that node is the end of a word and if it is adds string in the tuple with that node to the suggestions array that is returned. It then adds all of the tuples for the explored node's children to the stack, appending their letter to the substring in their tuple.
...
- I used a stacked-based DFS to implement this function. I used this implementation because it was very similar to doing BFS but instead of using a queue I used a stack, making it easy to transfer my BFS implementation to a DFS. The final words that were added to the tree would be found first, finding them from right to left. In the implementation I added each node to the deque at the end, "stack" - at the top, and then popped them off from the right side of the deque, popped off the top of the stack.
...
- For "th":
- the, thou, thee, thag, that, their, there, though, thought, through


## Experimental
- In my experimentation with the suggestions returned by the different algorithms I have noticed that the results
of BFS are very similar while DFS is very different. DFS suggestions can be somewhat random in length but similar
spelled words are suggested sequentially. BFS and UCS usually return the shorter words first. That makes sense
for BFS because since it explore everything at once it will find short words before long words. It happens for UCS
because shorter words have a cheaper cost to find then longer words so are found first. BFS and DFS are better
for suggesting words that are more similar in length to the current prefix, than DFS, which can be good because usually if not a lot is typed
its better suggest words close in length to the prefix then some random long word. Although how DFS groups words spelt similarly together can also be good.
I would say overall for word suggestion BFS and UCS are better than DFS.
