...
![Tree Diagram for test.txt](IMG_5431.jpg)


### Code analysis

- **build_tree(document)**
   - **Explanation**: Constructs the trie from a given document.
   - **Purpose**: Constructs the trie from a given document.
   - **How it works**: 
     - It splits the document into words and iterates through each character of each word.
     - If a character is not already a child of the current node, it creates a new node for that character.
     - It increments the frequency of each character as it builds the trie, marking the end of each word appropriately.



### Your output

For each prefix provided: After running your autocomplete suggestions (DFS, BFS, UCS) with specific prefixes, document the output here. You’d typically capture what suggestions were provided based on the trie built from the text file.
...
- **suggest_bfs(prefix)**
   - **Explanation**: The Breadth-First Search (BFS) explores nodes level by level. Starting from the root, it examines all immediate children before proceeding to the next level of nodes. This ensures that the shortest paths to complete words are found first, which aligns with its structure of exploring outward from the root.
   - **Purpose**: Suggests words using Breadth-First Search (BFS).
   - **How it works**: 
     - It initializes a queue with the root node and processes nodes level by level.
     - If a node represents a valid word that starts with the given prefix, it adds it to the list of suggestions.
     - It continues to enqueue children nodes until all nodes have been processed.


### Your output

![Output for BFS](BFS_Output.jpg)
...
- **suggest_dfs(prefix)**
   - **Explanation**: Depth-First Search (DFS) dives deep into the trie as far as possible before backtracking. This recursive approach allows the program to explore each complete word along paths from the root until it hits a leaf node. This method captures all possible completions based on the prefixes provided.
   - **Purpose**: Suggests words using Depth-First Search (DFS).
   - **How it works**: 
     - It defines a recursive inner function that explores each node deeply.
     - If a node is a valid word that starts with the prefix, it adds it to the suggestions.
     - The recursion continues until all paths from the root have been explored.



### Your output

![Output for DFS](DFS_Output.jpg)

### Recursive DFS vs Stack-based DFS

- Recursive DFS: Utilizes the call stack for depth traversal. It’s generally simpler to implement and understand due to its holistic approach to explore all children of a node recursively.
- Stack-based DFS: Requires manual management of the stack. This might be more memory efficient in scenarios where recursion depth might be high, but it can be more complex to implement and maintain.
- Used Approach: In your implementation, the `suggest_dfs` uses a recursive approach. This method is straightforward and more intuitive for traversing trees.
...
- **suggest_ucs(prefix)**
   - **Explanation**: The Uniform Cost Search (UCS) method prioritizes nodes to be expanded based on a cost function (here, the inverse of frequency). It aims to suggest words based on how frequently they appear. By using a priority queue, the algorithm ensures that it always expands the most promising node next (in this case, the node with the lowest cumulative cost), which leads to offering higher-frequency suggestions sooner.
   - **Purpose**: Suggests words using Uniform Cost Search (UCS).
   - **How it works**: 
     - It first traverses the trie to find the node corresponding to the given prefix.
     - It uses a priority queue to explore nodes based on their cumulative cost, which is calculated as the inverse of their frequency.
     - It ensures that the most frequently occurring words are suggested first by expanding nodes with the lowest cumulative cost.

### Your output

![Output for UCS](UCS_Output.jpg)

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 

| Algorithm | Behavior | Advantages | Disadvantages | Example Output |
|-----------|----------|------------|---------------|----------------|
| BFS (Breadth-First Search) | - Shows words in order of increasing length/complexity<br>- Starts with simpler/shorter words<br>- Gradually moves to longer words | - Most organized presentation<br>- Predictable suggestions<br>- User-friendly for beginners | - May delay showing relevant complex matches<br>- Might not prioritize common usage | the → thou → thee → that → thag → though → their → there → thought → through |
| DFS (Depth-First Search) | - Explores one branch deeply before others<br>- Shows complete word patterns before alternatives<br>- Backtracks to show variations | - Can find complex variations quickly<br>- Good for exploring all possibilities<br>- Memory efficient | - Appears random to users<br>- Less intuitive suggestion order<br>- May not show simple matches first | the → there → their → thee → thou → though → thought → that → thag → through |
| UCS (Uniform Cost Search) | - Orders based on edit distance from input<br>- Prioritizes minimal changes<br>- Balances complexity and relevance | - Practical for real-world use<br>- Good balance of simplicity and relevance<br>- Efficient for common use cases | - May require more computational resources<br>- Complex implementation | the → thee → thou → that → thag → there → their → though → thought → through |
