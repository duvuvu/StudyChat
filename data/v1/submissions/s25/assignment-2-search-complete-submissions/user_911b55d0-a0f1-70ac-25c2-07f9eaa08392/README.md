...
Iterates through each word in the document and for each character in the word, traverse through the tree or create a new node if the character doesn't have a corresponding node yet. At the end of each word, use the empty string as a key with a value of an empty Node. 
...
- Traverse the tree until you reach the last node of the prefix and add that node to the queue. If the first node's children contains the empty string add it to the word list, then add the children to the queue and repeat until the queue is emtpy.
...
- Traverse the tree until you reach the last node of the prefix and add that node to the queue. Using the double ended queue as a stack, if the head node has not been visited, check if the current prefix is a word and add all children to the front of the stack then mark the head node as visited. If visited before, pop the node from the stack.
...
-  A recursive DFS implementation keeps calling itself until it reaches the tail where the node has no children where as the stack-based DFS implementation keeps adding nodes to the front of the stack until the stack is empty. I used a stack-based DFS implementation because I found it easier to visualize the difference between DFS and BFS by using the different ends of the double ended queue.
...
- Traverse the tree until you reach the last node of the prefix and add that node to the heap with cost 0. While the heap has nodes, pop the minimum cost element and if the node marks the end of the word, add it to the list. Then for every child of the current node, if not the empty string, push onto the heap.
...
DFS: slay snatched sis simp simping sus shook savage salty\
UCS: sis simp sus shook salty slay snatched savage simping\
BFS shows the words sorted by word length which is useful for short words such as "the" that are really common in sentences. DFS shows the words such that similar words are shown near each other. From the example above, simp is shown right before simping. UCS shows the words based on how much the characters appear meaning generally, shorter words will appear first with common prefixes showing earlier. All algorithms still show all words, just in different orders.