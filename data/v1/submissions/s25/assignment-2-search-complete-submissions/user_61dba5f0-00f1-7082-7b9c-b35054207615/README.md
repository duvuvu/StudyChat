...
### Tree diagram
- ![image](tree_diagram.jpg)

### Code analysis

- To build the tree I make every new character I have not seen before into a node, if I have already seen it, then
  I go through the path of nodes I have already created until I reach point that separates that word from my current one
  and add new nodes from there from my current word. Every node starts off with 1 word count, if that node is traversed again
  the word count goes up by 1. This is for calculating the cost for UCS.
...
- I traverse through the tree until where the prefix ends, then I explore each node level by level, by using a queue which 
  is a first in first out data structure. When I reach a node that has is_word set to true, I add that to my suggestion list.
  So the shortest word would be the first suggestion.
...
- I start by pushing the initial node onto the stack and while the stack is not empty I pop the node from the stack and first
  check if it is a word, if not then I continue exploring to the end of that chain of nodes before I go back up and explore the 
  next chain of nodes. 
...
- I used a stack based DFS since I felt that it was more structured and easier to implement. I believe a recursive based DFS would also use more space
than a stack based DFS as the tree gets larger. A stack based DFS only needs to store the nodes in essentially a list, whereas a recursive DFS needs to store the node data and the return address for each recursive call.
...
- When I built the tree, each node already has a number of words that it leads to, so to calculate the cost to that node,
  I can just take the inverse and add it to the current cost. Then if two paths have the same cost, I choose the next node
  in terms of alphabetical order, since I sorted the characters. So this method always chooses which ever node leads to the
  most words since the path cost is lower.
...
## Experimental
- Using BFS, I see that the suggestions are sorted from shortest to longest in terms of word length. The suggestion results for DFS seemed to be random, since it depends on the structure of the tree. UCS also returns suggestions roughly in the order from the shortest to longest words, but not because the search was level by level like BFS, but because shorter words tend to lead to other words. If all the words were very different from each other, the suggestions would simply be ranked by alphabetical order.
