...
```mermaid
graph TD;
    %% Root node: "t", followed by 'h'
    ROOT --> T[t]
    T --> TH[h];

    %% Branch for words starting with "tha"
    TH --> TH_A[a];
    TH_A --> TH_AG[g];
    TH_A --> TH_AT[t];

    %% Branch for words starting with "the"
    TH --> TH_E[e];
    %% Extending "the" further:
    TH_E --> TH_EE[e];
    TH_E --> TH_EI[i];
    TH_EI --> TH_EIR[r];
    TH_E --> TH_ER[r];
    ...
    %% Branch for words starting with "tho" (except "through")
    TH --> TH_O[o];
    TH_O --> TH_OU[u];
    %% Extending "thou" to "though" and "thought":
    TH_OU --> TH_OUG[g];
    TH_OUG --> TH_OUGH[h];
    TH_OUGH --> TH_OUGHT[t];
...
for each char in word (word comes from a document of words), char is assessed  to see if it is already in the tree as a child of our  current node. if it is, then we move on and our current node becomes the [char] node. otherwise, a new node is created as a child of our current one with the name char.
...
The first section of my code process our prefix to ensure that we can make suggestions as to its ending. The second section creates a queue to store the nodes we need to process, assesses if each element in the queue is a valid word, and then adds each child node to the queue along with the accumulated word.
...
Just like in BFD, the first section of my code process our prefix to ensure that we can make suggestions as to its ending. The second section creates a stack (basically the only thing different between the DFS and BFS approach) to store the nodes we need to process, assesses if each element in the queue is a valid word, and then adds each child node to the queue along with the accumulated word.
...
Rather than using a recursive approach where python remembers the state "behind the scenes" I went with a more visualizable stack approach. This allowed me to generate a list in Python that could viewed at anytime (in a debugger). I initially tried using a recursive approach but then got a bit confused and didn't want to worry about making a helper function as the recursive approach without one kept erasing my prexisting work, preventing me from 'remembering' the prefixes.
...
When an input prefix is entered, my code locates the corresponding node and uses a priority queue (sorted by the negative of each node’s subtree count) to explore the children whose subtrees are largest first. As it expands each node, it collects any complete words and continues until all possible completions have been identified. The resulting list is in order of most commonly used prefixes, like 'the' and 'thou' which each are used in multiple other words.
...
## Experimental
When comparing Breadth-First Search (BFS), Depth-First Search (DFS), and Uniform Cost Search (UCS), distinct differences in their behaviors and outputs become evident. BFS systematically explores nodes level by level, leading to results that are returned in order of word length, as shorter words are found first. In contrast, DFS dives deep into the tree structure, producing outputs that appear more random, as it exhaustively explores one branch at a time without considering depth in relation to word length. Similarly, UCS focuses on the cumulative costs associated with transitioning between nodes, resulting in an output that can also seem random, as it prioritizes paths based on cost rather than depth or length. Ultimately, while BFS yields structured results by word length, both DFS and UCS provide outputs defined by their distinct exploration strategies, making their use dependent on the specific requirements of the search task.
