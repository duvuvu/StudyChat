...
- Put the intuition of your code here
  build_tree iterates though each word in the document, and then iterates over each character in a word to check if its exists as a child of the current node. if the character doesn't exist, it's added as a new node and assigned as a child. Finally, is_word is marked as True to show the end of a valid word. 
...
- Put the intuition of your code here
We start from the root node and traverse to the node that corresponds to the last character of the given prefix, if the prefix doesn't exist we return an empty list. From the found node, add nodes to queue to traverse the subtree and enqueue child nodes as we traverse. As each node is visited, if the node is marked as the end of a word then we add that word to the list of suggeston words.
...
- Put the intuition of your code here
First we find the node that corresponds to the last letter of the prefix, then we use a recurisive DFS function to traverse the tree, going to the "end" of a branch before backtracking. As each node is visited, if the node is marked as the end of a word then we add that word to the list of suggeston words. We iterate over the children of current node by recursively calling dfs and passing in the current node and given prefix. 
### Your output

- Put the output you got for the prefixes provided here
'the', 'there', 'their', 'thee', 'thou', 'though', 'thought', 'that', 'thag', 'throug
### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
Recursive DFS - the function keeps calling itself for each letter until we reach the "end" of a branch. When a node is marked as the end, add it to suggestedWords. The function then backtracks and explores the next possible branch. 

Stack-based DFS -a stack is used to keep track of which nodes still have to be visited, and the most recnetly added node is processed next so that the an entire branch is explored until we reach the "end" of a branch. Similarly, when a node is marked as the end, add it to suggestedWords. The stack the ensures that siblings are explored after children. 

I chose to use recursive DFS here - mainly because I have more experience in coding DFS recursively. 
...
- Put the intuition of your code here
 After assigning costs to the nodes, we find the node that corresponds to the last letter of the prefix, then uses a priority queue to visit paths with the lowest cost first (those with common prefixes have lower cost). As each node is visited, if the node is marked as the end of a word then we add that word to the list of suggeston words. Once done exploring the current path, we explore child nodes in order of increasing cost. 
### Your output

- Put the output you got for the prefixes provided here
'the', 'thou', 'thee', 'that', 'thag', 'though', 'their', 'there', 'thought', 'through'
...
DFS: 'the', 'there', 'their', 'thee', 'thou', 'though', 'thought', 'that', 'thag', 'through'
UCS: 'the', 'thou', 'thee', 'that', 'thag', 'though', 'their', 'there', 'thought', 'through'

prefix s-> 
BFS: ['sus', 'sis', 'simp', 'slay', 'salty', 'shook', 'savage', 'simping', 'snatched']
DFS: ['salty', 'savage', 'shook', 'sus', 'simp', 'simping', 'sis', 'snatched', 'slay']
UCS: ['sis', 'simp', 'sus', 'shook', 'salty', 'slay', 'snatched', 'savage', 'simping']

In BFS, the words are given in increasing order of length. Like the short words are suggested first and the longest words are suggested last. In DFS, the words with similar prefixes are explored all the way before backtracking. For example, all the wrods that start with 'the' are expored before moving on to 'tho' then 'tha' and fiallly 'thr'. In UCS, the words are based on cost so words that have lower cost (those with more common prefixes) are explored first. 
