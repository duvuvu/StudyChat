...
- The build_tree function first creates a frequency map of how often each character follows every prefix in the document, then builds the trie characterwise while storing these frequencies as inverse costs in each node, allowing for probability-based path selection during suggestions. It keeps track of when a word ends as well in order to add only completed words to the suggestions.
...
- The suggest_bfs function traverses the trie level by level using a queue, starting from the node corresponding to the given prefix, and collects all valid words by appending each character to the current prefix. If the is_word flag is true, we know the word is completed and it is added to the suggestions to be returned.
...
- The function uses a stack to perform DFS, exploring each path completely then backtracking. It again starts at the node from the given prefix. It uses reverse alphabetical order and once again adds all is_word words to the suggestions to be returned.
...
- Recursive DFS relies on the call stack to manage traversal, using the system's stack to explore nodes. Stack-based DFS explicitly uses a stack data structure (in my case just a list) to manage traversal. In this implementation, I used stack-based DFS because it provides better control over the traversal process, and personally I am more comfortable with its implementation.
...
- The function traverses a trie starting at the given prefix and uses a priority queue to explore paths in order of increasing cost, where the cost is determined by character frequencies that were set in build_tree. It again collects words by checking is_word, this time ensuring suggestions are prioritized by the lowest cumulative cost as defined by the frequency.
...
## Experimental
- The BFS suggestions were (expectedly) sorted by length, since the algorithm checks each level of the tree one at a time, so smaller words will finish and be appended before larger words. This was not the case for DFS and UCS, as results were not sorted by length. Instead, DFS would return "strands" of words, listing all words with the same prefix together, like thag and that. Finally, UCS allowed for some longer words to appear before shorter if their frequency was high enough to allow their cost to be lower, like how though appeared before their and there.
