...
![Tree Diagram](llasic_tree.pdf)
### Code analysis

- Put the intuition of your code here
I use a dict to hold the frequncies of certain prefixes to later use to weight. 
The first loop is used to break the words in a file into its indvidaul chars, then checking if that char has 
already been added to a node children, creating a path of prefizes that eventually wil lead to 
word. The loop allows chars that have already been made to not be reapted.

A stack is used to help us do dfs search where along the way we assign a weight along the way to the node 
based off the prefix in the frequncy dict.
...
We start by getting the node that contains the prefix and that will be our starting node
We use an array that will act like our queue and follow the FIFO principcal. Until the current node is 
a word it will get added to an array of words we will return. Using the queue will explore each 
layer from the prefix and add it to the queue, adding the current char to our desceing word until we reach the end.
...
The intuion is just that we swtich what data strucutre we will follow from bfs. We change it to a stack
to follow the lifo princpal allowing us to descend the tree in a dfs manner first, exploring a full path down before goiong to look at other paths.
The following logic is the same as it is for bfs.
...
Recursive dfs travels left to right since we keep calling the method
on the first avalibale node, where the stack based version first adds all the nodes to a stack for a nodes child, then pop the last one added which would make us travel right first.
I created the stack based since it was an easy switch from bfs to dfs
...
- Put the intuition of your code here
FInd the starting node to descend from. We will use a specail type of queue
called the priority queue which when we pop something from it will be the smallest value from our weight
Since we wieghted our graph we traverse using the wieght. The logic is the smae for the most 
part as we expand out children and add them to the queue and pop out the prefix that has the lowest wieght
Then adding the nodes that are words to the words array to return.
...
DFS often groups letters that have same starting prefixes toghter For example in the letter c this is puts clout curve . . . cap. In BFS and UCS the results are almost simliar where the only differnce is that clout and curve are swapped. For the letter s it seems like BFS shows in order words of short lenth to longer length as a way of showing its data. UCS meanwhile does not really have any disnerable patern to it the only thing is that for the letter b UCS and BFS have the same output. 
