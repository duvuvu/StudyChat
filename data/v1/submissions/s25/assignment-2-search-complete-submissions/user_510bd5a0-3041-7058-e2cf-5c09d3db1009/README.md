...
--> Construct a prefix tree by iterating through each word and character, creating nodes as needed and marking the end of words.
...
- Put the intuition of your code here
--> BFS explores the tree level by level, starting from the node corresponding to the last character of the prefix.

--> It uses a queue to keep track of nodes to visit, ensuring that shorter words are discovered before longer ones.

--> Made for finding suggestions in order of word length.
...
- Put the intuition of your code here
--> DFS explores the tree as deeply as possible before backtracking.

--> It uses a stack to keep track of nodes to visit, ensuring that longer words are discovered before shorter ones.

--> Made for finding suggestions in reverse order of word length.
...
--> Using a stack-based DFS is easier because I'm given explicit control over the traversal process, making it simpler to manage and modify. It’s also easier to visualize and debug since I can directly inspect the stack at any point. Additionally, it offers flexibility to adapt the traversal logic without the risk of stack overflow.
...
- Put the intuition of your code here
--> UCS prioritizes suggestions based on the inverse frequency of characters.

--> It uses a priority queue (min-heap) to explore nodes with the lowest cost first.

--> The cost of a node is the inverse frequency of the character sequence, so more frequent sequences have lower costs and are prioritized.
...
BFS:
    Returns suggestions in order of word length (shorter words first).

    --> the, thee, thou, that, etc.

    Best for applications where shorter, more common words are preferred.

DFS:

    Returns suggestions in reverse order of word length (longer words first).

    --> through, thag, that, thou, etc.

    Best for applications where longer, more specific words are preferred.

UCS:

    Returns suggestions based on character frequency (more common sequences first).

    --> the, thou, thee, that, etc.

    Best for applications where more frequent or likely words are preferred.

Observations
- BFS is useful when you want suggestions in order of length.

- DFS is useful when you want longer or more specific suggestions.

- UCS is useful when you want suggestions based on frequency or likelihood.
