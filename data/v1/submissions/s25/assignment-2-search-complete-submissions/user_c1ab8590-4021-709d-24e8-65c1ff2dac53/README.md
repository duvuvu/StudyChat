...
Example tree diagram with just 3 elements
![My Tree Diagram](tree.png)
...
Splits the input text into words and inserts each character of these words into a prefix tree, ensuring that each word is represented and marked at its endpoint.
...
- Put the intuition of your code here
Uses a queue to perform a breadth-first search from the node of the last character in the prefix, collecting all complete words that match the prefix in the order they are found.

### Your output

- Put the output you got for the prefixes provided here
![My Tree Diagram](bfs.png)
...
- Put the intuition of your code here
Employs a recursive depth-first search starting from the node of the last character in the prefix, retrieving all complete words that begin with that prefix.

### Your output

- Put the output you got for the prefixes provided here
![My Tree Diagram](dfs.png)

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.

In the context of Depth-First Search (DFS), the recursive approach leverages the call stack to navigate through the tree, making it easier to implement and understand. Conversely, stack-based DFS explicitly manages a stack to keep track of nodes, providing more control and potentially better memory efficiency for deep trees. In this implementation, I utilized the recursive DFS method for its simplicity and clarity in capturing suggestions from the prefix node.
## `UCS`

### Code analysis

- Put the intuition of your code here
Utilizes a priority queue to conduct a uniform cost search from the node of the last character in the prefix, prioritizing word suggestions based on the path length in the tree.

### Your output

- Put the output you got for the prefixes provided here
![My Tree Diagram](ucs.png)



## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 
BFS provides suggestions in the order they appear in the prefix tree, typically prioritizing shorter and more common words first, making it user-friendly for rapid feedback. In contrast, DFS explores deeper into the tree first, potentially suggesting longer or less common completions before more recognizable options. UCS enhances suggestions based on character frequency, prioritizing the most probable or frequently used words, which can lead to more relevant and personalized completions for users.