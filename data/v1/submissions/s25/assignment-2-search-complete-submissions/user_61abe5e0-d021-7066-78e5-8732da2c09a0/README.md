...
For each character of each word, it checks if that character is already in the tree.  If not, a new node is created for it and its cost is initialized.  If the character is already a part of the tree, its cost is incremented.

## 'prefixNode' (helper function used by every serach function)

- Starts at the root of the tree and iterates over each character in the prefix, searching down the tree to find a node matching to that character.  If a node for every character in the prefix is found, the node for the last character of the prefix is returned, and if not, an empty list is returned. 

## `BFS`

### Code analysis

- Gets the starting node using 'prefixNode', initializes a queue with that node and prefix, and creates a list to store words found.  It then loops through the queue, checking if that node forms a complete word, and then adding all its children nodes to the queue with their associated words.  Once que is empty, the list of stored words is returned.
...
### Code analysis
- Gets the starting node using 'prefixNode', initializes a list to store words found, and creates a dfs function that takes a node and word.  Starts by calling dfs on prefix node, which checks if the node is a word and then loops through its children, calling dfs on each child and its associated prefix until every one is visited.
...
- Recursive DFS utilizes a function that calls within itself for each child found, while stack-based DFS utilizes a while loop in which the current node is poped off and all child nodes are appended until the stack is empty.  I used recursive DFS as I have more experience with it and find it easier.  
...
- Finds words based on a given prefix by utilizing a priority queue to explore the tree nodes in order of their associated costs, which reflect the inverse frequencies of characters. An if statement ensures that division by zero is handled, and suggestions are returned once the search is done.

### Your output

- the, thou, thee, thag, that, through, their, there, thought, through



## Experimental
- BFS orders words by length, DFS orders words by prefixes, and UCS appears to order randomily even though it uses word frequency.
