...
graph TD;
    ROOT-->T_there[T]
    T_there[T]-->H_there[H]
    H_there[H]-->E_there1[E]
    E_there1[E]-->R_there[R]
    R_there[R]-->E_there2[E]

    H_there[H]-->O_though[O]
    ...
    E_there1[E]-->I_their[I]
    I_their[I]-->R_their[R]

    H_there[H]-->R_through[R]
...
Iterates through the characters of each word and add them to the tree. Checks if the sequence of characters of the word already exists and if it doesn't add it to the tree otherwise it exists so we will increment the frequency for that character for the UCS search. At the end of each word iteration, we will mark the node containing the last letter of the word as a letter (set is_word = True).

## `BFS`

### Code analysis

Iterate through the characters of the prefix given, if the prefix doesn't exist in our tree, return nothing. Otherwise, we run BFS on the node containing the last letter of the prefix. This is done by first checking if the current node is a word and if it is, add it to suggestion list, then add all its children in the queue. Now, pop the leftmost element (FIFO) and repeat until the queue is empty. 
...
I reused my BFS code, but the key difference between BFS and DFS is that DFS implements the idea of LIFO. Therefore, I just changed how we pop the rightmost element in the DFS method rather than the leftmost one. Everything else stays the same.
...
Recursive DFS is where you take advantage of function calls to traverse through the nodes. In this case, we are utilizing the function stack to implement the LIFO order of DFS. Stack-based DFS, on the other hand, requires an explicit stack to implement the LIFO order. I used stack-based DFS as I was able to modify the code for BFS for my DFS method by changing how I choose my next node (from the right instead of the left of my queue).

## `UCS`

### Code analysis

After getting the node for the last letter of the prefix, I initialized my array (queue) to store objects of the structure (node, prefix, frequency). Then, I basically do the same thing as BFS/DFS except for choosing the node, I go through the entire queue (frontier) to get the node with the lowest total path cost, delete it from the frontier, and explore all its children and add it to the frontier, but I made sure to append its correct total path cost by adding the inverse frequency of that node to the lowest total path cost I obtained before. Repeat until frontier is empty.
...
In BFS, the suggestions prioritizes shorter words over longer words. However, in DFS, we can see there are longer words that are prioritized before shorter words. Finally, in UCS, although the overall trend looks like BFS where shorter words are prioritized, there are instances where longer words are actually above in the suggestion list, like the 6-letter word "though" vs the 5-letter words "there" and "their" that are lower in the list. A weakness of BFS is how if I want to type the word "metamorphosis" and I only type "meta", then the actual word I want would actually be very low on the suggestion list if there are many other shorter words that starts with "meta" in our dictionary (although it's not very common that someone would type metamorphosis). In addition, although DFS looks like it's randomizing the suggestion list, we can actually see a pattern in the groups of words. For instance, "thou, though, thought" in the list are next to each other and they all share the common prefix "thou" and "the, thee, their, there" all share the prefix "the". 
