...
- The code builds a prefix tree by inserting each character from the word in the document. It creates a new node if the characet doesnt exist and also tracks the frequency of each node. At the end of a word, the node is arked as a valid word.
...
- I use a BFS search on the prefix tree, this code uses a dequeue to manage the queue of nodes to visit. We start from the prefix node and explore all possiblities, when we reach a node marked as (is_word=true), it is returned as a suggestion. 
- Put the intuition of your code here
...
### Code analysis
Here I use DFS on a tree to suggest words based on a prefix. Since it is depth first I used a stack instead of a queue starting from the node with the prefix. The algorithm then goes through each possibility before backtracking and then returns words when encountering the is_word = true. This code usually prioritizes longer words for the (depth) before shorter ones. 
- Put the intuition of your code here
...
- I used stacks for my DFS because it is what I am more familiar with and the code seems more readable in figuring out what exactly DFS is doing in this context. 
...
- Put the intuition of your code here
- Here I used UCS with a priority queue, a min-heap to suggest the words from the tree based on the prefix. This algorithm prioritizes paths with the lowest cost, where the cost is inversely related to the frequency of the charcter. (Cost is defined as 1/frequency). This makes it so that the more common words should pop up first. 
...
BFS has shorter words first, so better for basic autocomplete
Dfs has longer words first, also good for autocomplete
UCS has more frequent words, better for prediction 
