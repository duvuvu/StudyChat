...
The `build_tree` method constructs a trie by first counting the frequency of each character in the provided document during the first pass. In the second pass, it builds the trie by adding each character as a node, setting the `is_word` flag for the terminal node of each word. Additionally, while constructing the trie, it assigns the pre-counted frequencies to each character node, allowing for later use in algorithms that depend on character frequency, such as Uniform Cost Search (UCS).
...
## Experimental
BFS suggested words in order from shortest to longest. DFS suggested words by grouping similar prefixes and then returning the suggestions in order from shortest to longest. UCS suggested words in order from shortest to longest, similar to BFS.
