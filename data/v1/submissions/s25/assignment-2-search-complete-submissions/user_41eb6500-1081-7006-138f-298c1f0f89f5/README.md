...
## Submission Details
- <redacted> <redacted>
- URL: https://github.com/<redacted>/assignment-2-search-complete-<redacted>
- Commit ID: 1ecf44a
...
- I implemented the method by splitting the document into words and inserting each character of the words into a dictionary, creating child nodes if necessary. The last character of each word is marked with is_word = True, the dictionary is used to quickly access nodes in a subtree, and for each node I keep track of the prefix so far for quicker access later.
...
- I start by navigating to the node that corresponds to the end of the given prefix in the tree. Then, using the queue data structure (First In First Out), I am able to perform a breadth-first search, systematically exploring all child nodes and identifying completed words.
...
- I start by navigating to the node that corresponds to the end of the given prefix in the tree. Then, using the stack data structure (Last In First Out), I am able to perform a depth-first search, systematically exploring all child nodes and identifying completed words.
...
- Recursive DFS uses the call stack implicitly, making the code simpler and more efficient for tree traversal, but it can hit recursion limits with deep structures. In contrast, stack-based DFS explicitly manages a stack, avoiding recursion depth issues at the cost of slightly more complex code and resources. In this project I use stack-based DFS primarily because of how much more straightforward it is. 
...
- I start by navigating to the node that corresponds to the end of the given prefix in the tree. I then use a priority queue to perform uniform-cost search, which allows me to explore child nodes based on their frequency, thereby prioritizing nodes with higher frequency characters because they lead to potentially more complete words. As I process each node, I collect complete words and return them as the result.
...
## Experimental
- BFS explores words level by level, prioritizing shorter words first before moving deeper, resulting a pattern where shorter words are suggested before longer words. DFS on the other hand explores one branch fully before backtracking, leading to an order that depends on how nodes are pushed onto the stack, often prioritizing certain paths unpredictably, but simultaneously grouping similar prefixes together. UCS prioritizes words based on character frequency, meaning more common prefixes appear earlier regardless of depth, creating a mix of short and long words appearing earlier based on their likelihood.
