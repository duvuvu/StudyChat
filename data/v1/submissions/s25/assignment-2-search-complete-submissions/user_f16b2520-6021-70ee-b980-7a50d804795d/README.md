...
Autocomplete might seem like some complicated magic, but at its core, it's just an application of search algorithms on a tree (that's how it's done in this assignment for your simplicity, but it's done very differently in real world). Let's break down how this works:
...
For every character in the word, the code first checks if that character is already a child of the current node. If it is, it moves the current node into that child and then moves on to the next character. If it's not already a child, it makes a new child node for that character, moves into it, and moves on to the next character in the word. Once a word terminates, it sets the last node in that work to be true for is_word. That way our search algortihms know where words end when searching through the tree.
...
First, the code checks if the prefix exists in the character tree. If it doesn't, it skips the rest of the program and returns an empty list. If it does, the program performs a breadth-first search starting from that node. It enqueues the children of a node along with the word they would create (as a node, string pair) and always selects the first one put into the queue. It checks if it's the end of a word, and adds the word to our output list if it is. Then it queues all of its children as well. This process repeats until every node in the tree (downwards from the prefix root) has been checked.
...
My code is very similar to that of my BFS function, with one major difference. This code enqueues children into the front of the queue rather than the back, so that they are selected first when iterating through the queue. This results in a DFS where the characters at the bottom right of the tree are found first.
...
I decided to use a stack-based DFS, because it made it easier to keep track of the words being accumulated and because it required less code changes from my BFS code.
...
I used a heap to store the nodes instead of a queue. I added a comparator definition for < in the node class, so that characters with a higher frequency have a higher priority for selection in the heap (lower heap number though because it's a min heap). I then made sure every new character in the tree starts with 1 frequency, and it increases by 1 whenever it is revisited for building other words. Now, my ucs function will always first pick the unexplored character node with the highest number of words stemming from it.
...
BFS prioritizes shorter words first, and then sorts by the order words were put in.
DFS priotizes longer words, but cares more about the order you input words in the list rather than anything else.
UCS is the best algorithm for finding common root words, because it prioritizes words that have a lot of other words stemming from that word.
