...
The `build_tree` method iteratively processes each word from the input document, adding each character to a prefix tree structure while tracking the frequency of each character's appearance with respect to its parent character. It marks the end of each word to facilitate efficient storage and retrieval of words and their prefixes.
...
The `suggest_bfs` method employs a breadth-first search to explore a prefix tree structure starting from a node that matches a given prefix, collecting potential word suggestions as it traverses through the children of each node, thereby efficiently returning autocomplete suggestions that begin with the specified prefix.
...
The `suggest_dfs` method utilizes a depth-first search to navigate through a trie-like structure starting from a node that matches a given prefix, gathering autocomplete suggestions by exploring as deeply as possible along each branch before backtracking, ultimately returning all words that begin with the specified prefix.
...
A stack-based DFS uses a LIFO Queue (stack) to choose the latest added node as the node to explore next. A recursive DFS expands the tree recursively until it reaches the bottom, using an internal call stack instead of a user-defined one. The difference here is that the recursive DFS expands until it reaches the leftmost leaf first while a stack based DFS expands until it reaches the rightmost leaf first. I used a stack-based DFS since it's implementation was similar to my BFS algorithm, allowing me to reduce code duplication with helper functions.
...
The `suggest_ucs` method implements a uniform cost search to explore a prefix tree structure starting from a node that matches a given prefix, prioritizing paths based on the lower cost path in order to generate autocomplete suggestions. It efficiently finds and returns words that begin with the specified prefix while accounting for character frequencies to minimize search costs. It uses a priority queue, implemented with a heapq to efficiently find the node with the next shortest path cost.
...
The BFS and UCS were fairly similar. They mostly returned words in the order of their length, starting with 3 letter words and ending on the 7 letter words. Though the UCS returned the 6 letter word 'though' before the 5 letter words 'there' and 'their', while BFS maintained the previously described order. The order of words within each length was also different as UCS began with the 4 letter word with 'thou' while BFS began with 'thee'.

The DFS was much different from the others. Since 'r' must be the rightmost node of the prefix tree after the common prefix 'th', DFS found the whole word 'through' before going to other words. It then continued to find the words with "a" next, then the words with "o" next until finally finding the words with "e". Since it's order for exploring (depth-wise exploration) was fundamentally different from BFS and UCS (layered exploration), the result was too.
