...
### Tree diagram
- Tree diagram for `test.txt` here
<img title ="tree" src="https://cdn.discordapp.com/attachments/778325727006687242/1341900336427827210/image.png?ex=67b7ad93&is=67b65c13&hm=83c33fbf340869df0df733f38bb4ff7bf7d79189ded7c22d796ed679f8113c14&">

### Code analysis

- The first loop checks each word inside the document. The second loop checks every character of the word, if char not found, create a new node, if found, move to the next. is_word will be true at the end of the sceond loop. 
...
- First, link the prefix into the node, then enqueue the last char of the prefix, then starts the while loop until 10 suggestions reached, inside the loop, the leftmost node will be checked if its the last char of a word(FIFO), if is, add the prefix and suffix into suggestions. if not, enqueue its children as suffix.

### Your output

- BFS <img title ="BFS" src="https://cdn.discordapp.com/attachments/778325727006687242/1341812403297587250/image.png?ex=67b75bae&is=67b60a2e&hm=be50d35b752699ff1c890bb4483e38ba8630214d391eff6bb3d0186319395dc3&">
...
- Like BFS, but pop the right most instead of left most on stack. First, link the prefix into the node, then enqueue the last char of the prefix, then starts the while loop until 10 suggestions reached, inside the loop, the last node will be checked if its the last char of a word(LIFO), if is, add the prefix and suffix into suggestions. if not, enqueue its children as suffix.

### Your output

- DFS <img title ="DFS" src="https://cdn.discordapp.com/attachments/778325727006687242/1341812052401852436/image.png?ex=67b75b5a&is=67b609da&hm=9061892906c4f7de711f16ced8d776562adefc11998eca5b924f6ea34ecd352b&">

### Recursive DFS vs Stack-based DFS
- If recursion is used, then the while loop would not be necessary, therefore a queue(stack) would not used.  Instead, create a function that will call itself until there are no more children on this branch. Because of the nature of recursive functions, the first suggestion word would be the last one on the tree(last in terms of BFS) which is the same as LIFO. 
...
- Unlike DFS and BFS, UCS will find the lowest cost item first, then check if its a word. If is, add to suggestion. If not, update the cost, then add children of the current node, then loop again.

### Your output

- UCS <img title ="UCS" src="https://cdn.discordapp.com/attachments/778325727006687242/1341925994797338764/image.png?ex=67b7c578&is=67b673f8&hm=c389b1956f9aa3705d3c9a8668c4f81a7b472da62fb576f2ce3440809d2f80b7&">



## Experimental
- Based on the tree diagram generated by test.txt, BFS suggests which is_word comes first by scanning from the left-most branch to the right-most branch, since it uses FIFO. DFS suggests the right-most branch as it uses LIFO. UCS suggests branches with the least amount of cost, meaning branches that get visited the most will get suggested first since its cost-based. 
