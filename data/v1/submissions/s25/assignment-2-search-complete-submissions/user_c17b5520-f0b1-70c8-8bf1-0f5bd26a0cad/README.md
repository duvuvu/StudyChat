...
- For each character in the current word, I first checked if it was already a child of the current node; if not, I created a new child node. I also tracked the word count for the UCS function and marked the final node as a complete word at the end of the iteration.
...
- I first checked if the prefix existed in the tree. Then, I used a queue to track nodes and the words they formed, popping from the front each iteration. If a node represented a valid word, I added it to the result and then I enqueued the node's children.
...
- I first checked if the prefix existed in the tree. Then, I used a stack to track nodes and the words they formed, popping from the top with each iteration. If a node represented a valid word, I added it to the result and then I pushed the node's children onto the stack.
...
- I would've used a helper function for recursive DFS that would call itself with an updated stack. Stack based DFS manages the stack within the while loop, adding each node's children at every iteration within the loop. I chose the stack-based DFS in this code to prevent potential stack overflow from deep recursion and to handle large input sizes more efficiently with manual stack control.
...
- I used a priority queue to add each node's neighbors at every iteration, ensuring nodes were explored based on the shortest cost. This cost was tracked in my build_tree function, allowing for efficient path exploration.
...
## Experimental
- In my code, i saw that BFS prioritized shorter words first, resulting in a structured order, while DFS explored deeper paths first, leading to a less predictable sequence. UCS followed a cost-based approach, often resembling BFS but with slight variations based on cost assignments and priority.
