...
**Answer:** For each word in the file, for each character in a word, start from the root and check if the character is already a child node. If not, create that character as a child node, then move to that node and repeat for the next character. During the process, update its visited frequency and denote the node as the end of a word when reaching the last letter.
...
- Put the intuition of your code here \
**Answer:** Traverse the prefix in the tree, then add the last node in the prefix to the list as a frontier node containing a reference to the tree node, a path cost of zero, and a prefix string. As long as the list is not empty, pop the first frontier node out and add its children as a frontier node that accumulates both path and path cost from its parent. While repeating the steps, if the node in the frontier node is a word, add the accumulated string to the output list.
...
- Put the intuition of your code here \
**Answer:** Traverse the prefix in the tree, then add the last node in the prefix to the list as a frontier node containing a reference to the tree node, a path cost of zero, and a prefix string. As long as the list is not empty, pop the most recently added frontier node out and add its children as a frontier node that accumulates both path and path cost from its parent. While repeating the steps, if the node in the frontier node is a word, add the accumulated string to the output list.
...
**Answer:** I used stack-based DFS in this project. The main difference between these two DFS variations is that one uses an internal call stack as a frontier while the other uses an external stack as a frontier. As per our implementation in this class, there is also another main difference, which is that recursive DFS will explore from left to right of the tree, while stack-based will explore from right to left.
...
- Put the intuition of your code here \
**Answer:** Traverse the prefix in the tree, then add the last node in the prefix to the list as a frontier node containing a reference to the tree node, a path cost of zero, and a prefix string. As long as the list is not empty, pop the frontier node with the lowest weight out and add its children as a frontier node that accumulates both path and path cost from its parent. While repeating the steps, if the node in the frontier node is a word, add the accumulated string to the output list.
...
**Answer:** We can see that BFS always suggests the shortest word possible first, while DFS suggests the word from the right to the left of the tree. UCS, on the other hand, suggests the word that has the highest frequency of the following character first. In other words, UCS suggests the word that takes the smallest path cost to get to first.
