...
graph TD;
    ...
    H_there[H]-->O_though1[O]
    O_though1[O]-->U_though[U]
    U_though[O]-->G_though[G]
    G_though[G]-->H_though2[H]
...
- Iterates through every individual character in every word in the document. Each character is added as the key to the current node's children if it does not already exist, with a new node as the value being created as needed. At the end of each word, the last character's node is marked as being a word. 
...
- The node corresponding to the last letter of the prefix is found and its children are added to the frontier. Each iteration, the next level of the tree is checked for letters corresponding to the ends of words. If one is found, the word is added to the suggestions and then the next level of the tree is added to the frontier.
...
- After the node corresponding to the last letter of the prefix is found, its children are added to the frontier. Every iteration, the last child to be added to the frontier is checked to be a word, (and added to the suggestions if it is) and its children are added until a leaf node is found.
...
- I used Stack-based DFS. In Stack-based DFS, the last node to be added to the frontier is the first to be checked and expanded. In Recursive DFS, the first node to be added to the frontier is the first to be checked and expanded. In either case, the algorithm goes deeper into the tree first.
...
- Here, the path cost from the current node to its neighbor nodes are calculated and stored in a heap. Every iteration, the lowest cost is taken and the node with the corresponding cost is selected, checked for word status, and expanded, with the path costs being added to the existing path cost and added to the heap.
...
## Experimental
- BFS consistently outputted the shortest words corresponding to the prefix first. DFS outputted the words in the tree in reverse order (the words on the right side of the tree were added first, then the middle side, then the left side). UCS started with the words that started with "the" (the most words), then followed by the words started with "thou", then the words starting with "tha", and ending with through (the least words)
