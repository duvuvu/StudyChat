...
![alt text](50B82AD9-AB51-410E-9997-6EF03F8EF4F6_1_102_o-1.jpeg)

### Code analysis

- I built a Trie for this project, representing a tree structure with 26 children for each letter of the alphabet, where words are formed by traversing the pathways. To create the Trie, I iterated through each letter of the words in the text, adding nodes as needed and marking the end of each word in the Node class, resulting in a comprehensive tree that accommodates all the words in the imported text.

## `BFS`

### Code analysis

- For BFS, I utilized a deque to store results and iterated through the word to traverse the tree; if a node had no children, I returned the word, and while the queue was not empty, I appended all children to the queue, collecting words without children in a list, which is returned once the queue is empty.
...
- So I used recursion to make autocomplete using DFS, looping through the same function and extending onto suggestions. After getting to the current letter, I recursively transversed though the other function, which will keep looping as long as there is untraversed children nodes from the original.
...
- Recursive DFS will iterate through the same function with the only change being the node going to the next pointer, with a stack-based DFS looping until some value or until end_word pops up, changing the node at the end of every loop. I used Recursive DFS because I'm more familiar with it and seemed more intuitive to me.
...
- I enhanced the implementation of Uniform Cost Search (UCS) by adding a weight attribute to each node and implementing a comparison function to evaluate these weights, decrementing the weight each time a word traversed a letter node so that more commonly used nodes have lower scores and get processed faster. In the final code, I managed all child nodes with a min-heap, continuously pushing and popping the lowest weighted values to prioritize the most frequently used paths.
...
## Experimental
- When comparing the suggestions generated for the prefix "th" using BFS, DFS, and UCS, BFS prioritizes shorter suggestions, making it particularly useful for quickly typing short words. In contrast, DFS displayed words without consideration for length, favoring longer entries instead. UCS works somewhat similarly to BFS in this situation, favoring the shorter words that are weighed higher before getting to the longer words with a more costly path.
