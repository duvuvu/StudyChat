...
The build_tree method creates a tree where each word is broken down into individual letters, and each letter gets its own spot in the tree. If a letter has already been added for another word, it reuses the same spot. It also keeps track of how often each letter appears and marks the end of each word.
...
The suggest_bfs method explores all possible words that start with a given prefix by checking each letter one step at a time. It looks at all words of a certain length before moving on to longer ones. The result is a list of words with the given prefix, starting with the shortest ones first.
...
The suggest_dfs method searches for words that start with the given prefix by exploring one possible word fully before moving on to the next. It returns words after fully exploring one path at a time.

### Your output

- Put the output you got for the prefixes provided here

th: through, thag, that, thou, though, thought, the, thee, their, there

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.

I used stack based DFS. In recursive it searches the leftmost node first whereas in stack-based it searches the rightmost node first
...
Searches through all the possible words starting with the letters typed. It ranks them based on how likely they are using character frequencies and then returns all the possible options in order from most to least likely.

### Your output

- Put the output you got for the prefixes provided here

th: the, thou, thee, that, thag, though, there, their, thought, through
...
BFS suggested the shortest words first whereas DFS had a random assortment of lengths but if one word was a prefix of another, then it came before the longer version. UCS seemed to have shorter words first but it was not always the shortest. It seemed to combine short while also most likely to be next. BFS searches all the nodes in a level before moving to the next which is why it finds the shortest words first. DFS searches all the way down one path before it moves to the next path which means the length of the beginning words depends on the setup of the tree. Since UCS measures frequency in order to find which words have the shortest path, it tends to have shorter words at the beginning but it does not necessarily mean it will be the shortest.
