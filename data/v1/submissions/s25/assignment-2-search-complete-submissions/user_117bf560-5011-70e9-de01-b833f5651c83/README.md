...
- My code iterates through each of the characters in the words in the document, following the word down the trie and either adding a leaf if the char isn't present, or increasing the pathCost of the corresponding node if it does. Each end node is indicated as such with the is_word attribute. 
...
- Traverses the Trie following the path defined by the given prefix. When it reaches the final character of the prefix, it enqueues all its child nodes. It then enters a loop where it dequeues an element, and adds it to a list of suggested words if it is a complete word. Then it enqueues the children of this element for further exploration. This process continues until the queue is empty, at which point it return the list of suggested words.
...
- Traverses the Trie following the path defined by the given prefix. When it reaches the final character of the prefix, it adds all its child nodes to a stack. It then enters a loop where it pops an element, and adds it to a list of suggested words if it is a complete word. Then it adds the children of this element to the stack for further exploration. This process continues until the stack is empty, at which point it return the list of suggested words.
...
- I used Stack-based primarily because the code was almost the exact same as the BFS. This allowed me to avoid code duplication, and use one standard function for all the searches.
...
- Traverses the Trie following the path defined by the given prefix. When it reaches the final character of the prefix, it enqueues all its child nodes, and then sorts the queue by pathCost. It then enters a loop where it dequeues an element, and adds it to a list of suggested words if it is a complete word. Then it enqueues the children of this element before sorting the queue again. This process continues until the queue is empty, at which point it return the list of suggested words.
### Your output

- the, thou, though, thee, that, thag, thought, there, their, through

## Experimental
- BFS adds the words to it's list based on length, going from 3, to 4, to 5 and finally 6 letter words. DFS adds the words based on the latest initial prefix found (in this case the character "r" in "through). This leads to it showing words based on initial prefix, ("r", than "a", than "o", then "e") with the words with the same letter then organized from least to greatest length.  As for the UCS, the ordering of the words was based primarily on the frequency of the letters of the word. For example, 'e' had a frequency of 4, so 'the' was the first word, and since 'o', and 'u' had frequencies of 3, 'thou' was the second word. In contrast, 'r', 'o', 'u', 'g', and 'h' in 'through' all had frequencies of 1, leading it to be the last word as it had the longest string of 1-frequency characters. 
