...
## 383GPT
Did you use 383GPT at all for this assignment (yes/no)? yes, a bit

## `build_tree`

### Tree diagram
- ![alt text](tree.jpeg "Tree")

### Code analysis

- For each letter in each word of the document, we traverse the tree node by node and check to see if each letter is in the tree. 
If the current letter is not located in the tree, then we add it to the tree as a child of the previous letter in the word. 
We then update the new node's attributes accordingly (pred, value), and mark it as an end_of_word node if it is the last letter in a word.
...
- First we traverse the tree to get to the node that corresponds to the last letter of the prefix, and then we run explore the possible words 
from that point by exploring all possible paths down the tree level by level using BFS. Whenever we explore a node whose end_of_word attribute
is marked as true, we backtrack using the node's pred attribute to recover the entire word and then add it to the list of suggested words. 
Then we just return the list of suggested words when there are no more nodes to explore.
...
- First we traverse the tree to get to the node that corresponds to the last letter of the prefix, and then we run explore the possible words from that point by exploring all possible paths down the tree using stack-based DFS. Whenever we explore a node whose end_of_word attribute is marked as true, we backtrack using the node's pred attribute to recover the entire word and then add it to the list of suggested words. Then we just return the list of suggested words when there are no more nodes to explore.
...
- I used stack-based DFS in this project. In stack-based DFS, the intuition is to grab the last node in the path being explored (using a stack) 
so that we can keep exploring down one path until the entire path has been explored and then move onto the next path. Recursive DFS has the same
intuition of exploring one path until the entire path has been explored, but instead of popping off of a stack to get the next node to explore, the
method just calls itself again for each unvisited neighbor to achieve this.
...
- First we traverse the tree to get to the node that corresponds to the last letter of the prefix, and then we run explore the possible words from that point by exploring all possible paths down the tree using UCS, finding the most likely word that the user wants to type by finding the words with the highest frequency of characters after the prefix first. Whenever we explore a node whose end_of_word attribute is marked as true, we backtrack using the node's pred attribute to recover the entire word and then add it to the list of suggested words. Then we just return the list of suggested words when there are no more nodes to explore.
...
## Experimental
- BFS always returned the shortest word first and the longest word last. The order of the DFS seemed to be based on the 
order of the children of the prefix's last letter (in other words, the order of the tree's words from left to right). UCS returned the words
with the most commonly used next letter, which made its suggestions more relevant and likely.
