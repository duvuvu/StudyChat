...
- First the function loops through all the words in the document. 
- Then for each word, we initialize a new node. 
- The node in the autocomplete class has two attributes: a dictionary `children` and a boolean `is_word`. 
- We also loop through each character for each word.
- We through each character, and checks if the char exists the node's `children`. 
- If it doesn't, we add a new node as value to the dictionary with the char as key. 
- After we finished looping through all the characters, we will change `is_word` for the last character and node pair to true. Indicating that it is the end of one word. 
- This happens for all the words in the document. The tree will have as many children as there are unique first characters in the document, and each child will have as many children as there are unique characters in the word. 

## `BFS`

### Code analysis

- I initialized a function `find_prefix` that takes in `prefix` as a parameter. There are two outputs that this function returns. It returns the last `node` of the entered prefix. I edited it later to return `running_sum` to return the running sum of inverse frequencies for `suggest_ucs`. 
- In `suggest_bfs`, I first call `find_prefix` to get to the last node in the prefix. If the node is none, I will return an empty array of suggested suffixes. Otherwise, I will initialize a `queue` with a tuple including current node and current prefix. I also initialized an array `suggestions`, which will be an array of complete words from the tree. 
- To update suffixes, I will continue to deque the tuples in the queue until the queue is empty. When I deque a tuple, I will do two things. I will check if the node that has been dequed is a word using the is_word property of Node. If it is a word, I will append the "prefix" from the tuple to `suggestions`. 
- The second thing I will do, is to explore the node that I dequed. I will add all the children of the current node to the queue as well. I am adding both a node and a string("prefix") to the tuple. The string I will add will be a prefix that is word + char of the child node. This is so that I can keep incrementing for a word while I explore the tree and so that I can add a full word to `suggestions` if I found a node where `is_word` is true. 
- `suggest_bfs` returns a list of completed words. 
...
- I used `find_prefix` to get to the node that representes the last char in the prefix. 
- I initialzed `suggestions` the list of completed words and a `stack`. 
- The stack, like the queue before, will take in a tuple of the `node` and `prefix`. 
- While the `stack` is not empty, we will pop the top most tuple on the stack and do two things. 
- One of the things we do is check if the node is a word. If it is a word, we append the `prefix` in the same tuple to `suggestions`. 
- The next thing we do is explore the children of the current node and also add them as a `node` and `string` to the stack. The `string` is a concatenation of the current prefix with the char of child node. 
- after the loop completes we return `suggestions`. 
...
- I used a stack-based DFS here. If I had used a recursive dfs, I think the results would be ordered exactly the opposite with `there` coming first and `through` coming last. I used a stack-based approach here, because it was easier for me to visualize what would be happening to each letter. 

## `UCS`

### Code analysis

- I added a new property to `Autocomplete`: a character frequency hash map `self.freq_map`. 
- In `build_tree`, I populated `self.freq_map` with the count of frequencies for each character. 
- In `find_prefix`, I also returned another variable called `running_sum` that keeps the sum of inverse frequency of the nodes/chars that have been visited. 
- In `suggest_ucs`, I used `find_prefix` to find the node of the last char in the input prefix and the running_sum of inverse frequency till that node. 
- I also initialized a priority queue `pq` that takes in a three element tuple: `running_sum`, `prefix`, and `node`, the elements of `pq` are sorted with the lowest `running_sum`. 
- While the `pq` is not empty, we do two things. We check if the current node that we have dequed from `pq` is a word. If it is we append to `suggestions`. We also explore the neighbors of the node. We increment the running_sum of the child characters to the current sum and enque it along with a concatanation of the current prefix and child car to `pq`. 
- In `pq`, the elements are ordered `sum`, `string`, `node`, so that we can compare two strings lexicographically if the there is a tie in the sum. 
...
In BFS vs DFS vs UCS, if I used BFS, shorter autocomplete suggestions are listed first in the output. This is because BFS finds the shortest path first. If I use DFS, the words that are found first are smaller lexicographically at the same position compared to the words that come later. In DFS the order is not decided by the length of a word. If I use UCS, the words that have more characters that frequently occurs or lexicographically smaller comes first. This is because
