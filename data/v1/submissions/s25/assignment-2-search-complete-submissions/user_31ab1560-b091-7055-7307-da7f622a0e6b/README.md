...
build tree: reads the document and splits it into words. Iterates over each word, inserting its characters into a prefix tree. 

suggest_bfs: Implements Breadth-First-Search algorithm to find words starting with the given prefix.
suggest_dfs: Implements Depth-First-Search algorithm to find words with the given prefix.
suggest_ucs: Implements Uniform Cost Search based on character frequency.More frequent characters get a lower cost, making them appear first in suggestions.

### Your output

- Put the output you got for the prefixes provided here

suggest_bfs("th"): ['the', 'thee', 'thou', 'that', 'thag', 'there', 'their', 'though', 'thought', 'through']
suggest_dfs("th"): ['through', 'thag', 'that', 'thou', 'though', 'thought', 'the', 'thee', 'their', 'there']
suggest_ucs("th"): ['thag', 'that', 'thou', 'though', 'thought', 'through', 'the', 'their', 'thee', 'there']
...
Implements Breadth-First-Search algorithm to find words starting with the given prefix. Uses a queue (deque) to explore all possible words layer by layer.
...
Implements Depth-First Search (DFS) to find words with the given prefix. Uses a stack to explore deeper paths first before backtracking.
...
Recursive DFS: Uses the call stack to manage recursive function calls. Each recursive call processes one character and then calls itself for child nodes. Stops when reaching a leaf node or a word-ending node.

Stack-based DFS: Uses an explicit stack (list or deque) to store nodes to be explored. Iterates using a while loop, popping nodes off the stack instead of using recursion.

Used in code: Stack-Based DF
...
Implements Uniform Cost Search (UCS) based on character frequency. More frequent characters get a lower cost, making them appear first in suggestions. Uses heapq to maintain a priority queue based on path cost.

### Your output

- Put the output you got for the prefixes provided here

['thag', 'that', 'thou', 'though', 'thought', 'through', 'the', 'their', 'thee', 'there']

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 

BFS ensures that shorter words appear earlier in the results. DFS produces a different order, depending on the structure of the tree and how deep it goes. UCS prioritizes frequently occurring letter sequences, making it ideal for real-world autocomplete. The optimal choice of algorithm depends on your priorities.
