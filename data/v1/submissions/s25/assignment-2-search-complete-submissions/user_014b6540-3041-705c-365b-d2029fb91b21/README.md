...
<u>
The build tree code traverses through the selected document. By it starts at the root of the nodes, and iterates through each character. If the character does not exist it creates a new node for that node, then it moves the child node to its corresponding character and then increments the node's weight. It then marks the end of a word with is_word = True so that it indicates the end of a word. 
</u>
...
- Put the intuition of your code here
<u>
The BFS searches through the tree by going by "layers". It uses a queue system where it adds each node to the queue to search through the layers. It travels down children and the children of the children searching. It orders the outputs based on the length of the word, since it goes by layers.
</u>
...
<u>the, thee, thou, that, thag, there, their, though, thought, through </u>
...
<u>
The DFS Searches by traversing all the way to the end and then returning to the parent nodes. It outputs all words that share the same "prefix" for example the their there thee share are all in a row because they share the same "prefix" of "e". The DFS implementation I used was a stack based DFS. The stack based is to avoid revisiting nodes and loops until the stack is not empty. Once the node has been visited it is popped from the stack and then puts the unvisited neigbors and children onto the stack until all nodes are visited. 
</u>

### Your output

- Put the output you got for the prefixes provided here
<u> through, thag, that, thou, though, thought, the, thee, their, there </u>
...
- Put the intuition of your code here
<u>
The UCS searches based on the "costs" of each letter/ node. It finds the most "efficient" pathing. It looks for the nodes that costs the least first to traverse to rather than bfs or dfs where they traverse in a certain pattern. The UCS function utilizes a priority queue, and using the weights it finds the nodes with smallest cost, and if ther is a node that has a lower costs it updates it using the priority queue. 
</u>
...
<u>the, thou, thee, that, thag, though, their, there, thought, through
</u>

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 

<u> BFS tends to return based on length, from shortest to longest. DFS tends to group based on the "prefix", in this case the letter following th and so on grouping them together so if "e" was used, it would group all 4 of the outputs together. And UCS generally goes from shortest to longest, which is naturally how the weights are calculated, although it is not always the case.
</u>
