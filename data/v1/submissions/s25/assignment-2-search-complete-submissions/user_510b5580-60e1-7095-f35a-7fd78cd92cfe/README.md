...
- For each word we start at the root and traverse the tree according to each next character in the word. If the next character is already present as a child we simply move to that node, if not then we add a new child node for that character.

## `BFS`

### Code analysis

- First we find the node where the specified prefix ends, then from that node we enqueue all the child nodes. We then repeatedly go through the child nodes at each level and if we find a node with the "is_word" flag we can append it to our word list.
...
- We start again by traversing to the node corrosponding with the end of the prefix. Then we recursively call our DFS helper function on each child node checking for valid words.
...
- I find recursive to be more suited for DFS. This is because we theoretically we want to keep enqueuing until we reach the first end of our tree then we can start working back until we find another branch to go down. You can use a stack for this but simply instantiating a new function to handle each new node to me just feels like a cleaner implementation.
...
- My implementation does path cost during the enqueuing. I had to add comparison methods comparing node characters to my node class because the queue will try and compare the enqueued node objects if the costs are the same.
...
- DFS returns words based on the starting prefix, so the closest words to the prefix get generated first. BFS prioritizes shorter words first and UCS prioritizes branches that branch a lot so theoretically more frequent prefixs are explored first.
