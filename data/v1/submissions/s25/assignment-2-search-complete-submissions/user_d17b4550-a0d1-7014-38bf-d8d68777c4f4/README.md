...
Iterates through each word in the input document, splitting it into individual characters. For each character in a word, it either creates a new node in the trie if that character's path doesn't exist, or follows the existing path. Once all characters of a word are processed, marks the final node as a complete word. This builds a tree structure where each path from root to a marked node represents a word from the document.
...
First traverses down the trie following the prefix and if any character isn't found, returns empty list. Then from that node(the last character in prefix), explores all possible paths level by level using a queue, building complete words by appending characters. When a end-of-word node is processed, adds the constructed word to suggestions list. Returns the list of suggestion words.
### Your output
...
The method starts from the node corresponding to the last character of the given prefix. It uses recursion to explore the trie depth-first, meaning it go deep letter by letter into one path until it finishes a word before finding the next whole word. This approach identifies words that share common patterns(prefixes), allowing related suggestions to be grouped together. As it traverses the tree, it checks for complete words that match the prefix and adds them to the list of suggestions.
...
Recursive DFS uses call stack to temporarily store the intermediate nodes and traverse the tree, this method tends to be simpler and more readable, but it can lead to stack overflow for trees with significant depths. Stack-based DFS uses an explicit stack data structure, which avoids stack overflow and more memory efficient. For this assignment, I used recursive DFS for its simplicity.
...
The method starts from the node corresponding to the last character of the given prefix. It uses a priority queue to explore the trie, prioritizing paths with lower cumulative costs. The cost is determined by the frequency of character transitions, with more frequent transitions having lower costs. As it traverses the tree, it checks for complete words that match the prefix and adds them to the list of suggestions, ensuring that more likely words are suggested first.
...
### Experimental
- BFS tends to suggest shorter words first as it explores level by level. This can be useful to find short words since long words appear later on.
- DFS dives deep into one path before backtracking, which may result in longer and less common words appearing earlier in the suggestions.
- UCS prioritizes more frequent character transitions, suggesting more likely words first based on the given prefix so it is more balanced and not get biased based on the length of a word. In my opinion, this approach gives more relevant suggestions most of the times in general cases compared to the other 2.
