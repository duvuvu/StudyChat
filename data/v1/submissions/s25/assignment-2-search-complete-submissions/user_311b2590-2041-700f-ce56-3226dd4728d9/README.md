...
![alt text](tree_diagram.jpg)

### Code analysis

- Put the intuition of your code here

At every step I created a new node for each new character. If the character already existed in the tree then I would move down the tree to add new characters. the is_word property determines it we've reached the end of a word in our tree. The freq property is updated every time the node is encountered. It is the count of words with that character at that position. The inv_freq is the reciprocal of freq.
...
Used a queue to  go through the tree layer by layer. All the nodes in the same layer are explored before their children nodes. Every time I encountered a node for which the is_word property was true, I added the word to the list of suggestions.
...
Used a stack (LIFO) to explore the nodes that were added last (children) first. Every time I encountered a node for which the is_word property was true, I added the word to the list of suggestions.
...
Recursive DFS explores the nodes from left to right while stack-based DFS does it from right to left. I used stack-based DFS using two deques to track nodes and their corresponding words.
...
Used a priority queue to explore nodes ordered by inverse frequency. Every time I encountered a node for which the is_word property was true, I added the word to the list of suggestions.
...
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS.
  ...
  BFS explores the nodes in the same layer (same distance from root node) first. It returns searches in lexicographical order
  DFS explores each branch fully before backtracking. DFS order depends more on the tree structure
  UCS explores the subtree with more number of words first. It returns searches ordered by the inverse frequency of characters appearing after previous characters
