...
3. **`utilities.py`** - This file contains the code to read the document provided and building the Graphical User Interface for the autocomplete feature. This file is not related to the core logic of the autocomplete feature. Please <redacted> not modify this file.
...
Iterates through each word in the document, inserting each character into the tree while updating frequency counts and marking nodes where words end.
...
Uses a queue to perform a level-order traversal starting from the node corresponding to the prefix. Words are collected when the node's is_word flag is True.
...
Uses a recursive function to traverse all branches from the prefix node. Whenever a node marked as a word end is reached, the corresponding word is added to the suggestions list.
...
I chose recursive DFS since it is clearer, more concise, and easier to implement. This is because recursion in itself already makes use of the call stack automatically. On the other hand, for stack-based DFS, I will have to manage an explicit stack.
...
Computes a cumulative cost for each path using the inverse of each child's frequency and uses a priority queue (min-heap) to make sure that lower-cost word completions are explored first. Any complete word encountered is added to the suggestions list.
...
BFS returns suggestions in order of increasing word depth, so shorter words will appear first. DFS explores one branch completely before moving to others. UCS orders suggestions based on cost derived from frequency, which will bias the search towards more frequent transitions and, therefore, potentially more common words.
