...
style="width:8.26389in;height:7.18766in" />

### Code analysis

- The function build_tree() searches every word and each character of every word. It builds a prefix tree data structure by creating a node for each character, iff a node doesn't already exist.
...
- The function suggest_bfs() traverses the prefix tree, by checking every node, until it reaches the last character of the input prefix. Using a frontier implementation of BFS using a queue (FIFO), it appends words as suggestions when a subtree is complete (complete word).
...
- The function suggest_dfs() traverses the prefix tree, by checking every node, until it reaches the last character of the input prefix. Using a recursive implementation of DFS (LIFO) to explore the full deapth of each subtree, it appends words as suggestions when a subtree is complete (complete word).
### Your output

- the there their thee thou though thought that thag through

### Recursive DFS vs Stack-based DFS
- Recursive DFS leverages the call stack to backtrack nodes after exploring a branch (expand recursively). On the other hand, stack-based DFS manually manages the exploration using an explicit stack. I used recursive DFS because I find it simpler.
...
- The function suggest_ucs() traverses the prefix tree to the node matching the input prefix while accumulating a cost. Once done, it employs a heap to expand nodes in order of increasing cost and appends suggestions when completing a subtree (complete word).
...
## Experimental
- BFS based suggestions on the words' level within the prefix tree, thereby returning shorter words first. DFS based suggestions by completely exploring each subtree, theyby returning words in an order based on the last nodes orders (all 'tha...' first, then 'tho...', etc...). UCS based suggestions on frequency, yielding the most common completions first - it seemed to be the most accurate!
