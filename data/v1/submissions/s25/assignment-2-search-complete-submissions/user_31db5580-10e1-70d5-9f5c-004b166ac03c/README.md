...
- Put the tree diagram for `test.txt` here
  ![New Note](https://github.com/user-attachments/assets/1984e124-ef89-4f5e-a3ec-e6004fc898ef)


### Code analysis

- The `build_tree` function constructs a tree by iterating through each word in the document and each character in the word. For each character, it creates nodes in the tree, it marks the end of a word by setting `is_word` to `True` and storing the complete word in the node for word storage.

### Your output

- No prefix output for `build_tree`
...
- This performs a breadth - first search on the tree to find complete words starting at the given prefix. Starts with traversal up till the last character of the given prefix. Then, it uses a queue, traverses through checking for a compelte word at each step and otherwise adds next nodes to the queue (frontier in math terms). Words are found in order of their length. 
...
- This performs depth - first search on the tree similar to the bfs. Starts with traversal up till the last character of the given prefix. Then, it uses a stack which allows it to explore deeply giving longer words first unlike BFS. Checks for complete words at each step otherwise adds nodes to the stack.
...
- I've used stack based DFS here. This uses an explicit stack to track the nodes or rather the frontier. This might be better for very large trees. 
- Recursive DFS at each node calls itself on its child nodes until there are no more to visit, and then it backtracks. 

## `UCS`

### Code analysis
- Performs Uniform COst Search to find complete words ofr the given prefix. Starts with traversal up till the last character of the given prefix. Uses a priority queue or min heap where the cost or heuristic used is how often characters appear after the prefix. Avoids double exploring of nodes by maintiang a visited set. Collects complete words or adds its children to the frontier here stored in a heap. 
...
## Experimental
- BFS searches for the shortes word first and then moves to the longer words. It moves level by level, ordering by alphabetical where required. Goof for finding shorter words. May not prioritize more often used words/ contexually relevant ones which we may need in this autocomplete use case of the algorithm.
- DFS searches deep path first so it suggests longer words first but still not in exact descending order since it explores the path until a word is found or the end is reached, it goes to the last unexplored node not the next longest word. Not a consistent algorithm. Still deosn't give contextaully relevant words needed for an autocomplete algorithm.
- UCS wokr similar to BFS but it uses a cost function for some order of preference, it prioritizes words based on words with more repeating characters. 
