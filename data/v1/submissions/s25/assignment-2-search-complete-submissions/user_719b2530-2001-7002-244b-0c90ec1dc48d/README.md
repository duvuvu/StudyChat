...
-   https://imgur.com/a/qmepiqT

### Code analysis

-   Put the intuition of your code here
-   Checks through each char in the word, and if its not already a child of the node, it creates a new node. Said node is then reassigned to the child to continue down, and the frequency of the char increases. The char node is_word is set to true once a word is found and after the loops function call calculate_path_costs is called.
...
-   Put the intuition of your code here
-   Traverses to the end of the prefix, returns nothing if the prefix isnt found. Queue stores node and current word, using FIFO it pops and appends the suggestion, and continues to add to the queue for all the child nodes in the children dictionary.
...
-   Put the intuition of your code here
-   Traverses to the end of the prefix, returns nothing if the prefix isnt found. Helper function recursively appends word suggestion if it is a word, or else recursively calls the helper method for the child nodes in the children dictionary.
...
-   Recursive DFS uses the system's call stack to remember state, and as the call returns it backtracks to the previous node. Stack based DFS uses an explicit stack to keep track of the traversals manually, can be suitable for deeper structures. I chose the recursive DFS for simplicity and because our data set won't run into any depth limits.
...
-   Put the intuition of your code here
-   Traverses to the end of the prefix, and returns nothing if the prefix isnt found. Uses a priority queue, pushing the cost, accumulated suggestion, and the current node in a tuple. Appends to suggestions if the current node is a word, and traverses through the children, pushing to the queue with the path cost and word.
...
-   In the BFS, the order of the suggestions looks like its in order of length, which makes sense as its "breadth first". In the DFS it almost looks like they are grouped together by their similar characters, like the first four the third letter is e, then 3 are o, etc. UTC seems like it would give the most effective suggestion, calculating the frequency of the next character seems to be very helpful.
