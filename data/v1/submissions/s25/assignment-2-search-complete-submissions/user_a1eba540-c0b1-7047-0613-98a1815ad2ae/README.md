...
- The function takes a string of words and splits it into a list. It iterates through each character of each word, creating nodes if the character doesn't exist in the current node's children, effectively building the trie. It records the frequency of each character at each node for potential use in suggestions. Marks the node as a complete word and stores the full word once all characters are processed.
The function efficiently constructs a trie from the given document, enabling quick prefix searching and storing relevant frequency data, while also marking word completions for accurate retrieval.
...
- BFS (Breadth-First Search) is a tree traversal algorithm that explores all the nodes at the present depth prior to moving on to the nodes at the next depth level.
In our context, BFS will search for all possible words that start with a given prefix, expanding level by level through the trie until it gathers the first several valid word completions.
A queue is utilized to manage nodes for exploration, storing the current node and its corresponding prefix string. The method iterates through the children of nodes while appending new nodes to the queue.

### Your output
- <img width="1512" alt="Screenshot 2025-02-21 at 11 09 59 PM" src="https://github.com/user-attachments/assets/cbc8dd9b-35f8-455d-a6e8-0f12cf6ef818" />
...
- DFS (Depth-First Search) is a traversal method that explores as far down a branch of the tree as possible before backtracking.
In the implementation here, DFS collects words that match the prefix by exploring each branch completely before moving to the next branch.
A stack is used to store nodes for exploration, along with the current constructed word prefix. The stack-based approach allows us to manage backtracking manually.

### Your output


- <img width="1512" alt="Screenshot 2025-02-21 at 11 10 26 PM" src="https://github.com/user-attachments/assets/2d71c1f1-c372-4e0e-812a-be69e676c74f" />

### Recursive DFS vs Stack-based DFS
- Recursive DFS: This method relies on the call stack to keep track of the nodes in the current path, automatically backtracking when a path ends. It is often more elegant and easier to implement, but can lead to stack overflow if the depth becomes too large.
Stack-based DFS: This method explicitly uses a stack data structure to manage exploration paths. It provides more control over the process and avoids recursion limits, making it more suitable for scenarios where deep recursion might occur.
Which one did you use here?: In the given code, I used a stack-based DFS implementation. This was done to avoid potential issues with recursion limits in Python and to have better control over the traversal and backtracking process.
...
- UCS (Uniform Cost Search) is an adaptation of BFS that incorporates path costs when determining which node to explore next. In our context, we use the frequency of characters to inform the cost.
The priority queue ensures that we explore the least costly paths first, and this is helpful in situations where you want to prioritize suggestions based on the commonality of character sequences in valid words.
The effectiveness of UCS lies in its ability to find more optimal completions based on user input.

### Your output

- <img width="1512" alt="Screenshot 2025-02-21 at 11 09 05 PM" src="https://github.com/user-attachments/assets/b76c965c-d712-44c8-a4d8-44b28962ebb9" />




## Experimental
- Tends to return a larger number of suggestions that may vary broadly in terms of popularity and relevance. It's efficient and straightforward in generating suggestions quickly.
DFS: The suggestions may be influenced by deeper paths in the trie. If deeper or less common words closely match the prefix, they might be prioritized, leading to different results compared to BFS. DFS can generate results in different orders each time due to the nature of exploring one branch fully at a time.
UCS: Provides results based on the frequency of characters, meaning commonly used words based on character frequency are suggested first. This can lead to suggestions that are more aligned with user behavior and word usage, making UCS potentially the most useful in the context of autocomplete suggestions.
In summary, the choice of algorithm for autocomplete significantly influences the output. BFS is quick and broad, DFS can give depth, and UCS aims for informed, relevant suggestions based on frequency. Each method has its trade-offs, making the choice dependent on the application’s purpose and user needs. 
