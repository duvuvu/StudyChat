...
flowchart TD
    A["Root"] --> B("T")
    B --> n1["H"]
    n1 --> n2["O"] & n7["A"] & n9["E"] & n16["R"]
    n2 --> n3["U"]
    n3 --> n4["G"]
    n4 --> n5["H"]
    n5 --> n6["T"]
    n7 --> n8["T"] & n15["G"]
    n9 --> n10["R"] & n12["I"] & n14["E"]
    n10 --> n11["E"]
    n12 --> n13["R"]
    n16 --> n17["O"]
    n17 --> n18["U"]
...
- The `build_tree` function I implemented constructs a Trie (prefix tree) from a given document by processing each word character by character to create nodes for each prefix in the Trie. It maintains a frequency count of each prefix encountered, which is later used to calculate a cost for each Trie node. It marks the end of each word with is_word=True and recursively assigns costs to each node
...
- The `suggest_bfs` function I implemented generates suggestions for a given prefix by first navigating through the Trie to locate the node that corresponds to the end of the prefix. It then performs a breadth first search (BFS) from that node, collecting all complete words (indicated by `is_word` being `True`) by traversing through the potential child nodes, effectively returning a list of suggestions that start with the specified prefix in order of increasing length from the prefix.
...
- The suggest_dfs function implements a Depth-First Search (DFS) algorithm to generate autocomplete suggestions for a given prefix in a prefix tree. It begins by navigating to the node representing the last character of the prefix, then uses a stack to explore the tree by following the deepest paths first, adding complete words (marked by currNode.is_word=True) to the suggestions list in reverse alphabetical order due to the reversed iteration of children.
...
- In a recursive depth-first search (DFS), the algorithm leverages the function call stack to traverse the structure, returning to previous nodes automatically as function calls complete. This approach is more intuitive and cleaner in terms of code, as it naturally expresses the recursive nature of the traversal, allowing for easy handling of base cases and child node explorations.
In contrast, a stack-based DFS explicitly manages the traversal state by using a stack data structure. This approach allows for more control over the order of node exploration and can help avoid hitting recursion limits. The stack-based method can be more effective in scenarios where the depth of recursion is high or when non-recursive behavior is preferred for clarity or efficiency.
In the `suggest_dfs` function I implemented, a stack based DFS approach is used. This method allows for an iterative traversal without the potential limitations associated with recursive depth, making it suitable for handling deeply nested Trie structures.
...
- The `suggest_ucs` function I implemented uses Uniform Cost Search (UCS) to generate suggestions based on a given prefix. It begins by navigating through the Trie to locate the node that corresponds with the end of the prefix. Then, it employs a priority queue (min-heap) to explore the Trie, prioritizing child nodes based on their associated cost, which reflects the frequency of their prefixes (lower cost indicates higher frequency due to costs being inverse frequencies). The function collects complete words as it traverses the Trie in cost order, returning a list of suggestions that prioritize more frequent words.
...
- I made the following test file to experiment with the various search functions: the the the the tho tho thx thx they there their thnx thicc thot thug
- I found that BFS explores all child nodes at the current level before moving on to the next level. This means that it tends to return suggestions based on prefixes that are complete first, and then subsequently explores deeper prefixes at the same level. So it prioritizes shorter words (e.g., "the", "tho", "thx") over longer ones.

- I noticed that DFS explores as far down a branch as possible before backtracking. This means suggestions may prioritize longer words that descend deep into the Trie before considering shorter or more common words. Also, it can include longer words early in the result set and may not cover all variations.

- I found that UCS prioritizes paths with the lowest cumulative cost, where costs are inverse frequencies
- "the" (freq=4) comes first because its path cost is lowest (1/4 = 0.25), followed by "tho" and "thx" (both freq=2, cost=0.5), then less frequent words, with ties broken alphabetically or by heap order.
