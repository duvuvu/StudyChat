...
![image info](./images/treediagram.png)

### Code analysis

Splits the text file into words, then each character in a word becomes a child node, and node.is_word = True is set at the last character of a word. The frequency of each edge is kept tracked in node.edge_freq[char] for use in UCS.
...
Uses a queue to do a level by level traversal from a node downwards, grabbing all the possible words that extend the prefix. We then return suggestions in the order BFS encounters them (shorter levels first)
...
Uses a stack to do a depth first traversal from a node, and returns suggestions in order they're discovered by DFS (going deep down a path before trying siblings)
...
I used a stack based DFS because it is faster than recursive DFS in python and you hit a limit on how big of trees you can process with recursive DFS (not that we would hit that limit in this project), it is also easier to debug and you dont have to rely on how Python processes the call stack for recursion. 
...
Here I used a priority queue to perform UCS, the cost to move from one node to a child node is 1/edge_freq where that is how many times that child edge has appeared in the entire document. Lower path cost => more frequent sequences => expansions come earlier from the priority queue (in theory)
...
## Experimental
BFS: I saw shorter words come first "the, thee, thou" before longer words like "though, thought", since BFS explores all nodes at a given depth level first this makes sense. 
DFS: The first word might be long (e.g "through") since DFS goes all the way down one path before exploring siblings. You can see it pick "through" and then backtrack to simpler words like "thag" and "that".
UCS: Here the word order isn't shortest word first nor depth-based but is based on our cost function 1/edge frequency. Paths with frequently used edges have lower cost, so UCS would return those words first. The word ordering is different when you compare the UCS and BFS so im hoping my implementation is correct. 
