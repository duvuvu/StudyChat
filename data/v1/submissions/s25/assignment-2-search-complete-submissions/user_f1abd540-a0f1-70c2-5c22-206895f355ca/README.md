...
first the document is split by the spaces. Then for each word we start with the root. If the character is present it explores the children from the node. If it doesn't then it creates a node and explores from there. Then it keeps changing the node reference and explores till the end of the  word
...
- Put the intuition of your code here
we start at root node and iterate over the prefix. Then we explore its children and add them to a queue. At each step we dequeue and check first if it the end of the word. Then we add the children of it to the queue and this goes on until we get the final word.
...
- Put the intuition of your code here
We start at the root node and iterate over the prefix.If it is not found we return an empty list. We use a helper method which checks if it is the end of the word, if it is. it adds the all prefix to suggestions. after the tree is explored at the depth we come back and start again exploring the other paths and suggestions is returned.
...
recursive dfs- calls the stack and backtraces automatically. When the word is found or a leaf node is met, it goes to the previous state and searches again if needed.
stack dfs- uses an actual stack and requires more management and explicitly adding of nodes.
Used recursive dfs.
...
- Put the intuition of your code here
first frequency is calculated: for giver prefix, what are the most common subsequent characters. A priority queue is used, following low cost paths(characters that appear more frequently are lower in cost). When the end of a word is reached we add the word to the suggestions.
...
bfs and dfs generally suggests shortest words first but in different order ith respect to different prefixes. dfs explores all subsequent related children before suggests other suggestions. ucs gives suggestions that are of words that have more commonly found characters first before the rest relative in size.
