...
- The build_tree method iterates through each word in the document and, for every character, checks if a corresponding child node exists. If not, it creates a new node and initializes its frequency to 1, otherwise, it increments the frequency counter. At the end of a word, the node’s is_word flag is set to True.

### Your output

- Put the output you got for the prefixes provided here
- Output with suggest_random: [thtrw, thwyk, thvgi, thotw, thoow]

## `BFS`

### Code analysis

- The suggest_bfs method first finds the node that represents the end of the given prefix. It then uses a queue to traverse the tree level-by-level, appending a suggestion every time it encounters a node marked as a complete word. This ensures that shorter completions or words closer to the prefix are found first.
...
- The suggest_dfs method uses a recursive helper function to traverse from the prefix node down through each branch. As soon as it finds a node that marks the end of a word, it appends the current path (the full word) to the suggestions list.
...
- Recursive DFS leverages the call stack for backtracking, resulting in cleaner and more concise code. However, it may run into recursion depth issues on very deep trees. Stack-based DFS, by contrast, uses an explicit stack to control the traversal, which can be more robust for deep or highly branched trees. In this assignment, I used recursive DFS due to its simplicity and clarity.
...
- The suggest_ucs method uses a min-heap priority queue to explore nodes based on cumulative cost. Here, the cost of moving from one node to a child is defined as the inverse of the child node’s frequency. This means more frequent letter transitions incur a lower cost, so words with more common paths are suggested first.
...
- BFS: Returns words in order of their level in the tree. This results in shorter words or those with fewer additional characters beyond the prefix being suggested first.
- DFS: Explores a full branch before backtracking, which leads to suggestions that dive deep into one branch and output longer words before moving to other branches.
- UCS: This approach prioritizes more common continuations and yields more accurate autocomplete results.

These differences illustrate that the choice of search algorithm affects not only the order of the suggestions but also their relevance based on word length and frequency.
