...
![alt text](383HWTree.jpg)
### Code analysis

- Put the intuition of your code here
For this code, since the provided code already iterated and split through each word, I only needed to iterate through each node (character) in each word. I made the node equal the next child of the node, and if the child didn't exist I made it stop the traversal.
...
For the BFS, I used the previous code of build_tree to traverse each word and character, and then for the actual BFS mechanism I created a queue with the tuple of the possible words, and the bfs search would print the remaining words of the given child node. Since it was BFS, it would traverse the list by popping left.
...
- Put the intuition of your code here
This was very similar to the BFS, but since it was DFS it would pop from the bottom of the stack instead of left.
### Your output
...
I used a stack-based DFS, because it was similar to the BFS for consistency's sake.
...
- Put the intuition of your code here
For the UCS I created a priority queue which was based off the length of the words. The initial cost was 0 and it would add the length of the words to the new_cost and traverse the list from smallest to greatest cost.
### Your output
...
The main differences in the returns of BFS is that it would sort the autocomplete by numerical order, so it would prioritize the least amount of letters before alphabetical order. DFS was the opposite, where it prioritized the alphabetical order before the amount of letters. UCS was similar to BFS because the cost of the words was determined by the amount of time it takes to get through the word, so the amount of letters as well- opting for that before it prioritizes the alphabetical order.
