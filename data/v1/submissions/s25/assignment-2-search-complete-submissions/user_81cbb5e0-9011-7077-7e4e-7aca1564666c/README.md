...
### Tree diagram
![image](https://github.com/user-attachments/assets/c26ddc69-64cf-4a47-a659-3633946e66a5)


### Code analysis

This function iterates through words in a document and checks if the word is a child of the root node. If it is not already a child node, it creates one and makes that the next node to start iterations from. The loop ends if all words are in the tree.
...
The loop iteratively checks if each character of the prefix exists in the current node. If any character is missing, it returns an empty list indicating that no completions are available for the given prefix. For BFS, a queue is initialized with the current node and prefix and we use a deque for queue operations. The while loop runs as long as there are items in the queue and there is a check to see if the word has been searched and found. New entries are appended to the queue over iterations if required over multiple iterations.

### Your output

![image](https://github.com/user-attachments/assets/6f1c2e1b-41f2-43fc-9669-d7ef7059ce10)
...
The loop iteratively checks if each character of the prefix exists in the current node. If any character is missing, it returns an empty list indicating that no completions are available for the given prefix. For DFS, a stack is initialized with a tuple containing the current node and prefix. The while loop runs as long as there are items in the stack and there is a check to see if the word has been searched and found. New entries are appended to the stack if required over multiple iterations.

### Your output

![image](https://github.com/user-attachments/assets/30e8021e-3ba5-42ad-8bef-319f64c78359)



### Recursive DFS vs Stack-based DFS
Recursive DFS is based on the principle of call stack and recursion. When you call a function recursively, it keeps pushing function calls onto the call stack until it hits a base case. In the context of traversing a tree or graph, you visit a node, mark it, and then recursively visit all of its unvisited children. Once all children of a node have been visited, the recursive call unwinds, returning to the previous level of the tree or graph.

Stack-based DFS uses an explicit stack to manage the nodes that need to be visited. Instead of relying on the call stack, you push nodes onto a stack and pop them off as you traverse the structure. This approach simulates the call stack behavior with manual stack management, allowing you to control traversal in an iterative manner.

I used stack-based DFS in autocomplete.py.

## `UCS`

### Code analysis

The loop iteratively checks if each character of the prefix exists in the current node. If any character is missing, it returns an empty list indicating that no completions are available for the given prefix. A priority queue is used to tack nodes based on cost in UCS. The heap is initialized with a tuple containing the cost (starting at 0), the current prefix, and the current node. In UCS, nodes are processed based on their cost, promoting nodes that appear to be less costly first. During each iteration loop, the node with the lowest cost is popped from the priority queue. All child nodes are pushed into the priority queue with the updated cost (incremented by 1), this correctly accounts for the depth in the tree and the function return the list of completions.

### Your output

![image](https://github.com/user-attachments/assets/dba909cc-4e21-4625-ad5a-170d261f163c)



## Experimental
In BFS, shorter and diverse words are prioritized.
In DFS, longer words are emphasized first.
In UCS, there is hybrid behavior. Results are similar to BFS unless specified costs change the outcomes.
