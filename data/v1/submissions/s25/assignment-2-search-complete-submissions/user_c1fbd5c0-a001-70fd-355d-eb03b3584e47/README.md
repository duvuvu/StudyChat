...
![alt text](TestDiagram.png)

### Code analysis

- Put the intuition of your code here

Iterates through each character of each word in the list. If for the current letter, a node does not currently exist following the specified prefix, a node is added. The path cost for that letter is also incremented. After the function finishes iterating through a word, the last letter in the word is indicated to be the last letter in the word. Finally, each node's inverse path cost is calculated. 
...
Iterates through the tree data structure to find where the prefix ends, setting the node value to the last char of the prefix. The function then runs a BFS by using a queue, first enquing the starting node and the starting prefix. For each node dequeud from the front of the queue, if the node is indicated to be the last letter of a word, then the word is added to the suggest list. Otherwise, each of the nodes children is appended to the end of the queue with each of the child nodes letters being added to their corresponding prefixes.
...
Iterates through the tree data structure to find where the prefix ends, setting the node value to the last char of the prefix. The function then runs a DFS by using a stack, first pushing the starting node and the starting prefix. For each node popped from the top of the stack, if the node is indicated to be the last letter of a word, then the word is added to the suggest list. Otherwise, each of the nodes children is pushed to the top of the stack with each of the child nodes letters being added to their corresponding prefixes.
...
For a recursvie DFS, the function uses the call stack to keep track of the nodes, which each node being visited going down into the tree until the lowest level of that path is reached, and then bubbling the result back up. For this excersise, a recursive approach would reach the last letter of a word and then bubble up suffixes. A stack-based DFS uses a LIFO structure like a stack, taking the most recently explored node each time until it reaches the end of the path, and then backtracking. I used a stack-based DFS, as it intuitively built the prefix each time it went down a path. 
...
Iterates through the tree data structure to find where the prefix ends, setting the node value to the last char of the prefix. The function then runs a UCS by using a heap, first pushing the starting node and the starting prefix. To figure out which node should be popped based on least value, the heap uses a magic method that defines the less then operator for a Node by comparing the path_costs of the nodes and choosing the smallest path_cost. For each node popped from the heap, if the node is indicated to be the last letter of a word, then the word is added to the suggest list. Otherwise, each of the nodes children is pushed to the heap with each of the child nodes letters being added to their corresponding prefixes and their path costs being incremented. 
...
There were some key differences in the suggestions generated depending on whether BFS, DFS, or UCS were used. For example, BFS and UCS prioritized shorter words which did not come up in DFS. However, while BFS seemed to stay strictly to this, UCS would sometimes show certain longer words higher than shorter words largely due on whether an uncommon letter was present or not. Furthmore, DFS ended up grouping words with the same prefixes together in the suggestion box. Based on experimentation and the code structure, BFS's suggestion depended the most on word length, DFS's suggestions on word prefix, and UCS's on letter frequencies. 
