...
![Alt text](https://raw.githubusercontent.com/COMPSCI-383-Spring2025/assignment-2-search-complete-<redacted>/refs/heads/main/IMG_1424.jpeg?token=GHSAT0AAAAAAC6XQBVJ5OORNMM23QGM3O3GZ55HLGQ)
### Code analysis

- Put the intuition of your code here

My code created a node for each child of the current node starting at the root then repeating, for each child of the node. If it functioned correctly the second half of the function counted how many times each node occurs in the tree then used the helper function calcCost to determine the inverse, assigning that code to each node. 

## `BFS`

### Code analysis

My code functions as a classic BFS function, starting that the root and checking to make sure the nodes children assigns with what is being searched for. Then enqueue all of the current nodes children, search them, then once all of that node's children have been explored we move onto the the children on the first node we searched on that level of the tree. 
### Your output

- Put the output you got for the prefixes provided here
![alt text](https://raw.githubusercontent.com/COMPSCI-383-Spring2025/assignment-2-search-complete-<redacted>/refs/heads/main/BFSimage.png?token=GHSAT0AAAAAAC6XQBVJZN3XQC4TNZ4B4B4MZ55HK4A)
...
The code is the essentially the same as my BFS function, but it uses a stack instead of a queue. This allows my code to do a FILO (First In - Last Out) search, searching the depth of the tree then going back and searching each other branch afterward. 
### Your output

- Put the output you got for the prefixes provided here
![Alt text](https://raw.githubusercontent.com/COMPSCI-383-Spring2025/assignment-2-search-complete-<redacted>/refs/heads/main/DFSimage.png?token=GHSAT0AAAAAAC6XQBVJKJ5MOAESNDNSXF6YZ55HLBA)
### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.

 As mentioned I took the stack approach because I it is better for time complexity and is easier to code. The main difference between the stack approach and the recursive approach is that the recursive approach creates many new instances of the search problem for each node that it "discovers" wheras the Stack approach only functions in one instance, and takes advantage of the stack data structure to make it appear DFS. 
...
The UCS is almost identical to my BFS appreach besides the fact that it uses a priority queue to utilize the heappop function that pop the child node with the smallest cost as determined by the build_tree function. Since the cost is determined by the inverse of how frequently that character occurs in the words in the .txt document after the current prefix, the function should return each autocomplete suggestion in the order with the smallest path based on the frequency of occurance. 

### Your output

- Put the output you got for the prefixes provided here

![Alt text](https://raw.githubusercontent.com/COMPSCI-383-Spring2025/assignment-2-search-complete-<redacted>/refs/heads/main/UCSimage.png?token=GHSAT0AAAAAAC6XQBVJPH3ZTLK6ITQBKITWZ55HGPQ)

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 

In BFS I noticed that the nodes with the smallest length are recommended first, with the longer words coming after in ascending order. This makes sense based on the intuition of a BFS search because the nodes smallest paths will be explored first. 

In the DFS I noticed the opposite of BFS, the the LONGEST word in the document appeared first. Specifically when using the test.txt document the word "through" was shown first. This makes sense because the DFS alogirthm would finish the "through" path before moving onto the next one. Why it when for the word through I am not sure, however it could be accounted to how the build_tree function was utilized. I also noticed that when typing only the letter "th" into the GUI when the suggestions were grouped based on their third letter. Specifically it was all the words beginning with "thr", 'tha", "tho", then "the". This also makes sense because children for the prefix "th" would be the nodes "r", "a", "o", and "e" and the DFS algorithm would exhaust all of the paths coming from "r", then "a", ... and repeated until the entire tree has been searched. 

Finally in the UCS, its harder to draw a concrete connection because I wasn't entirely sure what to expect based on the code spec, and I wasn't entirely sure if how I created my build_tree function was correct. However the intuiton makes sense upon reinspection. Using the test.txt document, just typing "t" the first suggestion was the word "the" which makes sense intuitively because the letters "h" and/or "e" appear the most frequently in all or almost all of the preceding words besides "that" and "thag", next was the recommendation "thou" which also make sense because the suffix "ou" appears the second most in the document, in the words like "thr-ough" and "th-ough-t", the yields for the UCS would be interpreted more freely, so I can't say for certain my function words EXACTLY correctly. However I did notice that the prefix "thr" was suggested last which validates my intuition because the occurance of the letter "r" in the document the least amount of times which would mean that node would have the highest cost hence why it would be recommended last. 
