...
For each word in the input document, traverse down the tree one letter at a time until the word ends. Extend the tree if necessary if the letters don't yet exist in the tree. Additionally, record the frequency each node is being traversed during the tree creation (this will help for UCS).

### Your output

- Put the output you got for the prefixes provided here
  I think this question is a typo.
...
Enqueue all child letters of the last letter of the input prefix. Then, until the queue is empty: pop the letter that has been enqueued for longest (FIFO), and enqueue all of its children. Record any complete words found along the way. 
...
Enqueue all child letters of the last letter of the input prefix. Then, until the queue is empty: pop the letter that was last enqueued (LIFO), and enqueue all of its children. Record any complete words found along the way. 
...
Here's the intuition for a stack-based DFS:
Enqueue all child letters of the last letter of the input prefix. Then, until the queue is empty: pop the letter that was last enqueued (LIFO), and enqueue all of its children. Record any complete words found along the way.

Here's the intuition for a recursive DFS:
Beginning with the last letter of the input prefix: explore *any* child node (including children of children) as soon as it is discovered. Repeat this exploration for all other child nodes, backtracking when necessary. Record any complete words found along the way.

I chose to use the stack-based DFS, because it's what we focused on during the lecture.
...
Add the last letter of the prefix to a priority queue. While the queue is not empty, pop the node with the smallest cost and add all of its children to the priority queue, with an associated path cost which is the cost of the parent node plus the inverse frequency of the number of words that include the child node. Record any complete words found along the way.
...
It was interesting to play around with the different text documents to see the different outputs. As expected, BFS always produced "nice" looking suggestion lists, as BFS inherently sorts by smallest to largest size. DFS would most often give unintuitive orderings of suggestions, such as having "through" being the first suggestion to the prefix "t" for the GenZ file, when personally I would expect shorter, more common words like "the" to be at the top in real autocomplete systems. Another good example of what I'm talking about is how DFS suggests "quinoa" as the top suggestion from the recipes file, which was never what I was trying to type. Unfortunately, because DFS will explore all the way down a single branch at a time, long, unsought-after words are often suggested much higher than they probably should be.

Although I liked the simplicity and predicability of BFS' suggestions, I was more impressed by UCS. Making suggestions that take into account letter frequency had the effect of more often than not placing the words I was trying to spell at the top of the list, in contrast to BFS, which feels orderly to an almost unnatural degree, and DFS, which "clunkily" throws long words at you which you often weren't trying to spell. Small things like listing "to" before "a" in the recpies document feels much more natural to me, and I'm impressed UCS manages to feel so natural.
