...
- If the current node doesn't have a corresponding child to the current character, create one.
- Then, iterate to the node corresponding to the current character.

- The last character of each word is iterated to outside of the inner loop so that It may be marked as the end of a word.

- the cost defined for UCS is then calculated using a recursive function after the tree is completed

## `BFS`

### Code analysis

- navigate the tree down to the last letter of the prefix

- add the node corresponding to the last letter of the prefix to the queue. each entry of the queue will include both a node and the letters leading to it in string form

- now iterate through the queue from the front (FIFO),  removing the node/word pairs as you go.

- if a node is the end of a word, add the word to the list of suggestions.

- add all of the children of the node to the end of the queue
...
- The intuition behind this code is the same as BFS, but the next selected node is from the end of the queue rather than the front. (LIFO)

### Your output

- through, thah, that, thou, though, thought, the, thee, their, there

### Recursive DFS vs Stack-based DFS
- Stack-based DFS was used here, mainly because the foundation was already placed by the BFS function I implemented before. It made implementing DFS as simple as changing the queue from FIFO to LIFO, essentially making it operate as a stack.
...
- add the node corresponding to the last letter of the prefix to the queue. each entry of the queue will include the total cost to reach it, a node, and the letters leading to it in string form

- begin iterating through the queue

- using heapq, pop the next lowest cost node from the queue.

- if a node is the end of a word, add the word to the list of suggestions.

- add all of the children of the node to queue, taking note of the total cost to reach them

### Your output

- the, thee, there, their, that, thag, thou, though, through, thought
...
BFS generated suggestions from shortest to longest, whereas the suggestions from DFS tended to be more random. Suggestions from UCS were clustered by how common the next letter was, for example if there were many suggestions where the next letter was an 'e' and very few where the next was an 'h', the 'e' suggestions would appear at the top of the list.
