...
- Put the intuition of your code here
  Splits document into words → ["there", "though", "that", "the", "their", "through", "thee", "thou", "thought", "thag"]
  Inserts words into a Trie → Each word is added character by character
Checks if character exists in node.children
...
It finds all words that start with the given prefix, returning shorter words first.
It ensures that words appear in increasing length order because BFS processes nodes in the order they are added to the queue.
### Your output
I used the test.txt
Type 'th' is the result : ['that', 'thag', 'the', 'thee', 'their', 'there', 'though', 'thought', 'thou', 'through']
Type 'the' is the result : ['the', 'thee', 'their', 'there']
Type 'tho' is the result : ['though', 'thought']
Type 'thou' is the result : ['thou']
Type 'tha' is the result : ['that', 'thag']
...
Depth-First Search (DFS) explores the Trie deeply before backtracking.
It finds all words that start with the given prefix, but unlike BFS, it does not guarantee shortest words first.

### Your output

Type'th' is the result : ['though', 'thought', 'thou', 'through', 'there', 'thee', 'their', 'that', 'thag', 'the']
Type 'the' is the result : ['there', 'thee', 'their', 'the']
Type 'tho'is the result : ['though', 'thought']
Type 'thou'is the result : ['thou']
Type 'tha'is the result : ['that', 'thag']
...
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
 Recursive DFS
Uses function calls to traverse the Trie.
Each recursive call dives deeper before returning.
Memory usage is higher (since recursion stores calls in the call stack).
Easier to write but can cause stack overflow for deep tries
...
- Put the intuition of your code here
Uniform Cost Search (UCS) prioritizes words based on their frequency in the dataset.
Instead of exploring level by level (BFS) or deeply (DFS), UCS prioritizes frequently occurring words first.
Uses a priority queue (heapq), where:
Words with a higher frequency have lower cost (processed first).
The most common words appear first in suggestions.

### Your output

- Put the output you got for the prefixes provided here
Type'th' is the result ['the', 'that', 'though', 'there', 'thought', 'thee', 'their', 'through', 'thou', 'thag']
Type 'the' is the result ['the', 'there', 'their', 'thee']
Type 'tho'is the result  ['though', 'thought']
Type 'thou'is the result ['thou']
Type 'tha'is the result ['that', 'thag']
## Experimental
...
Algorithm	         Search Strategy	     Suggestion Order	             Best Use Case
BFS	                 Level-by-Level	         Shortest words first	        Classic Autocomplete
DFS	                 Deep exploration	     Longer words first	            Spelling correction
UCS                  Frequency-based	     Most common words first	    Real-world autocomplete
