...
- ![IMG_4881](https://github.com/user-attachments/assets/c8fdfaca-2d52-4441-a070-56258028db75)


### Code analysis

- Put the intuition of your code here
Iterates through each word in the document, adding characters to a trie while tracking their frequencies at each node. If a character is not present, a new node is created. Marks the final node of each word as a word endpoint.  
...
Traverses the trie using BFS starting from the node corresponding to the given prefix, adding words to the suggestion list when a complete word is found. Explores all possible word extensions by enqueueing child nodes, ensuring words are suggested in increasing order of depth.
...
- Put the intuition of your code here
Performs a recursive DFS to traverse the trie from the given prefix, adding words to the suggestion list when a complete word is found. Explores all possible word extensions by recursively visiting child nodes, ensuring deeper words are found before siblings.
...
Recursive DFS uses the function call stack to traverse deeper into the trie, automatically handling backtracking when reaching the end of a branch. In contrast, stack-based DFS explicitly manages a stack to track nodes, mimicking recursion iteratively.

In this implementation, recursive DFS is used for simplicity and familiarity, leveraging function calls to explore child nodes. A stack-based approach could be used to avoid recursion depth limits in very deep trees.
...
- Put the intuition of your code here
This implementation of Uniform Cost Search (UCS) finds words that start with a given prefix by prioritizing character sequences with the lowest accumulated path cost. It uses a min-heap (priority queue) to explore nodes in order of their cost, where the cost of each character is the inverse of its frequency, ensuring more common sequences are explored first.
...
- for BFS i saw that the suggestions came up in order of length, prioritizing the smaller swords before the larger ones, which makes sense in terms of the algorithim as BFS is best for retrieving shorter words first but does not consider frequency.

- for DFS Suggests words based on a depth-first traversal, meaning it may prioritize longer words first if they are encountered before shorter ones in the trie structure. So in the search suggestions I saw the chain of words in the suggestions list that build off the chararcters of the prev word suggested, until all words in that "form" were searched, and then it would move to the next possible char in the tree that provides a new "set" of words based on the prefix.

-  for UCS I saw that typically shorter words were recommended first as typically the shorter the path to a word, the higher the chance that the traversal doesn't have to travel across a long costly path. However "though" came up before some shorter words due to the frequency in words ending in "ough" with the same prefix. UCS provides the most intuitive autocomplete suggestions since it considers both structure and frequency.
