...
- First, you check if any of the root node's children is the same character as the first letter of the word you are looking at. If it is, you look at that node and check if any of its children is the same as the second letter, if not then you create a new node and repeat this for every letter of each word and every word in the document.

## `BFS`

### Code analysis

- Finds the prefix in the tree node by node, then queues all of the nodes of the children of the last letter of the prefix. Goes level by level and if the node you are at is the last character of the word, that word then gets added to the suggestion list.
...
- Finds the prefix in the tree node by node, then stacks all of the nodes of the children of the last letter of the prefix. Taking from the stack and expanding it through its children, adding the words to the suggestions list when reached and repeating until all the nodes from the stack have been taken out and explored.
...
- I used Stack-based DFS by always taking the last node added from the stack through a for loop.

## `UCS`

### Code analysis

- Finds the prefix in the tree node by node, then queues all of the nodes of the children of the last letter of the prefix. I then choose to explore and expand the node in the queue with the highest frequency/shortest path cost, adding the word made to the suggestions list if the node is the last character of a word and repeating until the queue is empty.
...
- I saw that for BFS, the shortest words were suggested first, and the longest last. 
- for DFS, the suggestions were from right to left of the tree and if words were substrings of other words, the smallest words were suggested first
- for UCS, there were a lot of surprising things. If I put in just t as the prefix, I thought at first that tea would be the first suggestion as it is repeated 8 times. But to my surprise, the was first. I wondered if I did the algorithm wrong, but then I realized that the is correct since th has been repeated 10 times then the another 4 times. UCS doesn't also always have the shortest words first suggested in comparison to BFS
