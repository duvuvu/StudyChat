...
- Put the intuition of your code here
    - it iterates through each word in the text file and then through each character in the word. It begins at the root for each word and then for each char in each word it iterates through, checking if it exists in the current nodes children and if it doesnt it adds it, and then either way it goes to that node. At the end of each word it sets the final node to have an "is word" value of true
### Your output

- Put the output you got for the prefixes provided here
    - My build tree method includes no output code. I created a print tree diagram method but that is a separate thing. 
...
- Put the intuition of your code here
    - First it finds the final char of the prefix that is inputted into the gui in the tree and begins the search from there. Then it creates a queue, where for each node it traverses it puts all of the children of said node into the queue, then clearing the node it just searched from the queue. As it searches the tree it generates a string for each branch, generating words that all share the same prefixes first.
### Your output
...
- Put the intuition of your code here
    - Just like BFS, first it grabs the final letter of the prefix as a node in the tree. From there, it takes that node and the prefix and runs the dfs algorithm. In the dfs algorithm, it checks if the node completes a word, and if it does, adds the word to the suggestions lists, and hten continues on, running the dfs algorithm on any children that node may have. Bascially, it goes as far as it can down each branch of the tree, once it cant go further down, it goes back up a level to explore the other sub branches and once all the sub branches of a branch have been explored, it moves on to the next. Thats why it explored all those with prefix the first, then to, then tha, then thr
### Your output
...
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
    - I used recursive DFS, but in practice they work pretty much the same way. To understand the difference, lets say we have a tree containing "the, there, their, that". In stack based it would add t to the stack, pop it, adding h to the stack, pop it, adding e and a to the stack. It would then pop e, adding r and i to the stack so it is now i, r, a, pops i so stack is now r, r, a. Pops r whcih has no children so it is now r, a, pops r, becomes e, a pops e, now its just a. In recursive the only difference is our stack is made up of function calls rather than nodes, so in our example of stack based, you would just replace each letter in the stack with a function call to explore the children of each node reoresented by that letter. 
...
- Put the intuition of your code here
    - grabs a prefix just like the other two searches. Once its done that it creats a priority queue using heapq. When initialized, the pQueue has only the prefix node as an entry which contains the cost, node itself, and prefix. Then it loops through the pQueue, each time grabbing the lowest cost option, if that node is a word, it appends it to our suggestions, and then for each of its children it calculates its cost and then pushes it onto the pQueue
### Your output

- Put the output you got for the prefixes provided here
    - the, thee, thou, that, there, thag, though, thought, through, their
...
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 
    ...
    UCS: the, thou, thee, thag, that, though, their, there, thought, through

    BFS and DFS definitely suit their names. BFS is much more broad, going from short to long as it iterates through each level as compared to DFS which goes through each branch. UCS on the other hand is a totall different animal which does not have any natural ordering but instead a variable one.
