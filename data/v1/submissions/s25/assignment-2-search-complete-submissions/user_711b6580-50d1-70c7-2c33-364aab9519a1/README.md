...
That part should be pretty easy tho
...
My build_tree() function makes a tree by splitting up the words in the text file, and goes character by character to see if it has been as added as a child node of the current node. If not, it will add it as a node and go onto the next character in the word. If the node is a word, we change the is_word flag to True. 
...
We first traverse down the tree to find the last character of the prefix, Then we use a queue to start at the prefix node and process the nodes by level, which is the FIFO method. If the node is a word, it gets added to the suggestions list we initialized, and continues until we find all the words with this prefix. This ensures that the shortest words come first and the longest are last.

### Your output

- Put the output you got for the prefixes provided here

the thee thou that thag there their through thought through
...
We traverse the tree to find the last letter of the prefix, just as in BFS. We then use a recursive function to do the DFS from the prefix's node. It recursively visits each child node and adds characters to the current word. When it finished and has added the completed word to the suggestions list, it goes back to explore the other words from the prefix. If a node is a word, we add it to the suggestions list. Here, DFS explores a word's whole path before going back and trying a new word. 
...
I used recursive DFS which calls itself on each child node, remembering what node to go back to once it has found the end of a word node. It is more like the actual structure of the tree visualization. In stack-based DFS, it pushes nodes onto the stack manually. It is LIFO where you choose the last node and take it off to be explored before it goes back to the beginning. In recursive DFS, you expand recursively until you are finished. 
...
It starts the same as DFS and BFS by traversing down the tree to find the last letter of the prefix. Then it uses a priority queue to explore the path's that have the lowest costs, here that means with the fewest characters. If there are the same number of letters, it sorts the words alphabetically. Nodes with the lowest cost will be explored first and when a node is a word, it gets added to the suggestion list. 
...
I somewhat explained it earlier but in BFS, the shortest words are suggested first and the longest last. This is because it explores all words that are at the same depth level and then moves onto the next level. IN DFS, it explores a whole branch before exploring the others. Therefore, some shorter words may appear after longer ones. In UCS, it is similar to BFS where the shortest words are suggested first and the longest last, but it also maintained alphabetical order. I prefer UCS, personally. 
