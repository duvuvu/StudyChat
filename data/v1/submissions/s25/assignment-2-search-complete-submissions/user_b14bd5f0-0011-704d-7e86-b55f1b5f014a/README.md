...
#### -> I first call my inverse frequency calculator function that returns a python dictionary where the keys are characters and the values are # of time the character appears.
#### Next, I iterate over each word in the document and each charachter of each word, and traverse the tree following the charachter by either existing charachters in the tree or creating new ones.
...
#### ->First I traverse the tree to get to where our prefix ends. Next, I use a queue to traverse the rest of the tree and append each word to a suggestions list.
...
#### -> First I traverse the tree to get to where our prefix ends. Next, I use a stack to traverse the rest of the tree and append each word to a suggestions list.
...
### I used stack-based DFS because it was more intuitive to implement and I didn't see a reason why a recursive approach would be necessary.
...
#### First I traverse the tree to get to where our prefix ends. Next, I use a heap to traverse the rest of the tree and append each word to a suggestions list, which returns the words with the minimal costs first.
...
#### -> BFS seems to find the shortest words first, while DFS generally prefers the longest. UCS prefers words that are shorter but also use more common characters.
