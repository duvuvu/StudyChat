...
The build_tree function iterates through each word in the document, and for each character in the word, it checks if the character already has a corresponding child node. If not, a new node is created and the traversal continues to the next node. Once the last character of a word is reached, it's is_word = True.
...
We first initialize a recommendation array and traverse the characters in the prefix to verify they exist in the tree. Next, we initialize a queue and append the prefix node. We then pop the prefix, add its child nodes, and if a valid word is found, it is added to the recommendation array, repeating this process by popping the first character and appending its neighbors.
...
We first initialize a recommendation array and traverse the characters in the prefix to ensure they are in the tree. Next, we initialize a stack and append the prefix node. We then pop the prefix, add its child nodes, and if a valid word is found, it is added to the recommendation array, repeating this process by popping the last appended character and exploring its neighbors.
...
In my opinion, I feel the recursive DFS is more intuitive than the stack based DFS. However, I already made Queue-based BFS so creating stack-based DFS was much more simpler and straight foward as I can reuse the code. Recursive DFS just takes each part of which character to pick as a subtree of the initial problem while a stack-based implementation follows the definition of DFS where all the children are explored first. 
...
We first initialize a recommendation array and traverse the characters in the prefix to ensure they are in the tree. Next, we initialize a heap queue and append the prefix with a cost of 0. We then pop the prefix, add its child nodes with calculated costs, and if a valid word is found, it is added to the recommendation array, repeating this process by popping the character with the least cost and exploring its neighbors.
...
BFS generates suggestions starting with the shortest words and progressively moves to longer ones, while DFS produces a mix of both short and long words since it explores deeper into the tree. UCS, on the other hand, groups words that are more similar to each other, prioritizing suggestions based on their frequency and cost.
