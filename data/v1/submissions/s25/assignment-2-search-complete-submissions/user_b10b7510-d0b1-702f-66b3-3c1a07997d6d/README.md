...
This is my hand-drawn image of the tree:

![Tree Diagram](images/tree_diagram.JPG)

### Code analysis

- Put the intuition of your code here
To check if the character is in the children of node, I follow the CEEC algorithm. It will iterate through each character in the word and if the character is not in the children of the node, it will create a new node and then move on to the next node, creating a path between the node. Once the end of the word is reached, the star will be added to indicate the end of the word 
...
The function will first take the prefix, and locate every word in the tree that begin with the prefix. From there, it use the queue to investigate to explore the remaining letters of the word. Valid words indicated by "*" are gathered and return as a list. 
...
- Put the intuition of your code here
Starts from the node corresponding to the last character of the prefix and explore the tree by using stack and the return the words. 
...
Output: the there their thee thou though thought that thag through

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.

Recursive DFS focus on exploring the nodes recursively until it hits the leftmost left first, whereas stack-based DFS visit the last node pushed first, traversing the rightmost leaf first. I decided to use stack so the words can be visited by sequence and for better performance and to avoid stack overflow.
...
The function have priority queue to exploring the least-cost node first, expands it by iterate over the child nodes and then push the child into the heap with updated cost and word (if the low cost path is found)
...
Output: the thag that thee thou their there though thought through
...
DFS: Returns the word in depth-first manner (LIFO) (Left-Right)

UCS: gives the most common word first depending on the frequency or cost (use pritiory queue)


BFS is best for autocomplete, DFS is good for full-word searches, and UCS can show real-world typing behavior or common user patterns by ranking frequent words first.
