...
- ind the next letter to autofill based on alphabet. it is ordered by ascending order.
...
Th --> thag that the their there though thought through

## `DFS`

### Code analysis

- Gives suggestions based on word length. longer words seems to appear before others order seems to be based on longer suggestions first.

### Your output

- Put the output you got for the prefixes provided here

Th --> thag that the thee their there thou though thought through

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.

recursive DFS uses a depth first order while stack based works iteratively working through a stack.

## `UCS`

### Code analysis

- It suggests words that are more likely to appear based on the frequency of letters. Relevant completions appear first in the autocomplete results.
...
DFS searches for answers by exploring deeply into one branch first, which may result in recommendations that are less valid.  BFS examines every option in detail and makes recommendations that are more in line with your requirements. UCS goes one step further by offering the best recommendations, frequently based on what makes the most sense given the circumstances.
