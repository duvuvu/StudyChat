...
![Tree Diagram](TestTree.png)

### Code analysis

- `build_tree` splits the document into words, and then constructs a graph of character nodes for each word. It also keeps track of character frequencies and path costs in separate dicts. The last character of each word is marked as such to denote the leaves.
...
- I used a stack to implement DFS because I was already familiar with that way and didn't care to think about how to do it recursively. Recursion sounds like it would be more intuitive given that recursive function calls would just be implicitly managed by the call stack, but perhaps the overhead of the call stack would make it less memory efficient?
...
## Experimental
- BFS seemed to show shortest to longest words, with those of the same length being alphabetically sorted. DFS seemed to show alphabetically sorted words, with those of the same alphabetization (like the and thee) showing the shortest first. UCS seemed to show shorter words first, though it wasn't strictly ordered by length. Perhaps this is an instance of Central Limit THeorem, whereby it follows a normal distribution that plots the mean commonality of each word's characters in the genZ list. This would explain why shorter words would be more likely to show first.