...
-![image](https://github.com/user-attachments/assets/402ed77f-3810-4b48-85b7-9bdcdd21acb6)


### Code analysis


This function takes a text document, splits its content into a list of words, and iterates over each word and each letter to create and add child nodes to the tree if they do not already exist. The traversal begins at the root node, and it moves down the tree by setting each child node as the current node. This process continues until all words have been inserted into the tree, at which point the loop concludes.
### Your output
...
The code begins traversing from the root node by iterating through the given prefix. If the prefix does not exist, it returns an empty list. It then initializes a queue of tuples containing the node and the prefix, using deque for efficient processing. While the queue is not empty, it pops the first element and checks whether the letters formed so far represent a complete word. If they do, it adds that word to the list. If not, it expands the node and continues to traverse down the tree by making the child node to be the current node until it forms a complete word.
### Your output


for a prefix of S the output was sus, sis, simp, slay, salty so on
...
The code begins traversing from the root node by iterating through the given prefix. If the prefix does not exist, it returns an empty list. It then initializes a list as a stack of tuples containing the node and the prefix. While the stack is not empty it pops the last element and checks whether the letters formed so far represent a complete word. If they do, it adds that word to the list. If not, it expands the node and continues to traverse down the tree by making the child node to be the current node until it forms a complete word.
### Your output


for S slay, snatched, sis, simp, simping so on

### Recursive DFS vs Stack-based DFS

For the recursive approach we will make a function def dfs(currentnode,currentword) which will check if the current word is a word and if it is add it to the list but ifm not it will recursively call the function in the same for loop as the one i wrote in my code by initialising currentnode and currentword to be the items from list node.children.items(). while in the stack based approach we make a stack and then run a while loop to traverse through the tree, until the stack is empty.
I used the Stack-based approach in my code.
...
The code begins traversing from the root node by iterating through the given prefix. If the prefix does not exist, it returns an empty list. It then initializes a cost variable as 0 and a list named priority_q of tuples containing the cost,node and the prefix. While the priority_q is not empty it pops the least cost element using heapq methods and checks whether the letters formed so far represent a complete word. If they do, it adds that word to the list. If not, it expands the node adds 1 to the cost and continues to traverse down the tree by making the child node to be the current node until it forms a complete word.

### Your output


for S - sis, sus,simp, slay, salty so on 


## Experimental
 

for UCS the words that have less number of letters are generated first.
for DFS longer words were seen first.
for BFS shorter words seen first.
