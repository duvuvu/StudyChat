...
## 383GPT
Did you use 383GPT at all for this assignment?  
**Yes.**
...
### Tree Diagram
The document `test.txt` contains the words: `there though that the their through thee thou thought thag`. Below is the text-based tree diagram:
...
### Code Analysis
The `build_tree` method processes each word from the document, inserting each character into a trie structure. For each character in a word, nodes are created if not present, and frequency counts are updated. The end of each word is marked with `is_word = True`.

### Your Output
For `test.txt` with prefix `"th"`, the output includes all words starting with `"th"` in insertion order due to BFS traversal.
...
### Code Analysis
BFS uses a queue to explore each level of the trie from the prefix's end node. Words are collected in order of increasing length, ensuring shorter words appear first.

### Your Output
With `test.txt` and prefix `"th"`, BFS suggestions are ordered by length:
the
their
there
though
...
### Code Analysis
DFS uses a stack to explore depth-first, processing children in reverse order to maintain insertion order. This results in longer words appearing earlier if deeper paths are taken first.

### Your Output
For prefix `"th"` in `test.txt`, DFS may suggest longer words first:
thought
thag
thou
thee
...
- **Recursive DFS**: Uses the call stack to traverse the tree. It is simpler to implement but may hit recursion limits for large trees.
- **Stack-based DFS**: Uses an explicit stack to manage traversal. It avoids recursion limits and allows more control over traversal order.  
**Choice**: Stack-based DFS was used here for better control and to avoid recursion limits.


## `UCS`

### Code Analysis
UCS prioritizes paths with higher transition frequencies using a priority queue. Costs are cumulative inverses of frequencies, promoting common paths first.

### Your Output
For `"th"` in `test.txt`, UCS suggests high-frequency words:
the
their
there
though
...
- **BFS**: Yields shortest words first, ideal for quick suggestions.  
  Example: `"the"`, `"their"`, `"there"`, `"though"`, `"through"`, `"thee"`, `"thou"`, `"thought"`, `"thag"`.
- **DFS**: Provides deeper, possibly less common words, useful for exploring variants.  
  Example: `"thought"`, `"thag"`, `"thou"`, `"thee"`, `"through"`, `"though"`, `"there"`, `"their"`, `"the"`.
- **UCS**: Optimizes for frequency, enhancing relevance by prioritizing common terms.  
  Example: `"the"`, `"their"`, `"there"`, `"though"`, `"through"`, `"thee"`, `"thou"`, `"thought"`, `"thag"`.
