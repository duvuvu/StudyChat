...
## Late Days
I am not using late days for this assignment.

## 383GPT
I did not use 383GPT for this assignment.
...
- For each word in the input, start at the root node and iterate through each letter, adding it as a child of the current node if it doesn't already exist, then moving the current node to the child corresponding to that letter and updating its frequency. When the word is complete, add an end node (unless one already exists).
...
- Iterates through the letters of the prefix and moves down the tree to find the node that corresponds to the last character of the prefix. Does BFS starting from this node until its queue is empty to explore this sub-tree, adding all word-ending nodes to a list. Iterates through this list to construct each word, working backwards from the word-ending node to the root.
...
- Iterates through the letters of the prefix and moves down the tree to find the node that corresponds to the last character of the prefix. Does stack-based DFS starting from this node until its stack is empty to explore this sub-tree, adding all word-ending nodes to a list. Iterates through this list to construct each word, working backwards from the word-ending node to the root.
...
-  In a recursive DFS implementation, you choose the next node by recursively running the algorithm on the children of the current node, which essentially uses the call stack as the stack for the nodes. Since it runs this on the first child first, it will go from "left" to "right". In an iterative stack-based implementation, you define your own stack, and you choose the next node by first adding the children of the current node to the stack, then you pop the most recently added to be your next node. This ends up picking the last child first, so it will go from "right" to "left". I chose the stack-based implementation since I had already written the algorithm for BFS, and changing that to stack-based DFS only requires changing the queue to a stack.
...
- Iterates through the letters of the prefix and moves down the tree to find the node that corresponds to the last character of the prefix. Does UCS using a priority queue (heap) starting from this node until its queue is empty to explore this sub-tree, adding all word-ending nodes to a list. Iterates through this list to construct each word, working backwards from the word-ending node to the root.
...
## Experimental
- BFS simply sorts words from shortest to longest. DFS suggests in a more random manner when looking at length, but it groups words by common prefixes. For example, when typing "th", it groups together words that start with "tha", words that start with "tho", and words that start with "the". If a word is a prefix of another word, the prefix word will come first. UCS differs from both of these. It tends to prioritize shorter words over longer ones, but this is not a strict rule like in BFS. Instead, it will also prioritize words that share a common prefix with other words and words that appear in the sample text many times. Notably, "though" is higher than "there" and "their", and is not grouped with the other "tho-" words. This is likely because both "thou" and "though" are common prefixes, but "though" is a longer word, so these factors balance out and it gets placed near the middle of the list when searching "th". Another notable example is that when searching with no prefix, "snatched" is near short words like "flex" and "lit", likely because it appears in the sample text more times than other words - 9 times as opposed to "lit"'s 3 and "flex"'s 4.
