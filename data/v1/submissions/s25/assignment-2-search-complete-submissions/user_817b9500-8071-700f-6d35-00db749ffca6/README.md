...
- The implemented code maintains a Tree structure and calculates character frequencies to optimize autocomplete suggestions, using depth-first and breadth-first traversal methods. The primary control structure involves iteratively constructing the Tree via nested loops for each character and recursively or iteratively exploring nodes to retrieve suggestions.
...
- Put the intuition of your code here
- The BFS method utilizes a queue to explore all nodes level-wise starting from the node corresponding to the last character of the input prefix, efficiently collecting words as it traverses through child nodes. Each node's suggestions are built by concatenating characters while ensuring that word completions are detected through the `is_word` flag.
### Your output

![Screenshot 2025-02-21 204735](https://github.com/user-attachments/assets/f46c00a5-7106-4951-81a6-3708b36400d8)
...
- Put the intuition of your code here
- The DFS method employs a stack to recursively or iteratively explore each path from the prefix node, building word suggestions by concatenating characters until reaching a leaf node marked with `is_word`. This allows the method to effectively gather all potential words that can be formed from a given prefix in a depth-first manner.
### Your output

![Screenshot 2025-02-21 204817](https://github.com/user-attachments/assets/7b1bcb93-b6d4-4c9c-8f68-abe27108addf)


### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
- Recursive DFS uses the call stack to navigate through nodes, simplifying implementation at the cost of potential stack overflow with deep trees. It naturally handles the backtracking process but is limited by the recursion depth in many programming environments.
Recursive DFS uses the call stack to navigate through nodes, simplifying implementation at the cost of potential stack overflow with deep trees. It naturally handles the backtracking process but is limited by the recursion depth in many programming environments.
In this implementation, I have used a stack-based DFS approach. This decision ensures robust handling of deep structures without risking stack overflow, allowing for efficient and clear node exploration when retrieving autocomplete suggestions based on user input.
...
- Put the intuition of your code here
- The `suggest_ucs` method implements the Uniform Cost Search algorithm to efficiently retrieve autocomplete suggestions based on character frequencies, prioritizing paths through more frequently occurring characters. It uses a priority queue to explore nodes in the Tree, ensuring that the most relevant completions are returned based on the accumulated cost, which reflects the inverse frequency of character usage. This design enables dynamic and efficient suggestion generation tailored to user input.
### Your output

![Screenshot 2025-02-21 220218](https://github.com/user-attachments/assets/be4e3f95-3a12-4b6f-aa9e-1631a333f714)

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS.
- BFS returns the shortest word completions first, ensuring broad coverage but not considering word frequency or relevance. DFS explores words deeply in a single branch before backtracking, usually prioritizing longer words. UCS prioritizes suggestions based on cost, making it more user-friendly by returning the most relevant words first.
