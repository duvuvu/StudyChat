...
![Tree Diagram](https://github.com/<redacted>/assignment-2-search-complete-<redacted>/blob/main/IMG_5083.jpg?raw=true)
### Code analysis

- Put the intuition of your code here
- Splits the document into words, then goes through each word. If the character already exists in the tree, it moves to that node and continues from there, otherwise it creates a new node. Once all the chars for the word are inserted, it markes the node as the end of the word. 
...
- Put the intuition of your code here
- Traverses the tree level by level, starts from the node corresponding the the last character in the prefix. Explores all the possibilities by pushing the children nodes into the queue and and suggesting a word when it reaches the marked end of word. 
...
- Put the intuition of your code here
- Uses the dfs function to recursively traverse the character tree. Adds a word to suggestions when it reaches the marked end of word, backtracks to the last function call and continues down the next unexplored branch. 
...
- With recursive DFS, the function is called recursively to explore each branch until induvidually to the end, when end is reached, it backtracks by returning to the function call. With BFS we manage the stack ourselves, the children of each node is pushed on for later exploring. I implemented it recursively. 
...
- Put the intuition of your code here
- uses a priority queue, nodes are prioritized based on the cost calculated from by the frequency of their characters. more frequent characters have a lower cost, which means words composed of more frequent characters are explored earlier and suggested before others. 
...
- BFS explores layer by layer in the tree, so the shorter ones are explored first and the longest last. DFS explores one branch as deeply as possible before going back, so based on the structure of the tree, if it explores the "the" branch first, it finishes with all words starting with "the" before going to the next. UCS takes into account the frequency and prioritized based on the cost of the characters although the longer words are explored last, this is becuase the cost is likely to accumulate to a higher number. 
