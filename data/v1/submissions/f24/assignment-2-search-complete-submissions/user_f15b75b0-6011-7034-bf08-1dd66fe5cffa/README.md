...
![build_tree diagram](build_tree_diagram.png)

### Code analysis

- Put the intuition of your code here

# The `build_tree` method processes the input document by splitting it into words and then inserting each word into a prefix tree (Trie). Each character of a word becomes a node in the tree, and the end of a word is marked by a special child node (`'!'`), which signifies that a complete word ends at that node. 
...
# In the `suggest_bfs` function, I implemented a breadth-first search (BFS) strategy on a prefix tree (Trie) to find and return all words that begin with a given prefix. First, I navigate through the Trie to locate the node corresponding to the last character of the prefix. If the prefix is found, I enqueue this node along with the prefix and iteratively explore all possible subsequent characters using a queue.

### Your output

- Put the output you got for the prefixes provided here
![build_tree diagram](suggest_bfs_suggestions.png)
...
- Put the intuition of your code here
# The `suggest_dfs` method performs an iterative depth-first search (DFS) to find all words in the Trie that start with a given prefix. It first traverses to the node corresponding to the last character of the prefix and then uses a stack to explore all possible word paths from that point. As it explores, the method adds complete words (marked by `'!'`) to the suggestions list, ensuring all relevant words are found by following a depth-first traversal.

### Your output

- Put the output you got for the prefixes provided here
![build_tree diagram](suggest_dfs_suggestions.png)


### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
# In recursive DFS, the system’s call stack manages the depth-first traversal automatically, making the implementation more concise but potentially prone to stack overflow for deep trees. Stack-based DFS, on the other hand, uses an explicit stack data structure to manage the traversal manually, giving more control over memory usage and avoiding recursion limits. In this implementation, I used stack-based DFS because it handles deeper trees more safely and efficiently, allowing me to explore all word paths without risking stack overflow.
...
- Put the intuition of your code here
# The `suggest_ucs` method implements Uniform Cost Search (UCS) to find all words in the Trie that start with a given prefix, prioritizing more frequent character paths. It first navigates to the node corresponding to the last character of the prefix, then uses a priority queue (min-heap) to explore nodes with the lowest cumulative cost, where the cost is the inverse of the character's frequency. As the search progresses, complete words are added to the suggestions list, ensuring that more frequent word completions are returned first.

### Your output

- Put the output you got for the prefixes provided here
![build_tree diagram](suggest_ucs_suggestions.png)


## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 
# I think that from the results generated by using BFS, DFS, and UCS, a few noticeable differences emerged in the ordering of the word suggestions. **BFS (Breadth-First Search)** suggested shorter words first because it explores all nodes at the current level before moving deeper. As a result, words like "the", "thee", "thou", and "that" appeared before longer words like "though", "through", and "thought". On the other hand, **DFS (Depth-First Search)** explored deeper branches of the Trie first, suggesting longer words such as "through", "though", and "thought" earlier, while shorter words like "the", "thee", and "thou" appeared later in the list. **UCS (Uniform Cost Search)** prioritized paths with lower cumulative costs, favoring more frequent character sequences. This caused UCS to suggest words like "the", "thou", "thee", and "that" early on, similar to BFS, but with an added preference for frequent words based on character frequency. Overall, while BFS focused on shorter words, DFS prioritized longer words, and UCS balanced frequency and length in its suggestions.
