...
Iterates through all the words in the document, and for each character in the word, if it is not already in the current node's children, it creates a new node and updates the frequency of the node. Once all the characters in the word have been processed, it marks the end of the word.
...
Traverses down the tree data structure to locate the node indicating the end of the prefix. Uses a queue to explore all the child nodes by adding each child to the queue along with the word formed. Continues the process until there are no more child nodes, adding the complete words to an array that it returns after the search ends. 
...
Traverses down the tree data structure to locate the node indicating the end of the prefix. Uses a recursive DFS algorithm to recursively explore each child node, appending the child's character to the current word, and adding each complete word to an array that it returns after the search completes.
...
In general, recursive DFS is easier to implement because it automatically handles the call stack, whereas in stack-based DFS, we need to explicitly handle it manually. However, recursive DFS can often lead to stack overflows in deep graphs whereas stack-based DFS offers more control over the stack. I used the recursive DFS algorithm here because it simplifies the code a lot and explores the tree in a more straightforward manner. Also, since we are using a tree, recursive DFS is more intuitive since it implicitly handles the exploration and backtracking without any explicit loop or stack intializations. 
...
Traverses down the tree data structure to locate the node indicating the end of the prefix. Uses a minimum priority queue and a cost that is the inverse frequency of the characters, popping the node with the lowest cost and adds complete words to the suggestions array that it returns once the UCS algorithm completes.
...
BFS generates the suggestions sorted by lengths in ascending order because it traverses through the tree by level and therefore returns the simpler, shorter words first. 

DFS generated the suggestions from left to right of the prefix tree, returning words from one branch before moving on to the next. However, because of this nature of search, it may suggest potentially less common wotrds first.

The suggestions generated by UCS is more "random" at first look because it is based on the frequency of the character combinations, prioritizing more relevant words first. Therefore, the results are more optimal using this search algorithm. 