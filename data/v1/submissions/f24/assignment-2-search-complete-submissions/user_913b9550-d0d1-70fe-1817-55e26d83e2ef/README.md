...
![tree_diagram_hw2](https://github.com/user-attachments/assets/bdde79a3-6e80-4aae-9028-7bcd5abb67a2)

### Code analysis

- Put the intuition of your code here

The code will iterate through all words. For each letter in a word, if it is not already in the tree (in that location in relation to the root), it will add it to the tree with count 1, otherwise, it will increase the "count" of that word by one (used for UCS). If a word is completed, it will add a child to the end letter node of that word with key "word" with the value of that word.

### Your output

- Put the output you got for the prefixes provided here

N/A this is not a search algorithm so will not output.
...
The code will first traverse the tree through the prefix, so that the current "root" node is the end of the prefix string's path in the tree. Then, it will put that node in a queue. It will then repeatedly remove the first item of the queue, then add all children from that item into the end of the queue, each time checking if the current node ends a word. If the current node ends a word, it adds it to the suggestions, and continues iterating through the queue. Since we always put all children from the same "layer" all in a row, and use FIFO, we will end up with a BFS-ordered queue.
...
The code will first traverse the tree through the prefix, so that the current "root" node is the end of the prefix string's path in the tree. It will then recursively visit all children of those nodes, checking if they end correct words and adding them to the suggestions along the way.

### Your output

- Put the output you got for the prefixes provided here

there, the, their, thee, though, thought, thou, that, thag, through

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.

I utilized recursive DFS since that is the implementation I am most comfortable implementing.
...
I will add a part to the tree-building code creating a counter that is stored alongside the nodes with how many times that letter is visited.
The code will first traverse the tree through the prefix, so that the current "root" node is the end of the prefix string's path in the tree.
It will then add the child of that node's (1/freq, node) pair to a heap-queue, that is sorted based on 1 / the frequency of the letters. (had to implement a comparison function for Node, but I just left it as whatever one is first is "less than", so that heapq would work correctly.)
It will traverse in this order until all character nodes are visited, adding words to the suggestion in that traversal order as the complete words are found.

### Your output

- Put the output you got for the prefixes provided here

the, thou, though, thag, thee, thought, there, their, that, through
...
BFS tended to be in length order, with shorter answers first. DFS tended to be in semi - alphabetical order, with answers grouped  by shared sub-prefixes (additional prefixes after the original prefix), though not in direct alphabetical, as th-a came after th-o. UCS originally seemed fairly random, with the different letters somewhat split out. However, that actually makes some sense, since most full words in the list are not sub-words, and thus will end up being added to the queue with frequency 1 (value 1/1), and thus will always be visited last. All words that are also sub-words (the, thou, though) were added first, then since the rest of the sub-words all weren't full words, they were traversed in the correct order but weren't added to the suggestions. After all the sub-words were traversed, then all of the remaining full words with frequency 1 were added in whatever order they were visited in (or maybe reverse order, it doesn't really matter).
