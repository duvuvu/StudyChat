...
### Tree diagram
![Tree Diagram](./tree_diagram.jpg)

### Code analysis

- Inserts words from a document into a tree where each Node represents a character of the given word. After the tree is created a breadth-first traversal is carried out to compute path costs for each Node (path cost = inverse Node visit frequency on words insertion, or 0 if leaf node).
...
- First finds the first node of the subtree representing what comes after the given prefix. Then, a queue is used to carry out a breadth-first search of the subtree; when a leaf node is found, the word built up to that point is added to a list which is ultimately returned.
...
- First finds the first node of the subtree representing what comes after the given prefix. Then, a recursive helper function is called to carry out a depth-first search of the subtree; when a leaf node is found, the word built up to that point is added to a list which is ultimately returned.
...
- The difference between recursive and stack-based DFS effectively comes down to the order in which we explore the tree. In other words, with recursive DFS, the left-most child will be explored first, whereas with stack-based DFS, the right-most child will be explored first.

- I initially implemented the stack-based version of DFS for this assignment due to its similarity in code structure to BFS and what I perceived as better suggestions for `genZ.txt`. However, I decided to refactor it into a recursive version because I ultimately figured since BFS explores from left-to-right it would perhaps be more consistent to use the version of DFS that also explores from left-to-right. Depending on the implementation of word documents, however, it *would* matter which version of DFS is used if suggestion quality is desirable (i.e. recursive DFS would likely be better if word documents follow the format `thou though thought`).
...
- First finds the first node of the subtree representing what comes after the given prefix. Then, a priority-queue is used to carry out a uniform cost search of the subtree; when a leaf node is found, the word built up to that point is added to a list which is ultimately returned.
...
## Experimental
- UCS gives the most relevant suggestions (to the input document) because its method of searching uses a kind of heuristic. If a word document contains more than one occurrence of the word (or words that are nested in other words like `thou` and  `though`), the path cost for each character will decrease (and thus an increased likelihood of that word/path being found/travelled first). BFS finds shortest words first (if words are the same length, then it finds words in insertion order). DFS finds words in the tree insertion order (or reversed insertion order, in the case of stack-based DFS).
