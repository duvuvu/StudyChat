...
## `Idea`
    The code aims to provide a Trie-based autocomplete system with different suggestion algorithms (random, BFS, DFS, UCS). Each method offers a unique strategy for returning word completions:

    - BFS for shortest first.
    - DFS for deeper explorations.
    - UCS for frequency-based optimization.

## `Code Structure for the 'autocomplete.py'`

## Node Class: 
    Represents each character in the Trie. It contains:
    "children": A dictionary mapping characters to their respective child nodes.
    "end_of_word": Boolean indicating if the node represents the end of a word.
    "frequency": Tracks how often this character is part of a word.
    "path_costs": Stores the cost of reaching each child node, calculated based on frequency.

## For the autocomplete class:
    build_tree(document): Constructs the Trie from a document by inserting each word character by character. Additionally, it calculates path costs for use in the UCS algorithm, based on node frequencies.
    suggest_random(prefix): Generates random suffixes to complete a given prefix.
    suggest_bfs(prefix): Uses Breadth-First Search to return word suggestions by exploring all possible continuations of the prefix, level by level.
    suggest_dfs(prefix): Uses Depth-First Search to find and suggest completions of the prefix by exploring each possible path as deeply as possible.
    suggest_ucs(prefix): Implements Uniform Cost Search to prioritize and suggest the most frequent (cheapest) word completions based on precomputed path costs.

## `build_tree`

### Tree diagram
- Put the tree diagram for `test.txt` here
![Tree Diagram from test.txt](http://url/to/img.png)
The diagram starts from a ROOT node that branches out into three main paths: A, B, and C.
In the first branch, A, it further connects to I, which leads to R.
In the second branch, B connects to A, which then leads to L. From L, the path continues to another L node.
The third branch, C, connects to A, which splits into two different paths. One path leads to T, while the other goes to R. From R, there are multiple branches: one leading to D, another to P (with a subpath from P to E and then to T), and a third to R (which continues to Y). Additionally, the A in this branch also connects to P, which leads to E.

### Code analysis
- Put the intuition of your code here
Goal: This function constructs the Trie (prefix tree) from the input document. Each word is inserted into the tree, with each character becoming a node. If a node for a character does not exist, it is created.
Frequency Tracking: Each node also stores a frequency count, incremented each time the corresponding character is encountered during word insertion.
Path Costs: After building the Trie, path costs are calculated to assist the Uniform Cost Search (UCS). These costs are determined by dividing the total frequency of all child nodes by the frequency of each individual child. This ensures less frequent nodes (which are harder to reach) have higher costs.

## `BFS`

### Code analysis
- Put the intuition of your code here
Goal: This method uses Breadth-First Search to suggest completions of a prefix.
Intuition: BFS explores the Trie level by level. Starting from the node corresponding to the prefix, it visits all of its children, adding them to a queue. It collects words in the order they are found, prioritizing those closer to the root (thus shorter words are discovered first).

### Your output
- Put the output you got for the prefixes provided here
BFS Suggestions:  ['the', 'thee', 'thou', 'that', 'thag', 'there', 'their', 'though', 'thought', 'through']

## `DFS`

### Code analysis
- Put the intuition of your code here
Goal: This method uses Depth-First Search (DFS) to suggest completions of a prefix.
Intuition: DFS explores as deep as possible along each branch before backtracking. Starting at the node corresponding to the prefix, it recursively explores each child, appending valid words when the end of a word is encountered.

### Your output
- Put the output you got for the prefixes provided here
DFS Suggestions:  ['through', 'thag', 'that', 'thou', 'though', 'thought', 'the', 'thee', 'their', 'there']

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
In recursive DFS, the function calls itself to explore the Trie. The recursive call stack handles the traversal implicitly. Whereas, in stack-based DFS instead of using recursion, an explicit stack (usually a list) is used to store the nodes yet to be visited. Nodes are manually pushed to and popped from the stack during traversal.

In this code, stack-based DFS is used (in suggest_dfs()). I manually manage a stack (stack = [(node, prefix)]) to keep track of nodes and their corresponding prefixes, pushing and popping nodes from the stack as we explore.

I used stack-based DFS because of the below reasons:
Control: Using an explicit stack offers more control over traversal and avoids Python’s recursion depth limit.
Scalability: It is safer for large Tries with deep paths, as the function won’t hit recursion limits.

## `UCS`

### Code analysis
- Put the intuition of your code here
Goal: This method uses Uniform Cost Search to suggest prefix completions, prioritizing paths with lower costs.
Intuition: UCS is similar to Dijkstra’s algorithm, where paths with the lowest cumulative cost are expanded first. The priority queue (heap) is used to track the nodes by their total path costs. Nodes with lower cost are explored first, ensuring that more likely or “cheaper” word completions are found before others.

### Your output
- Put the output you got for the prefixes provided here
UCS Suggestions:  ['the', 'thou', 'thee', 'though', 'there', 'their', 'that', 'thag', 'thought', 'through']

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 
When testing suggestions for different algorithms with the prefixes "ca," "ab," "li," and "th," distinct patterns emerged. For the prefix "ca," BFS suggested ['cap', 'cancelled'], favoring shorter words like "cap" before longer ones like "cancelled" as it explores the Trie level by level. In contrast, DFS returned ['cancelled', 'cap'], diving deeper into the Trie and suggesting the longer word first. UCS, which prioritizes word frequency or cost, also produced ['cap', 'cancelled'], favoring "cap" due to its higher frequency.
For the prefix "ab," all three algorithms—BFS, DFS, and UCS—returned no suggestions, indicating that no words with this prefix existed in the dataset. When testing with the prefix "li," all algorithms—BFS, DFS, and UCS—suggested ['lit', 'liter'] in the same order, likely because both "lit" and "liter" were close in depth and frequency within the Trie.
For the prefix "th," BFS generated ['the', 'thee', 'thou', 'that', 'thag', 'there', 'their', 'though', 'thought', 'through'], prioritizing shorter words first. DFS, however, suggested ['through', 'thag', 'that', 'thou', 'though', 'thought', 'the', 'thee', 'their', 'there'], exploring deeper words like "through" before shorter ones. UCS, which considers word frequency, returned ['the', 'thou', 'thee', 'though', 'there', 'their', 'that', 'thag', 'thought', 'through'], prioritizing more common words like "the" and "thou" over longer or less frequent options. 