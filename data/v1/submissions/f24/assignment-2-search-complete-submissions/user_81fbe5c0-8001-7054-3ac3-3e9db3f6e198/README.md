...
![My image](./IMG_0942.jpg)

### Code analysis

- Put the intuition of your code here
* Changed the for loop to take in 2 variables and enumerated the word so I get get an index and a character. I then check if the current char is currently in the childrens dictionary of the current node. If it isn't I create a new node for it. After that, I check if i's length is less than the word's length - 1. This is to make sure I don't go out of bounds, I then increment the value associated with the character in the dictionary. I then go to its child's node in the tree and after all of it is done, I set the last node's property of being endOfWord to be true.
### Your output
...
- Put the intuition of your code here
* First iterated to the last character in prefix and passed it into a queue tuple to store it. Then I used a while loop to iterate thru the tree. I then assigned curNode and curPrefix to the result after I called .popLeft() since BFS follows a FIFO algorithm. Then I check the curNode to see if it has the endOfWord marker on it marked as true or not, if it's true then I append the curPrefix to my return array. If not, then I append to the queue the next node as well as the updated prefix.

### Your output

- Put the output you got for the prefixes provided here
* BFS
    For prefix "li": ['lit', 'liter']
     - For prefix "l": ['lit', 'liter', 'lowkey']
...
- Put the intuition of your code here
* Kind of the same thing as for BFS but we use a stack instead of a queue. First iterated to the last character in prefix and passed it into a stack tuple to store it. Then I used a while loop to iterate thru the tree. I then assigned curNode and curPrefix to the result after I called .pop() since DFS follows a LIFO algorithm. Then I check the curNode to see if it has the endOfWord marker on it marked as true or not, if it's true then I append the curPrefix to my return array. If not, then I append to the stack the next node as well as the updated prefix.
...
DFS
    For prefix "li": ['lit', 'liter']
     - For prefix "l": ['lowkey, 'lit', 'liter']


### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
* Honestly, I used stack-based because when I used 383gpt to look up pseudocode for it that was what I was given. I probably would've used stack based either way though, my recursion skills are kinda rusty and I'm not too confident in them either. As for which one to use over the other - I'm not too sure which one would be better than the other. 
...
- Put the intuition of your code here
* Same as BFS/DFS where I had to first go to the last character's node in the tree. I then pushed it into the heap along with the cost it took to get there and the prefix. I then extracted the cost, curNode, and curPrefix from the heap after popping it. Then, I checked to see if the curNode was the last character or not. If it was then I appended it to the return array. If not, I pushed the child onto the heap along with its cost and new prefix.
...
* The ordering was different which reflects how different each searching algorithm traverses the tree.
