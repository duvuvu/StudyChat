...
![test txt ](https://github.com/user-attachments/assets/9aeadb18-7647-4313-9201-45316f8d7594)


### Code analysis

- It iterates over each word, and then for each word it adds the character into the prefix tree. It creates a new node if the character doesn't exist. The line node = node.children[char] ensures that the node is updated to the current character, so that the node points to the next letter's node in the tree. 
...
First, I found the node that is the last character of the prefix, then I performed the BFS, I used a queue which stored the node and current word. The queue is dequeued with each character and its children are enqueed with the new word (current word + child character). Lastly, when a node has no children, it is added to the list of suggestions because that means it's a completed word. 

### Your output

['thee', 'that', 'thag', 'there', 'their', 'thought', 'through']
...
In my function, I created a stack which contains the current node and the prefix, and the stack is used to track the nodes to visit next, which follows DFS. This continues in a loop until the stack is empty, where in each iteration it pops a node and the word formed so far from the stack, if the node has no children that means it's a complete node. If the node has children, it adds each child node to the stack. 

### Your output

['through', 'thag', 'that', 'thought', 'thee', 'their', 'there']

### Recursive DFS vs Stack-based DFS
In the recursive DFS approach, it uses the stack to keep track of the traversal state, each recursion call explores a child node, which calls itself until it reaches a leaf node. The stack based appraoch actually uses the stack data structure for the traversal. 

I used the stack-based implementation because it avoided recursion limit, and prevented possible recurssion errors that could happen with very deep trees. Additionally, stack approach can be more memory-efficient. 

## `UCS`

### Code analysis

I prioritized word sugfestions based on the character frequency for this UCS approach, and calculated the path cost by using the inverse of the frequency of each character transition, so that the higher frequency word is favoured. I used a priority queue so that the paths with the lowest costs are explored first
...
BFS explores level by level, and hence it gets the shorter words quick, so when it was 'th' it returned thee and that before thought and through. DFS goes deep, and hence explored through and thought before thee and that, and hence is useful when finding less frequent deeper suggestions. With UCS, it prioritized more freuquent character, and hence suggested thee and their before less common words. 

I tried with the prefix 'sa' too, and I got BFS: ['salty', 'savage'], DFS: ['savage', 'salty'], and UCS: ['salty', 'savage']

In this we can see BFS finds shorter words first, salty before savage. DFS explores the deeper words first so savage before salty. UCS uses frequency and word length, but in this case the character frequency between salty and savage has no difference, which is why it has the same as BFS order. 

To see a difference, I tried with just 't' and I got: 
BFS: ['tbh', 'tea', 'thee', 'that', 'thag', 'turnt', 'there', 'their', 'totally', 'thought', 'through']
DFS: ['through', 'thag', 'that', 'thought', 'thee', 'their', 'there', 'turnt', 'totally', 'tea', 'tbh']
UCS: ['tea', 'tbh', 'thee', 'thag', 'that', 'their', 'there', 'thought', 'turnt', 'through', 'totally']
