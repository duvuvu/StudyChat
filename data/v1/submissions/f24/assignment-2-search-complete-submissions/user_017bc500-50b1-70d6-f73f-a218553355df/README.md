...
For each word, we sequentially iterate through the letters. We start at the root node
, and check if the current letter is a child of the current node. If it is then we move into that child node.
If it is not then we create a child node for it, and then move into it. Once we finish the current word, we mark the current node as an end of a word.
...
First we navigate to the node of the last letter of the prefix. Then we create an empty queue, and add an array of each of the current nodes children, and the current string (prefix + letter). Then we loop till the queue is empty. In the loop we dequeue and then if the current node is the end of a word, we add the word to the completed words list. Then we add all the nodes children nodes + the string (parent string + new letter) to the queue. Once the queue ends all the words found are returned
...
First we navigate to the node of the last letter of the prefix. Then we create an empty stack, and add an array of each of the current nodes children, and the current string (prefix + letter). Then we loop till the stack is empty. In the loop we pop the top of the stack and then if the current node is the end of a word, we add the word to the completed words list. Then we add all the nodes children nodes + the string (parent string + new letter) to the top of the stack. Once the stack is empty all the words found are returned.
...
I used a stack based DFS because I thought iterating through a stack would be easier to implement in this case. The amount of things needing to be kept track of, and updated upwards in a recursive based implementation seemed harder then just looping, and storing pairs of nodes and strings in a stack, and appending words to a completed words list. 
...
In the build tree function, I added a counter for frequency of each node. First we navigate to the node of the last letter of the prefix. Then we create an empty heap, and push all the current nodes children and the current string, plus we give them a priority equal to 1/their frequency. Then we loop till the heap is empty. We pop off the lowest prio node from the heap, check if its an end of a word, and if it is add the word to the completed list. Then we add all the children to the heap, updating the string of each, and giving them a priority of the parent node prio + 1/ the new node frequency. Then once the heap is empty we return the completed word list.
...
For BFS the list was essentially just ordered by length, because it was going through each level of the tree, so all the short words come up first. For DFS it would go fully down a full subtree then trace up one then back fully down, so the words that build out of each other are very close to each other. For UCS it just seems enitrely random tho it does seem mostly the shorter words are higher up, and the shorter words trend towards the end.