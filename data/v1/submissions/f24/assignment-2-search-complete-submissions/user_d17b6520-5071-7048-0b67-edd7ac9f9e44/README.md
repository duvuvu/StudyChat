...
- Put the tree diagram for `test.txt` here
    ![tree diagram](tree.JPG)

### Code analysis

- Put the intuition of your code here
    Iterates through each character of each word in the document and creates a child node for the current node starting at the root node. If the node (character) already exists, it will move onto the next character unless it has reached the end of the word. In that case the period attribute it updates and the word attribute is true. The ordering of the children are prioritized by which was created first.
...
- Put the intuition of your code here
    Checks if the prefix is in the tree and if it is a word itself. The queue starts with the children of the last node of the prefix. The code loops through the queue as long as it is not empty to check if it is a word or not. Then it will check the children of the current node by adding it to the queue (checked after it is their turn).
...
- Put the intuition of your code here
    Check if the prefix is in the tree. Using recursion, we check if the current node has been visited (visited nodes are stored). If it has not been, it will be explored by first checking if it is a word. Then it is added to the visited set and we iterate through its children. The code iterates through all the grandchildren first before we explore the children of the original current node.
...
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
    Recursive DFS allows us to use for loops and recall our function. Begin by exploring the current node. Then we iterate the nodes at the current level and use recursion on the first child. This process repeats, thus the for loop allows us to explore unexplored children after grandchildren. Where as stack-based DFS uses a data structure (LILO).The nodes that need to be visited are added to a stack to determine the ordering of discovery. I used a recursive DFS after attempting a stack-based DFS.
...
- Put the intuition of your code here
    Checks if prefix is in the tree and if it is a word itself. Using recursion, we check if the current node has been visited (visited nodes are stored). If it has not been, it will be explored by first checking if it is a word. Then it is added to the visited set and explores the children in order of lowest period number.

### Your output

- Put the output you got for the prefixes provided here
    the, thee, there, their, thou, though, thought, that, thag, through


## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS.
    While playing around with the different searches I saw that BFS and UCS had similar suggestions, but the UCS had suggestions that were more accurate to the prefix. With DFS and UCS, some of the searches were similar as well but the DFS generated results that--personally--looked messy. You could clearly tell that the prefix was in the words but visually it was harder to read because some bigger words were on top or in the middle and the smaller words were on the bottom. As for DFS and BFS, since DFS was diving into a path, it was easier to spot words that had the same letters come after (besides the prefix). Comparatively, the BFS results were easier to spot the smaller words (as mentioned before).
