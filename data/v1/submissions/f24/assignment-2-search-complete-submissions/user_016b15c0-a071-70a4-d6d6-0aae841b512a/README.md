...
### Tree diagram
![tree_diagram](https://github.com/user-attachments/assets/150e1b0b-6ea3-4358-ac0c-d0b487c4bef3)

### Code analysis
The function builds a tree by going through each character of each word in the document by finding a child of the current node that matches the current character we are looking at and adjusts its priority accordingly. If no such child exists, we create one by adding a new node as the current node's child. This function also adds a node containing `'$'` to symbolize the end of a word.
...
The function traverses the tree from the root following the characters of the given prefix. Assuming words with this prefix exist in the tree, we perform BFS using a deque to represent a queue data structure (LIFO) that contains the node and the word that node represents so we can easily determine the entire words to return if we find the character `"$"` in one of the child nodes (as this represents the end of a word).
...
The function traverses the tree from the root following the characters of the given prefix. Assuming words with this prefix exist in the tree, we perform DFS using a deque to represent a stack data structure (LIFO) that contains the node and the word that node represents so we can easily determine the entire words to return if we find the character `"$"` in one of the child nodes (as this represents the end of a word).
...
A recursive DFS would involve calling the `suggest_bfs` function on each child of the subtree found after traversing the characters of the given prefix from the root and concatenating all of the words returned by each recursive call into one list of words to return. I used the stack-based DFS as it involved only changing the data structure from the `suggest_bfs` function into a stack and is more memory efficient as I am not making many recursive calls. The main difference between the two versions of DFS is that the recursive DFS starts at the "left-most" branch while the stack-based DFS starts at the "right-most" branch.
...
Once again, the function traverses the tree from the root following the characters of the given prefix. Assuming words with this prefix exist in the tree, we perform UCS using Python's heapq to make a priority queue that contains a 3-tuple with the priority, the word the node represents (acts as a tie-breaker for words with equal priority by comparing them lexicographically), and the node itself. We contain the word and the node as it lets us easily determine the whole word we need to return if we find the character `"$"` in one of the child nodes (as this represents the end of a word).
...
## Experimental 
BFS always seems to return suggetions in non-decreasing order in terms of the lengths of the words that match the given prefix. DFS seems to group words with similar prefixes together; for example with the prefix `"th"`, the words that have the prefix `"thou"` are next to each other. BFS does not seem to do this, and DFS also does not group by word length like BFS does. UCS sometimes groups words with similar prefixes, but this isn't always the case and is likely due to them sharing sequences of letters that are equally common. UCS does not always group or order by word length but seems to order by how often certain sequences of letters occur after the given prefix based on the given text document the tree was made from.
