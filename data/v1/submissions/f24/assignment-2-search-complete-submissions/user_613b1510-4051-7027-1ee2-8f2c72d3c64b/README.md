...
- Put the tree diagram for `test.txt` here
  ![image](https://github.com/user-attachments/assets/f4c71735-d516-4f45-89b6-6d536d8a9c43)


### Code analysis

- Put the intuition of your code here
- This function iterates through all the words in the document. then for each character in the word, if the its already in the dictionary it will set the current node to that character. else it will create a new node and set the current node to that node.
...
- Put the intuition of your code here
- This code starts by finding the node at the end of the provided prefix then adds each of its children to a queue. then it will pop the first item from the queue and start the whole process over until all the nodes are searched. if the node is an end node, it will work its way back up the tree in reverse to rebuild the word.

### Your output

- Put the output you got for the prefixes provided here
- "TH" : the, thee, thou, that, thag, there, their, though, thought, through
...
- Put the intuition of your code here
- This works exactly the same way as the BFS but instead of popping from the front of the queue it pops from the end. this means the newly added children are searched first instead of completing the whole layer. 

### Your output

- Put the output you got for the prefixes provided here
- "TH" : through, thag, that, thou, through, thought, the, thee, their, there

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
- Recursive DFS will traverse through the tree recursively (hence the name) meaning it will immediately search the down one path of the tree until it hits the bottom.
- Stack based DFS will add each child node to a stack then pick one to search and repeat until it reaches the bottom of the tree. Then it will move back up searching all the children it skipped over in the stack

- This is the stack based DFS because it wil add all children before moving to the next node rather than immediately searching them.
...
- Put the intuition of your code here
- This works similar to BFS and DFS by adding nodes to a list but instead of popping from the front or back it pops the node with the least path cost, or the most frequently used word.

### Your output

- Put the output you got for the prefixes provided here
- "TH" : the, thou, though, thee, that, thag, thought, there, their, through


## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 

BFS printed the suggested words in order of length starting with the smallest and getting bigger. this would be good for minimizing the number of steps needed to reach the goal

DFS' suggestions were in order of prefixes, for example when provided with the prefix"th" it matched all the "tha" words together then "the" words etc. this would be good for find lots of similar paths towards a goal

UTC lists the suggestions in order of frequency, so words that are used more a suggested more often. this would be useful for finding frequently used paths towards a goal
