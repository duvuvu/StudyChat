...
- For each character, if the current node contains it in children, traverse down that path and continue to the next character. If the character is not found, create a node for the current character and add it to the children of the current node, tracking frequency, cost, and if a node is the last of a word.

## `BFS`

### Code analysis

- After traversing the tree to the selected prefix, two queues are implemented: one to store the nodes being viewed, and one for the resulting words. By taking a queue approach (first in first out) the BFS traversal is implemented with words being pulled out by a flag for any ending characters.
...
- After traversing the tree to the selected prefix, two stacks are implemented: one to store the nodes being viewed, and one for the resulting words. By taking the stack approach (first in last out) the DFS traversal is implemented with words being pulled out by a flag for any ending character.
...
- I used a stack based DFS as it would be easier in order to track and store multiple stacks and lists. Instead of dealing with many parameters for a recursive method, I could have the two stacks and a final word list simply defined within the method.
...
- After traversing the tree to the end of the prefix, two priority queues are created to store the nodes and their corresponding words. These are then traversed by pulling from the priority queues based on lowest stored path cost, creating the final list of most frequent words.
...
## Experimental
- The words for BFS were organized in length order, as the tree was traversed each level at a time. DFS ordered the words based off of continuing letters, for example all words with an 'e' next were next to each other in the order. This was because each subtree was explored fully before the next. Finally, UCS put words that were substrings of other words and/or shorter first, as the frequencies of these words were higher and/or their paths were shorter, leading to lower cost.
