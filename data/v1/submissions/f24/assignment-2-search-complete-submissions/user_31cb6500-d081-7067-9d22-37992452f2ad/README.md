...
![Assignment Tree](https://github.com/<redacted>/assignment-2-search-complete-<redacted>/raw/40a031f9c488cba1f3eeff46cb088e936c87b71b/CS383-Assignment2Tree-1.png)
  

### Code analysis

- Put the intuition of your code here
The intuition behind the build_tree is to spilt up each the document into each word, then each word into its characters, and build the tree. If this is the first time going to the char (doesn't exist in tree), then we have to create the node with a count of one for the first time encountering it, else we have to add one to the count because another word uses a previously made node with the same prefix. We then navigate to that node until we finish the word, and once we exit the for loop we have the last node/char in our word, so we say it is an end node so we can recognize it and add to suggestions later. This will build the tree while keeping count of frequency of each word, and not creating unnecessary duplicates.

### Your output
The output of this is simply a tree of nodes each representing a char of their prefixes, as shown in the 'test.txt' diagram above. I tested this using print statements for each word and node created but it does not seem feasible to the total output of that in here.

## `BFS`

### Code analysis

The intuition of the code is to use the navigate function that I made to get to our starting node. It follows the tree using the given prefix and navigates to the starting node. If it is not a valid sequence it returns none and bfs returns no suggestions because it is not in our tree. After we have our start node we use a queue to run BFS adding each node's children to the end and popping them off as we go. If we reach an end node we add it to our suggestions list and keep traversing as a prefix of a word may be an output as well. Once we finish our queue we have reached all nodes so we return suggestions.

### Your output
Output: Autocomplete suggestions using bfs for prefix 'th':
...
The intuition is similar to BFS. We use navigate to traverse to the starting node, and then from there run DFS using recursion. We keep running DFS on the first child from the start node until we reach all nodes. We keep track of the prefixes as we go and store suggestions outside the function so we can return them when we finish.

### Your output

Output: Autocomplete suggestions using dfs for prefix 'th':
...
I used a recursive DFS because it is easier for me to deal with not knowing fully how to use a stack in python yet. Being able to call it recursively makes sense as we went through it intensely last semester in CS 311 (Algorithms), so that was my chosen implementation.
...
For the UCS I navigate to the starting node using the same inution as BFS and DFS, but this time I had to add a path cost to each node. We keep track of the frequency of each node as 'count' in build_tree, so we know the path for each node is just 1/(the node we are navigating to)'s count. We keep track of this in the start node and then from there we use a priority queue that stores the path cost for each node based on the parent's path cost + the cost to get to the child. Since we know the parent's path cost is correct from navigate and then it will always be updated correctly we know when we pop off the next node, it will always be the lowest path cost. We then store any end node in suggestions and return.

### Your output
Output:
Autocomplete suggestions using ucs for prefix 'th':
...
I notice that when we use BFS it goes level by level in our tree, returning the words at each depth before returning ones from the next one. When experimenting with the GUI it was easy to see that it returned each level correctly because of the length of the words returend. It returned each level first remaining the shorter words were always first because of our level-by-level approach.

DFS operates differently as it goes through the same prefix all the way down to the end before trying another. For example, it first returned all with the prefix 'the' as 'the', 'there', 'their', 'thee', before going to 'tho' 'thou', 'though', 'thought', and finally 'tha' and 'thr' as 'that', 'thag', 'through'. This is also easy to see with the GUI as all the suggested words are built off of one another with the same common prefixes.

Finally, for UCS it returns the most frequently used path. Since the has 4 words that use the 'the' prefix it is the lowest cost and therefore returned first. 'thou' is returned second as it contains 3 words that use the prefix, before returning 'thee' which is the lowest cost from there since it is only 1 away from 'the', and this process continues taking the smallest path based on what we had. This one was harder to see with the GUI unless you know the frequency, but it gave the most common answer which was cool to see.
