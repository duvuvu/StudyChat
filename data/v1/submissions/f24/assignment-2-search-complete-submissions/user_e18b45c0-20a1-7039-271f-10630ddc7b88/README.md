...
Yes, I used 383 GPT to help me clean up my code and provide context throughout the code. 
...
### Code analysis
The build_tree function constructs a prefix tree from a text document by calculating the frequency that each char appears by iterating through every word in the doc. Then, the function creates new nodes for the char that do not exist yet and marks the end of a work with an asterik. it also stores the inverse frequency of characters for the UCS function.


## `BFS`

### Code analysis
the suggest_bfs function starts from the node that corresponds to the prefix that is given, then collects all words that start with said prefix. It traverses through the tree on a level-by-level basis, and returns a list based on the words found in the document.
...
### Code analysis
the suggest_dfs function starts from the root corresponding to the prefix at hand, and explores each branch until the end before searching the next branch. This implementation also uses a stack to traverse through the tree and return a list of words from the document based on the input prefix from the user.
...
Recursive DFS: the function calls itself to explore each branch of the tree fully, therefore relying on the stack to manage the traversal of the tree. Better for shorter trees since it is a more compact implementation.

Stack-based DFS: differing from the recursive implementation, a data structure is created to keep track of the nodes explored. This means the function does not have to rely on the systems call stack. This implementation is longer, but is better for larger trees of navigation.

I ended up using a stack-based DFS implementation in my script, just to make sure I did not run into any problems with depth limits that a Recursive DFS might have.
...
## Experimental
Using UCS, it seems that the words with the most letters appear last in the list. With DFS, you can see how the search algorithm works as a longer word is built out under suggestions from shorter words. With BFS, most of the suggestions are short words since it is moving through the tree level by level, meaning that there will be less characters. 
