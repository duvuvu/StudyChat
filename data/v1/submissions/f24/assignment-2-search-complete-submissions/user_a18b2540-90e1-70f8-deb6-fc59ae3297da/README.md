...
The function iterates through each character of every word, adding 1 to the frequency of a specific letter in the tree if the corresponding node already exists, or creating to corresponding character node if it doesn't already exist. When a word is fully iterated through, the last node is marked as the end of a word. Once every word is added to the tree, a quick bfs search is made to calculate the priority of each character.
...
The function starts by iterating through the tree to pass the prefix. If the prefix isn't fully in the tree, the function returns nothing. Otherwise, it starts a regular BFS by starting at the last node in the prefix, all the children are added into a queue. Then, iterate through the queue until empty, starting with the first node in the queue, and adding to the return array a node's string from the root of the tree if the node represents the end of a word.
...
Just like BFS, the function starts by iterating through the tree to pass the prefix, and if the prefix isn't fully in the tree, the function returns an empty array. If it doesn't, it starts a stack-based DFS by starting at the last node in the prefix, all the children are added into a stack. Then, iterate through the stack until empty, starting with the last node added to the stack, and adding to the return array a node's string from the root of the tree if the node represents the end of a word.
...
A recursive DFS would repeatedly call a helper function where each call looks at a deeper node. With a stack-based DFS, you just use an array and a while loop with the end condition being that the stack is empty. For my implementation, I used a stack-based DFS.
...
Like BFS and DFS, the function starts by iterating through the tree to pass the prefix, and returning an empty array if the prefix isn't fully in the tree. The base idea of the UCS is fairly similar to the BFS, except for how the queue is formed. At every step where you would add to the queue, you instead create a temporary array of the priorities of all the nodes currently in the queue and all the nodes to be added to the queue. The temporary array is then heapified, and then you iterate through the temporary array, if each node to be added that has the iterated priority number hasn't been added yet, it gets added to the new queue.
...
BFS seems to have generated the list of suggestions in order of word length. If you type "th", the first suggestion is 'the', followed by 'thee' and other four letter words. DFS generated the list of suggestions in order of what letters might come next. If you type "th", the first suggestion is 'through', then the next few suggestions start with 'tha', then 'tho', and finally 'the'. UCS generated the list of suggestions in order of how many times the particular word appears. The first suggestion, 'the' appears in genZ.txt four times, followed by 'thou', which appears three times. The word 'though' appears twice, and the rest only appear once.
