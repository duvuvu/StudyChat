...
[BuildTreeDiagram.pdf](https://github.com/<redacted>/files/17107362/BuildTreeDiagram.pdf)


### Code analysis
  The code builds a tree-like structure (trie) where each node represents a character in a word.
  Words are added by creating a path of nodes for each character, with shared prefixes using the same nodes. The is_end flag marks complete words, allowing differentiation between full words and prefixes of longer words.

### Your output

- No output as per note on Campuswire
...
- Put the intuition of your code here
  INTUITION
  The suggest_bfs method implements a breadth-first search on the trie, starting from the node corresponding to the given prefix. It uses a queue to 
  explore nodes level by level, appending complete words to the suggestions list as they are encountered. The method returns all words in the trie 
  that start with the given prefix, in order of increasing depth.
...
- Put the intuition of your code here
- INTUITION
The suggest_dfs method implements a depth-first search on the trie, starting from the node corresponding to the given prefix. It uses recursion to explore branches of the trie, adding complete words to the suggestions list as they are discovered. This method returns all words in the trie that start with the given prefix, in the order they are found during the depth-first traversal.
...
The provided code implements a recursive DFS approach for traversing the trie. Recursive DFS utilizes the call stack to manage the traversal, offering a concise and intuitive implementation. This method is well-suited for trie structures, as they typically don't reach depths that would cause stack overflow issues. The code uses recursion through the dfs function, which calls itself for each child node, effectively exploring the trie depth-first.
...
- INTUITION: 
It uses a min-heap to prioritize exploration based on cumulative path costs, where lower frequencies (higher costs) are explored first. The method traverses the trie, adding complete words to a priority queue and expanding to child nodes, ultimately returning suggestions ordered by their cumulative frequency-based costs.

### Your output

  the thee their there thou though thought thag that through



## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS.
BFS explores the trie level by level, resulting in suggestions ordered by word length. This is evident in the output, where shorter words like "the" and "thee" appear first.
DFS explores the trie depth-first, leading to suggestions ordered by the trie's structure. This explains why "there" and "their" appear early in the output, as they share a common prefix.
UCS prioritizes suggestions based on path costs (inverse frequencies), resulting in an order that reflects word popularity in the document. Common words like "the" and "thee" appear first, while less frequent words like "thag" and "through" appear later.
