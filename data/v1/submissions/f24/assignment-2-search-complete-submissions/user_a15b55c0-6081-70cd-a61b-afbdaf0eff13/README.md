...
- Build tree splits the words into a list and goes through each one. Then it goes through each character in each word. If that character exists as a children of the prefix, it then goes to that child node and then increases the frequency by one. If it doesn't exist it creates a new node and instantiates it to a frequency of one. Once each character has been seen in the word, the final node will have a boolean value of true to indicate it is the end of the word.
...
- First, we go to the last character of the inputted prefix. If there is not character that is a child of that character, then it returns an empty list. Then the BFS is initiated by creating a queue. We first enque the first node along with the prefix via a tuple. We then go through the queue and pop off the first node on the queue. If that node is the end of a word we add it to the suggestions list. We then iterate through the current nodes children and add them to the queue along with the prefix plus the character of the current node. We then continue our search until we have traversed the whole tree and then return the suggestions. This finds the shorter words first.
...
- First, we go to the last character of the inputted prefix. If there is not character that is a child of that character, then it returns an empty list. Then the DFS is initiated by creating a stack. We first enque the first node along with the prefix via a tuple. We then go through the queue and pop off the last node on the stack. If that node is the end of a word we add it to the suggestions list. We then iterate through the current nodes children and add them to the stack along with the prefix plus the character of the current node. We then continue our search until we have traversed the whole tree and then return the suggestions. This finds the longer words first.

### Your output

[through, thag, that, thou, though, thought, the, thee, their,]
...
- First, we go to the last character of the inputted prefix. If there is not character that is a child of that character, then it returns an empty list. Then we utilize a priority queue to implement the UCS. Utilizing heapq we create a minheap.  Within the priority queue we add a tuple that has the cost, a counter to break ties, the current node, as well as the prefix. We then go through the priority queue and pop off the item with the lowest cost. We add the current node to the suggestions if it is one word. Then we take the inverse of the frequency of all the childrens and add them to each ones individual path cost, and push them onto the priority queue. This prioritizes characters with higher frequency.
...
BFS suggested all of the shortest words first, and then the longest words.
DFS tended to suggest the longer more uncommon words towards the front.
UCS tended to suggest words that had common prefixes first before going to words with lesser prefixes

I chose to implement a stack based dfs vs a recursive since recursion can go pretty deep, especially in a situation where we are given a large trie.. 
