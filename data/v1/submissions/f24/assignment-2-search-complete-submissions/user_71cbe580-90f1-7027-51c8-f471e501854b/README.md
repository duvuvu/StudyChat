...
- ![tree drawio(2)](https://github.com/user-attachments/assets/aa5e8c9c-e5bc-4f10-88ac-b456b8dcb098)



### Code analysis

- Iterates through the words in the document. For each word, starts from the top of the tree, which represents the empty string. Iterates through each character of the word: for each character, adds it to the expanding prefix (string --> string+char), looks for a child node representing string+char from string's node and creates one if it doesn't exist, and navigates to that node, incrementing the occurrences count and labeling the node with the correct string. Once each word is complete, it also labels the node corresponding to the full word as representing a complete word. Note that the occurrences don't represent the number of occurrences as a full word, but the number of times that node has been encountered while building the tree, so it represents the number of words that have the corresponding string as a prefix or as the full word.
...
- First navigates to the node corresponding to the end of the prefix, which will be the root of the subtree explored (if at any point in the traversal a needed node doesn't exist, returns an empty list). Then initializes an deque with the prefix's node in it and an empty suggestions list. Iterates through the nodes in the queue in FIFO order and adds all the node's children to the end of the queue, then adds that node's string to the end of the suggestions list if the node represents a full word. Ends when the queue is empty and returns the suggestions list.

### Your output

- with input th: the, thee, thou, that, thag, there, their, though, thought, through
...
- Pretty much the same as the BFS algorithm, but removes nodes from the queue in LIFO order instead of FIFO to address child nodes before sibling nodes. Navigates to the node representing the prefix, initializes a queue with the node representing the entered prefix, and iterates through the last added nodes to the queue, adding all their children to the end of the queue and adding that node's word to the suggestions list if it represents a full word. Ends when the queue is empty and returns the suggestions list.

### Your output

- For input th: through, thag, that, thou, though, thought, the, thee, their, there

### Recursive DFS vs Stack-based DFS
- I used stack-based DFS despite generally preferring recursive algorithms simply because at least for pre-order DFS the code could mostly be reused from the BFS function (and I don't believe there were specifications about which to use of pre-order, in-order, or post-order). The only difference is that here, new nodes should be taken from the end (LIFO) to look at the current node's children before a sibling node.

## `UCS`

### Code analysis

- Navigates to the node representing the entered prefix (if needed nodes for traversal don't exist at any point, returns an empty suggestions list). Initializes a heap of 3-tuples with format (-1*number of occurrences, entrycount (just an incrementing integer to be a tie-breaker), node) with the corresponding tuple for the node representing the prefix. Iterates through the nodes popped from the heap, which have lowest -count so the highest number of occurrences in the text data. Adds their children as appropriate 3-tuples to the heap, and adds the node in question to the suggestions list if it's a full word. Returns the suggestions list at the end.

### Your output

- input th: the, thou, though, thee, that, thag, thought, there, their, through


## Experimental
- BFS clearly sorted words by length, but not all words are more likely to occur; in particular, "thag" appeared much higher than it should have considering it occurs only once and is likely a typo. If the text database were larger, this would've been magnified as an effect, with lots of short typos populating the top of the list over much more common slightly longer words. In comparison, DFS generated words in sections clearly corresponding to the next letter being considered and as a result often prioritized long words; it went through all words starting with tha before considering the, which has 4 different words in the corpus starting with it. This is also not that helpful for this particular use case: in a broader application, it might suggest antidisestablishmentarianism before antibody just because it navigates through antid's children before antib's. UCS required more modification of the node class, but would most likely be the best of these options as common words are more likely to be the correct suggestion. However, modifying it so that it has separate counts for occurrences as a complete word and occurrences as a prefix might improve it, as in general thou occurs much more commonly as a prefix than it does as a complete word and shouldn't be recommended more frequently than though.
