...
- ![build_tree_test txt](https://github.com/user-attachments/assets/ef93e2af-20ec-4ce5-a6a5-3e6db099ad9a)

### Code analysis

- The build_tree method constructs a prefix tree (trie) from the given document. It iterates through each word, breaking it down into individual characters and adding each character as a node in the tree. If a character node already exists, it moves to the next character; otherwise, it creates a new node.

### Your output

- Put the output you got for the prefixes provided here
- i did not get any output without implementing any of the methods
...
- This method traverses the tree level by level, which ensures that shorter words are found first. It starts from the node corresponding to the last character of the prefix and then explores all children nodes in a queue-based manner.
...
- This method traverses the tree by diving deep into each branch first, starting from the node corresponding to the prefix, it recursively explores each child node, adding characters to build possible words. It finds longer words first because it continues down a path until reaching the end of a word before backtracking to explore other branches.
...
- Recursive DFS uses the system's call stack to traverse deep into the tree. It's simple and intuitive, but limited by recursion depth. In contrast, stack-based DFS manually manages the traversal using a stack, allowing deeper exploration without hitting recursion limits, but requiring more complex handling.

- I used recursive DFS for its simplicity and readability, as the tree structure naturally supports recursion. It's easier to implement and automatically handles backtracking, making it a practical choice for this problem's manageable tree size.
...
- This method finds cost for each path is based on the frequency of character transitions (i.e., common paths have lower costs). A priority queue (heapq) is used to prioritize more frequent paths.
...
In Breadth-First Search (BFS), suggestions are generated by exploring the tree level by level, prioritizing shorter words first. For example, with the prefix "th", BFS would suggest shorter words like "the" and "thag" before moving on to longer words like "thought" and "through". The key behavior of BFS is that it finds words closer to the root of the tree, ensuring the shortest completions appear first.

In Depth-First Search (DFS), the algorithm dives deep into the branches of the tree before backtracking, often suggesting longer or deeper words before returning to shorter ones. For the same prefix "th", DFS might return longer words like "there" or "through" earlier than shorter words like "the". DFS does not prioritize shorter words and instead explores each branch fully before moving to the next.

Uniform-Cost Search (UCS) prioritizes words based on the frequency of characters following the prefix. Words with more frequent characters appear first. For example, if "e" occurs more frequently after "th", UCS would suggest words like "the", "thee", and "there" before words like "thag" or "that". UCS focuses on suggesting more common or likely words by considering the frequency of characters.
