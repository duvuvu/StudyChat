...
![Tree Diagram](https://github.com/<redacted>/assignment-2-search-complete-<redacted>/blob/main/tree_img.png)

### Code analysis

- Begins at the root of the tree, and adds nodes to the tree letter by letter, increasing the number of times edges have been traversed previously, for use in UCS. Once the entire word has been added, it updates the last node to note that it was the final node in a word, which is used in all of the search algorithms.

## `BFS`

### Code analysis

- suggest_bfs first finds the node representing the characters the user has already typed. It then uses deque as a queue to store the frontier performing a breadth-first search. It performs the search by finding the sequences of characters in increasing length, and adds to the output list if the current node is marked as the end of a word.
...
- First skips to the first node after what the user has entered so far. It then performs a DFS search using a list as a stack, searching as deeply as possible before returning to shallower depths, and storing the words represented by any nodes marked as the end of a word. Once it has searched all of the nodes, the list of words that were found is returned.
...
- Instead of using an explicit stack, like I did with the list, the recursive version would instead use the stack generated by recursive calls to traverse the tree. As mentioned in class, this could change the order that paths are explored in, and therefore change the order that the words are returned in. I chose to use the list with iteration, instead of recursion.
...
- Once again the function skips to the current node the user has typed, and begins the search there. This time, I make use of a custom class I made, Path, to keep track of the total cost it has taken to reach each node. It then uses heapq to choose the next node with the lowest cost based on its Path, and then returns the list of words found at the end.
...
## Experimental
- With BFS, the results are returned based on length, so the shortest words will always be suggested before longer ones. UCS is similar, in that it typically prefers shorter words, but sometimes longer words with common prefixes are suggested above other shorter words. With DFS, the outputs are grouped primarily by common prefixes, with words that share more of a common prefix than others occuring next to each other. 
