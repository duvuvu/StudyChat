...
![unnamed](https://github.com/user-attachments/assets/e03d7ed2-8b88-47e2-bbea-4e9e14fb12fe)

### Code analysis

- Keep traversing the growing tree until a novel word in document is seen and initates the growth of new tree branches at line 25. After each loop of line 22, the current node is updated to the node of the child whos value matches that character in the input string. This way only novel paths are created in the tree.

## `BFS`

### Code analysis

- Find the last node in the search tree that represents the last character of the input and then maintain a queue of tuples each storing a node and the string of letters that lead to it. Add the children (tuples of the child node and string of characters that child represents) of the current node to the queue and chose a new node to expand on a first in first out basis. Everytime the end of a word is found, denoted by a boolean stored in the node, that string is appended to a list for the autocomplete.
...
- Identical to BFS except instead of poping the first item in the list, the search progresses by popping the last (most recent) item added to the list. 
...
- I used a stack based DFS therefore my search progressed from the rightmost to the leftmost leaf. In this case I was using a data structure to keep track of the the progression of nodes to search. If I were to use recursive, then the search would start from the left-most node and progress to the right-most while maintaining "location" in the recursive calls themselves. 
...
- The code progresses through the LIFO stack and new children are appended to the stack in order of least frequent -> most frequent. Therefore they are added where the node with the lowest path cost (highest frequency), is the last one appended, thus the next node traveled to. The order is calculated by the .sortfunction on a list of children based on frequency. 
...
## Experimental
- UCS seems to have the most ordered (most familiar in terms of actual autocorrect) suggestions because frequency seems to be an important indicator of usage potential. DFS groups suggestions into similar suffixes (branches of suffixes) while BFS suggests a mix of suffixes i. e. 2- examples of one suffix, 2- exmaples of another, 3-of another, 2- more that are continuations of the first examples and so on. Chilren and cousins are suggested together in BFS while in DFS, parents and suggested with all their chilren. 
