...
- This file has a `Node` class defined for you -
  ...
  - Each Node represents a single character within a word. The `Node class has 1 attribute - 1.`children`- This is a dictionary that stores - - Keys - Characters that which follow the current character in a word. - Values -`Node`objects, representing the next character in the sequence. **You might (most likely will) want the`Node`class keep track of more things depending on how you implement you`suggest` methods.\*\*
...
Iterates through the words and each of their letters. For each letter, we traverse down if it already exists in the tree and create it if it doesn't.

Sets the node as an end if it is the end of the word and every time a letter is traversed it increases the cost.

## `BFS`

### Code analysis

Traverses down the tree to find the prefix. Initializes a queue with the prefix node and string. Then it pops nodes from the queue and updates their visited state, adds the prefixes to results if they are the end of a word, and queues child nodes if they haven't been visited.
...
Traverses down the tree to find the prefix. Initializes a stack with the prefix node and string. Then it pops nodes from the stack and updates their visited state, adds the prefixes to results if they are the end of a word, and queues child nodes if they haven't been visited.
...
In a recursive depth-first search (DFS), the function calls itself for each child node until it reaches a leaf, using the call stack to manage backtracking automatically. Stack-based DFS uses an explicit stack data structure to explore nodes, allowing for more control over the traversal process and avoiding issues like hitting recursion limits for large trees. I used a stack-based approach to traverse the trie iteratively, enabling it to manage memory use more efficiently and handle larger datasets without the risk of stack overflow.

## `UCS`

### Code analysis

Traverses down the tree to find the prefix. Initializes a min heap with the prefix node and string and 0 cost. Then it pops nodes from the heap and updates their visited state, adds the prefixes to results if they are the end of a word, and queues child nodes if they haven't been visited with weights of their parent node plus the inverse of their frequency.

### Your output

the though thee that thag though there their thought through

## Experimental

BFS: Gives suggestions in order of shorter to longer words such that near words are not necessarily similar. This causes more jumping around while typing.

DFS: Gives suggestions seemingly in random groups of similar words. This causes more consistency while typing but tends to give worse suggestions that are farther from what is being currently typed.

UCS: Gives the most applicable suggestions between the three searches. Tends to give shorter words first unless a longer word is really common. Similar to BFS in terms of consistency while typing. Easily the one I'd most prefer to assist me in my typing.
