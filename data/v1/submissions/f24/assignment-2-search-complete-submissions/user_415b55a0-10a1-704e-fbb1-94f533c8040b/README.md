...
Iterates through every character of each word in the document. If the character is not already a child of the current node, create a new child with that character. Set the current node to the child with that character. Continue like this with the next character.

Calculate frequencies recursively. Frequency is 1 at an "end" node. At any other node, it is the sum of the frequencies of the nodes children. The inverses of these are used in UCS, but I found it easier to store the frequencies.
...
Go to the last node of the prefix by repeatedly finding the child that matches the next character of the prefix and moving there. 

Create a deque with the last prefix node in it. Until the deque is empty, pop the first element off the deque and add its children to the left end (keeping track of the word in progress by adding the next letter of each child). Any time you reach an "end" node, add the word to the suggestions array. After the loop, return the suggestions array.
...
Find the prefix in the same way as above.

Create a deque with the last prefix node in it. Until the deque is empty, pop off the first element and add its children to the right side (keeping track of the word in progress by adding the next letter of each child). If you find an "end" node, add the word to the suggestions array. After the loop, return the suggestions array.
...
In recursive DFS, you immediately go down the path of the first child you find, before returning back up and going through the other children. In stack-based DFS, you add all children to the stack and then explore the most recently added child. So, they may find words in different orders, but both would be depth first.
...
Use a heap. Store the cost, word-in-progress, and the node in tuples in the heap. Put the last prefix node in the heap in this way. Until the heap is empty, pop an element and push its children to the heap. If an end node is found, add the word to the suggestions array. At the end of the loop, return the suggestions array.
...
BFS just finds the shortest words first. DFS puts words close to others in the list that are close in the tree (have the most matching beginning characters). UCS tends to put shorter words earlier since they have less total cost, but not exactly. It puts common letters first, which is why thou is above thag, because o is a more common letter to come after th.
