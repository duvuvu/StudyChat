...
### Tree diagram
- ![IMG_3589](https://github.com/user-attachments/assets/c7e0b956-c504-4def-93a6-30f06b28e6f0)


### Code analysis

- The method inserts each word from the document into the tree one character at a time, with each character represented by a node in the tree.
- If multiple words share a common prefix (e.g., "bat" and "bar"), the method reuses the same nodes for those characters, reducing duplication and optimizing space.
- For each character that is not already in the tree at the current position, a new node is created and linked to its parent node, forming a hierarchical structure.
- Updates the frequency of the next characters for every node to aid in the UCS search.
- Once all characters of a word are added, the last node is marked to indicate that a complete word ends at that node, allowing the tree to distinguish between prefixes and full words.

### Your output

- Used suggest_random:
- thiuj, thsaz, thbfb, thocp.
...
- suggest_bfs implements a Breadth First Search to traverse the tree. BFS explores a tree by taking each level at a time and searching all nodes at that level before moving onto the next.
- Such an algorithm would provide shorter recommendations (words closer to the root) first since they usually terminate at upper levels, as we can see below. 
...
- suggest_dfs implements a Depth First Search to traverse the tree. DFS explores a tree by going deep into a branch of a tree before backtracking, exploring untill a branch terminates.
- Such an algorithm would provide longer recommendations depending on the structure of the tree, as we can see below.
- DFS may not always give the most "intuitive" suggestions in terms of word frequency or word length.

### Your output

- Used suggest_dfs:
- through, thag, that, though, thought, the, thee, their, there.

### Recursive DFS vs Stack-based DFS
- Recursive DFS uses the call stack of the programming language itself to maintain the state, making it more intuitive to write, but it could run into stack overflow issues with large inputs.
- Stack-Based DFS explicitly uses a stack data structure to control the traversal, offering more control over the search process and avoiding recursion depth limits.
- I have used a stack based approach to the suggest_dfs function.
...
- UCS explores nodes based on the cumulative path cost, prioritizing nodes that have the lowest cost first.
- In this case, we designed the cost to be the inverse of character frequency, meaning that more frequent characters lead to lower-cost words.
- Therefore, UCS prioritizes frequently used words while also considering word length.
...
## Experimental
- Using the prefixes "pa" and "th", I noticed some trends about the above searches as follows:
- BFS is best when shorter words should be prioritized.
- DFS can be useful for exhaustive searches but may not provide the most suited suggestions.
- UCS offers a balance, making it ideal when both word frequency and character length need to be taken into account, resulting in a more dynamic and practical autocomplete feature.
