...
- ![WhatsApp Image 2024-09-24 at 22 08 33_7feeba41](https://github.com/user-attachments/assets/7fd602b4-d3b5-4a2a-ae5b-560ba129831e)


### Code analysis

- Put the intuition of your code here
- the function iterates over each letter in each word starting from an empty tree and adding a char to the children of another if it is not already existing and update the node to be the last charchter for a word if that is the case
### Your output
...
- Put the intuition of your code here
- the function goes to the root and than search that the prefix indeed exists, than it iterates over all the childrens and add any words that ended to the word list and append all childrens to th end of the queue.
...
- Put the intuition of your code here
- the function goes to the root and than search that the prefix indeed exists, than it checks for each children if it is the last node, if so it adds it to the word list, if not it goes to first children recursively.
...
- I used a recursive DFS, i did it because there is no way to enter a loop in this tree so it is safe and quicker.
...
- Put the intuition of your code here
The function traverses the tree to find the end node of the given prefix, using a priority queue to handle nodes based on the cost associated with each character. It collects words that start with the prefix by popping nodes from the queue, checking for end-of-word markers, and enqueuing child nodes with updated costs based on the ASCII value of each character
### Your output

- Put the output you got for the prefixes provided here
- the, thag, thee, that, thou, there, their, though, through, thought.


## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS.
- It seems like the UCS is the most relevant one and usually will get words that ill might use. the BFS just gave the shortest words first what seems to me not helpful, the DFS was in between- sometimes found more common words first but sometimes suggested uncommon words.
