...
### Tree diagram
- ![Alt text](https://github.com/<redacted>/assignment-2-search-complete-<redacted>/blob/main/build_tree.jpg?raw=true)

### Code analysis

-  iterates through each word in the document, creating a new node for each character if it doesn't already exist in the current node's children, and marks the end of the word in the tree.

## `BFS`

### Code analysis

- method employs a bfs to find all completions of a prefix, collecting complete words by traversing the tree level by level

### Your output

- i read in a campuswire post " The report is based on genZ.txt and uses the prefix 'th' "... so all the outputs i have is based on that.['the', 'that', 'their', 'through', 'thag']
...
- method uses dfs to find all word completions for a given prefix, exploring nodes by going as deep as possible before backtracking to collect complete words

### Your output

- ['the', 'that', 'their', 'through', 'thag']

### Recursive DFS vs Stack-based DFS
- Recursive DFS uses the call stack to traverse nodes and it uses function calls to handle backtracking automatically. In contrast, stack-based DFS explicitly manages its own stack, allowing for more control over the traversal process and avoiding potential stack overflow issues in deep trees. So basically the recursive stack is replaced by a stack of nodes, that's the main difference. I used a stack-based approach.
...
- performs ucs starting from the node corresponding to the given prefix. It explores the tree by prioritizing nodes based on the length of the accumulated prefix, collecting complete words as it traverses, ensuring that the shortest suggestions are returned first.
...
## Experimental
- bfs provides a comprehensive view of all possible completions, dfs yields quicker results but might miss shorter options, while ucs prioritized brevity and relevance in its suggestions.
