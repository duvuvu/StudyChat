...
![image](https://github.com/user-attachments/assets/0d5839a1-78de-4cc7-850f-df9a9e49efde)

### Code analysis

- Method iterates through each word in the inputted document. Then iterates through the characters in each of these words. For each character, check if the character is contained in the current node's children. If so, then continue into that child node and set that to current, if not create a new Node child. At the same time, keep track of the frequency count each character in each node through incrementing. It stores the frequency of the next possible characters from that node. Once the end of word is reached, the node is marked as an end node.
...
- Put the output you got for the prefixes provided here
- ![image](https://github.com/user-attachments/assets/1a71e6ce-5219-43e2-87b7-eff4814d8892)
...
- A recursive DFS would utilize function calls to iterate deeper into the graph, so a DFS call would occur on each child of a node. Whereas with a stack-based DFS, each child is simply appended to the stack and the node which last entered gets explored next. I chose to implement the stack-based version of DFS as it is simpler to create due to its immense similarities to BFS implementation.
...
- First, store the root node. Then begin iterating through the trie which we have created and go down the path corresponding to the prefix which is being searched. Then get the last node at the end of the prefix string and add it to the priority queue for the UCS. Using the priority queue/min-heap ensures that nodes are processed based off their accumlated path costs, so words which are statistically more likely to occur will show up in the autocomplete feature first. As it traverses each node, the method checks if the current node signifies the end of a valid word and appends it to the autocomplete results if it does. Then, for each child node of the current node, the method calculates a new cost by adding the child's inverse frequency cost to the current total cost, pushing the updated state into the priority queue. This process continues until all reachable nodes are explored, ensuring that the autocomplete suggestions reflect the most relevant and frequently encountered paths in the trie, prioritized by their respective costs.

### Your output

- Put the output you got for the prefixes provided here
- ![image](https://github.com/user-attachments/assets/06f2d7d0-1ecd-4fdb-9965-cd6fd825431a)
...
- The BFS algorithm returns the shortest, closest words by length. This is due to its exploration by level, so it will return all of the words purely based off length and not relevancy. DFS returns the words which will be completed last following the inputted prefix string when the path is followed. This means it can return longer, completely unrelated words as the first option with very low relevance to the search. UCS returns the words that seem most likely to be written, typically the shorter words first and also more common phrases. This combines the best aspects of the searches by returning the words closer in length which also have the more relevancy to the search.
