...
![Tree Diagram](build_tree.jpg)


### Code analysis

- For this specific function, I tried implementing Trie and for each word, it iterates through the characters and keeps on creating new nodes. I added other attributes such as "end of word" tracker to see if its the end of the word and frequency which was useful in implementing UCS later.
...
- First, it uses the startsWith helper function to locate the Trie node corresponding to the last character of the prefix. If the prefix doesn't exist in the Trie, it returns an empty list. Otherwise, BFS is initiated using a queue that stores nodes along with their corresponding word fragments. The algorithm explores level-by-level, appending children of the current node to the queue and continuing the traversal. If a node marks the end of a word, the complete word (prefix + current char) is added to the result list.
...
- I primarily used a recursive DFS on the trie, starting from the node that matches the given prefix. It checks if the current node marks the end of a word, appending the accumulated prefix to the suggestions list when it does. It iterates through the children of the current node, recursively calling itself for each child, thereby exploring all possible paths and collecting complete words that extend the given prefix. 



### Your output

- Recursive output: the, there, their, thee, thou, though, thought, that, thag, through
- Stack based approach output: through, thag, that, thou, though, thought, the, thee, their, there

### Recursive DFS vs Stack-based DFS
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
- The suggest_dfs_recursive method uses a recursive approach, where each function call explores the trie from the starting node until it finds complete words. When a word is complete, it adds it to the suggestions list.

On the other hand, the suggest_dfs_stack method uses an iterative DFS with a stack to keep track of the nodes and their prefixes. This avoids the limitations of recursion, like running out of stack space as mentioned in the textbook as well. Both methods effectively find all word completions for the given prefix, but the stack-based approach can be more efficient for larger tries and gives better control over space constraints.
...
- Again for UCS, we start by finding the start node using the helper function that helps by finding the start node. I used a heap as suggested to find the lowest cost path first. A very cool thing that i found out was using itertools.count() to assign unique numbers to each entry in the heap, ensuring that nodes with the same cost don't get mixed up and to break orders with same cost in a deterministic way.
- As the implementation suggests, first i pushed the starting node in the heap and it keeps on running until all the words are explored. We keep on popping the ones with the lowest cost and if its the end of the word, then we append it to our list.
- Also, another thing critical for this function is to update the cost everytime cumulatively.
...
- Upon experimenting with different prefixes, I found out that BFS specifically as discussed in the lecture as well goes level by level and the length of the word keeps on increasing. The last word is the usually the longest in that list of suggestions.
- DFS on the other hand, goes down deep in one branch of the trie and keeps on suggesting words and it could be both short and long. With this basic implementation, we cannot necessarily say anything about the length of the words based on the order. Also another thing is that the order of words is different with recursive and stack based implementation.
- UCS suggests the most frequent words first as they have a lower cost when we take the inverse of it, it comes first in the priority list. They don't prioritize shorter words like BFS algoithm.
