...
> For each word, traverse its characters, creating 
> nodes for any new characters and assigning costs 
> to them while marking the last character as an 
> "End" node. After processing a character, update 
> the reference of the current node to point to its 
> child to extend the tree branch.

### Your output

- Put the output you got for the prefixes provided here

`build_tree` does not return any value.
...
- Put the intuition of your code here
> Using a queue of tuples containing (node, word), 
> traverse the tree level by level, combining the 
> word with the characters of child nodes to create 
> new words that will be added to the result if they 
> pass both checks: starting with the given prefix 
> and terminating at a valid "End" node. Since queues 
> operate in a 'First-in-First-Out' order, the program 
> explores the oldest nodes first, ensuring a breadth-first 
...
- Put the intuition of your code here
> Recursively traverse the tree by focusing on one branch 
> at a time, creating words by appending characters from 
> child nodes. As you form new words, check whether each 
> word starts with the given prefix and ends at a valid 
> "End" node.
...
> In a recursive depth-first search (DFS), the program 
> explores the tree by following one branch at a time. 
> Once it reaches the end of a branch, it backtracks to
> search through other branches. While stack-based DFS 
> employs a stack to store nodes, it operates similarly 
> to breadth-first search (BFS) in terms of program flow
> but the children of node needs to be reversed to maintain
> left-to-right order. Stacks follow a 'Last-in-First-Out' 
> order, which allows the program to prioritize the most 
> recently visited nodes, thus focusing on one branch at a 
> time. In the end, I chose the recursive DFS approach 
> because it is easier to implement in this case compared 
> to the stack-based method.
...
- Put the intuition of your code here
> My intuition is to traverse the graph similar to the BFS
> algorithm I described earlier but instead uses a priority 
> queue to pop the current node with the smallest cost of
> path in each iteration. The priority queue compares the
> total cost of path from root to the give char and if the
> costs are the same, compare their currently forming words
> instead.
...
> The suggestions generated by breadth-first search (BFS) 
> are sorted by word length in ascending order because BFS 
> traverses the tree level by level, with each level determining 
> the length of the current word. In contrast, the suggestions 
> generated by depth-first search (DFS) place all shorter prefixes 
> of a word before longer variations, as the program explores one 
> branch at a time. If a word ends in the middle of a branch, 
> any word that continues from that point will have the shorter word 
> as a prefix. For uniform cost search (UCS), the suggestions prioritize 
> words based on their frequency, as the search process explores branches 
> starting from lower-cost nodes before others, allowing words from those 
> with higher-frequency prefixes to appear first.
