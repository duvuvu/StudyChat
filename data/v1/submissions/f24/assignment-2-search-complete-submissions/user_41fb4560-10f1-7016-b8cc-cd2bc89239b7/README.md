...
No, but I did have to re-learn a bit of python

## `build_tree`

### Tree diagram

Pictoral (note that -| means the end of a word)
![Pictoral Node Tree](/Pictoral%20Node%20Tree.jpg)
Code-based
![Code-based Node Tree](/Code%20Node%20Tree.jpg)

### Code analysis

My tree-building is implemented by walking through the characters in each word, walking along the tree and creating nodes if they do not already exist. Each word is then ended by adding a null character as a child with an empty node to mark the end of a valid word. After the tree is generated, the frequency costs are calculated by doing a depth-first-search to recursively count the number of leaf nodes each node has connected to it (or a cost of 0 if it is a leaf node, and therefore a null character) and taking the inverse of that number for its cost.
...
My bread-first-search suggestions algorithm walks along the tree according to the prefix. Whatever node this ends at is searched from using a deque that keeps track of the words it builds by using tuples. As it walks the tree, it adds the characters it explores to the word part of its tuple to the queue. When it reaches a null character, it adds the word in the tuple to the suggestions it returns. As this is a queue, each level is searched before the next is searched, as nodes are searched in FIFO order. Eventually all words starting with the prefix will be returned in the order they are found, which will naturally be in order of word length from shortest to longest.
...
My depth-first-search suggestions algorithm walks along the tree according to the prefix, and then uses recursion. A helper function calls itself on the children of the current node with the associated current character added to the total word. As a base case, if the null character is being explored, the word is appended to the suggestions list and returned. As more base cases are reached, the list is merged with the base case returns until all valid words are appended, then the list is returned for a final time. Since each subcall must be evaluated until a base case is reached, the behavior is depth-first. This will naturally result in a list of words dependent on the order in which they were added to the tree. For example, if the first word started with "a", the first words suggested would be the words that start with "a".
...
I used recursive DFS as this saves some space that a stack-based imnplementation would take up. However, if the dictionaries used by this program were much larger, and therefore had much larger trees to search through, call-stack overflow may become a concern, and so a stack-based DFS might be better for scalability. I had this issue like this once for an assignment similar to this one with a much larger search space.
...
My uniform cost search suggestions algorithm walks along the tree according to the prefix. Starting at the node the initial prefix walk ended at, a priority queue (as a heapq) has children of current nodes added to the queue according to the current cost plus the frequency cost of the child. The queue is ordered by frequency cost (arbitrarily tiebroken with custom less than comparison in Node class) processed in lowest-cost order until all words are added to the suggestion list. The suggestions are then returned. This results in a list that prioritizes words that have more words with the same prefixes and also prioritizes shorter words.
...
## Experimental
BFS resulted in suggestions ordered by length from shortest to longest, DFS resulted in suggestions ordered by the order they were added (order in the file) with words of the same prefix grouped, and UCS resulted in suggestions ordered by a combination of prefix equivalence and short length.
