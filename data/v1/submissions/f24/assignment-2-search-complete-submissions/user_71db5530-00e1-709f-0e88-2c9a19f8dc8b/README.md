...
## 383GPT
Did you use 383GPT at all for this assignment? **Yes, only for the description of the UCS code.* 
...
A recursion based DFS would perform the search by recursing to the bottom of the tree continuously on a single path and return the path, whereas stack based  DFS would traverse to the bottom of the tree by adding each children to the end of stack and continue with the last children added. I implemented a stack-based DFS search algorithm because it is more straightforward than a recursion algorithm and the code would resemble the queue-based BFS algorithm.
...
BFS would give results in the order of ascending word length (number of characters), DFS would give results in the alphabetical order, and UCS would give results in the order of descending word frequency (number of appearance)
