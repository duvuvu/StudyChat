...
![Tree Diagram](https://github.com/<redacted>/assignment-2-search-complete-<redacted>/blob/main/IMG_5501.jpg?raw=true)

### Code analysis

- Put the intuition of your code here

The build_tree function constructs a tree by inserting each word from a list, creating nodes for each character and making paths that represent the words. 
...
The suggest_bfs function navigates through the tree based on a given prefix, using a queue to have all valid autocomplete suggestions. It checks for the end of words and gets suggestions by exploring child nodes level by level.
...
The suggest_dfs function finds autocomplete suggestions in a tree by traversing to the node for a given prefix and using a stack to explore child nodes and collecting words. It returns the list of valid suggestions encountered during the search.
...
Recursive DFS uses the call stack to navigate nodes which makes the code simpler and easier to follow. A stack-based DFS uses a stack to traverse through it. Recursive DFS is best for simple trees, while stack-based DFS is better for larger trees. I use the stack-based DFS to collect autocomplete suggestions by exploring nodes one by one.
...
The suggest_ucs function uses Uniform Cost Search to get autocomplete suggestions based on a prefix, by prioritizing nodes by their cost which is calculated by using character frequencies. This means that more common characters help generate more relevant suggestions.

### Your output

- Put the output you got for the prefixes provided here

['the', 'thee', 'that', 'thou', 'thag', 'there', 'their', 'though', 'thought', 'through']
...
When using BFS, the suggestions are diverse and cover a broader range of options as it explores all nodes at the same level before moving deeper. In contrast, DFS tends to focus on one path, which can result in fewer and maybe less relevant suggestions as it may overlook some. UCS prioritizes suggestions based on character frequencies, leading to more relevant results which makes it the most effective approach for generating useful autocomplete suggestions.
