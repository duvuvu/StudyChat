...
![alt text](tree_diagram.png)

- Tree Diagram Explanation:
This diagram illustrates the tree structure from the build_tree function. Blue indicate the weights of each branch, representing the inverse-frequency of character transitions. Red circles mark the end of words ('~'), also each end node shows the cumulative weights from the root to each endpoint.

### Code analysis

- Constructs a tree structure to represent character relationships in a given document. It calculates the frequency of each character following specific prefixes using two dictionaries: frequency, and total_count. As it processes each word, it builds nodes for each character, setting the cost of reaching each node based on the inverse of its frequency. It also marks the end of each word with a special character ('~') and assigns a cost of zero.

## `BFS`

### Code analysis

- Uses breadth-first search to explore the tree level by level, starting from the node that matches the prefix and collecting words by traversing through child nodes, appending **completed** words when the end-of-word marker is encountered.
...
- Performs a depth-first search to find all words that match the given prefix by recursively traversing the tree from the corresponding node, and examines each child node alphebtically as far as possible before appending **complete** words to the suggestions list when the end-of-word marker is reached.
...
- Recursive DFS uses function calls to traverse the tree, making it concise but more prone to human error in code. I chose Stack-based DFS as it manually manages the traversal, providing better control. I could follow along with each traversal on hand, making it easier to track.
...
- Expands nodes with the lowest cumulative cost first, ensuring the most frequently used words are suggested earlier by adding all children to the queue with their respective costs.
...
## Experimental
- BFS explores all nodes at the current level before moving to the next level. As a result, it lists shorter words first if they share the same prefix.
- DFS follows a path down the tree to the deepest node before backtracking. This leads to suggestions based on alphabetical order of characters (due to manual sorting in the code). It explores one branch fully before moving to the next, prioritizing "thag" and "that" before returning to "the" and others.
- UCS prioritizes words based on rolling sum of path cost. It results in "the," "thou," and "thee" appearing first, reflecting their lower cost. Overall, it seems to be the better of the three, for this context.
