...
At a certain character, check to see if there is a child of the current node that matches that character. If it doesn't exist yet, create it, then set to that child. When the for loop reaches the end of the word, add an empty child to signify the end of the word and add a "string" attribute to the node representing the last character of the word.

### Your output

- Not applicable, no output is expected
...
- Create a queue of nodes and iterate through them first-in-first-out. When BFS reaches the end of a word, add that word to the returned word list if it starts with the given prefix.
...
- Create a stack of nodes and iterate through them first-in-last-out. When DFS reaches the end of a word, add that word to the returned word list if it starts with the given prefix.
...
- DFS is an inherently recursive algorithm, but it can use either a call stack or a stack data structure. I chose to implement it iteratively with a stack.
...
- When the tree is created, add one to the frequency (inverted path cost) of a node each time it is explored. To perform a uniform cost search, create a priority queue (frontier) sorted by decreasing costs each iteration. Each iteration, choose the node with the lowest cost and add the word to the returned list if applicable.

### Your output

- the thou though thee that that thought there their through



## Experimental
- BFS autocorrect prioritizes shorter suggestions over longer ones. DFS suggests some longer words first, but they're not strictly decreasing - rather the suggestions get longer until the autocorrect finds runs out of words with the current word's prefix. UCS suggests words sorted by decreasing frequency.
