...
My code iterates through the characters of each word in the document and adds them to the trie, updating their cost values as the frequency that the character sequence is observed. Then, it recursively updates each cost as the inverse frequency of that character sequence. 

### Your output

- Put the output you got for the prefixes provided here

My code does not output anything for this method.
...
My code checks if the length of the prefix is at least 1, then searches the trie for the end node of the prefix, and then searches for all possible suggestions by first removing the first node that has been enqueued into the frontier, then checking if a character is an EOS token (goal node), then enqueueing all of its children nodes into the frontier, and then repeating this process until the frontier is empty. It returns the list of all suggestions generated by this search by tracking each node in the frontier as a tuple of both the node to be checked and its prefix + character as a suggestion.
...
My code checks if the length of the prefix is at least 1, then searches the trie for the end node of the prefix, and then searches for all possible suggestions by first removing the most recent node that has been enqueued into the frontier, then checking if a character is an EOS token (goal node), then enqueueing all of its children nodes into the frontier, and then repeating this process until the frontier is empty. It returns the list of all suggestions generated by this search by tracking each node in the frontier as a tuple of both the node to be checked and its prefix + character as a suggestion.
...
Recursive DFS will explore the first possible path for each branch based on how the dictionary is ordered, whereas stack-based DFS will explore the last possible branch for each path based on how the dictionary is ordered.

I used stack-based DFS because it was a very easy modification to my already-coded BFS (changing popleft to pop).
...
My code checks if the length of the prefix is at least 1, then searches the trie for the end node of the prefix, and then searches for all possible suggestions by first removing the node with the smallest cost into the frontier, then checking if a character is an EOS token (goal node), then enqueueing all of its children nodes with their total path cost into the frontier, and then repeating this process until the frontier is empty. It returns the list of all suggestions generated by this search by tracking each node in the frontier as a tuple of the total path cost of the node, the node to be checked, and its prefix + character as a suggestion.
...
The suggestions generated by BFS are ordered first by length and second alphabetical order, the suggestions generated by DFS are ordered by reverse alphabetical order with no attention to length, and the suggestions generated by UCS are ordered by the most frequent character sequences first with smaller words that are substrings of others coming first. 
