...
Yes, but I accidentally used the assignment 1 tab for some of the questions

## `build_tree`

### Tree diagram
- Put the tree diagram for `test.txt` here
    ![Tree Picture](tree.jpg)

### Code analysis

- Put the intuition of your code here
    For each word, it keeps track of the length of the word, what number character we are on, and has a pointer
    to the node of the previous letter or the root, if there are none.
    Goes through each letter of the words and adds it to the tree if it isn't present yet. 
    For each of the final letters of the word, set the lastLetter property to True
...
- Put the intuition of your code here
    Iterates down the tree to get to the last character of the prefix's node and adds each of the possible 
    next letters to the prefix, putting them in a queue.
    Goes through the queue, checking if the last letter of the string's node is the last letter of a word
    and if it has anymore children.
    If they don't add the word to the suggestions; if they do, add the letter to the string and put it back
    in the queue.
...
- Put the intuition of your code here
    Iterates down the tree to get the last character of the prefix's node and adds each of the possible 
    next letters to the prefix, putting them in a stack.
    Goes through the stack, checking if the last letter of the string's node is the last letter of a word
    and if it has anymore children.
    If they don't, add the word to the suggestions; if they do, add the letter to the string and put it back 
    in the stack
...
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here.
    I used the stack-based DFS because the logic is very similar to the queue for BFS, which means the code
    is very similar. It was just easier to translate the queue to a stack, rather than coming up with new 
    logic for recursion.
...
- Put the intuition of your code here
    Same starting logic as the two previous functions. 
    Compares the path cost for each letter in the queue and takes the lowest one.
    If it is the end of a word, add it to the suggestions; If it isn't add its children


### Your output

- Put the output you got for the prefixes provided here
    the, thou, thee, thag, though, that, their, there, thought, through


## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 
    BFS always gives the shortest words first, since it searches level by level.
    DFS can give longer words, since it fully explores a branch first. This gives the recommended words more 
    variation on length.
    UCS tends to give shorter answers first, since the cost of finding them is usually less than longer words.
    However, commonly used long words can be more cost efficient than some short words, so there is some variation
    in word length