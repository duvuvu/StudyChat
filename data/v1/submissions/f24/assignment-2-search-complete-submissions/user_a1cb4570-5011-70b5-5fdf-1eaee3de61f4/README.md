...
### Tree diagram
<img src="image.png" alt="Tree Diagram" width="400"/>

### Code analysis

The build_tree method constructs a tree by iterating over every word in the document, and every character in each word (nested loop). If the character is not already a part of the tree, it creates a node for that character. Then traversal continues onto the child node, and frequency of that node (prefix) is incremented. The frequency represents how many times the prefix up to that node has been encountered in the document. At the end of every word, the is_word attribute is set to true, indicating the end of a word. 
...
The suggest_bfs method performs a breadth first search by using a while loop with a counter to iterate over the queue. It simulates a queue by appending child nodes to the end of the list and incrementing the counter to process nodes level by level and collecting words that start with the given prefix.

### Your output

![Output for BFS](bfs.png)
...
The suggest_dfs method performs a depth first search recursively by starting from the node corresponding to the last character of the prefix. It defines a helper function dfs that recursively explores each child node, building up words by adding characters, and appends complete words to suggestions when a word end is found.

### Your output

![Output for DFS](dfs.png)

### Recursive DFS vs Stack-based DFS
Recursive DFS uses function recursion and the call stack to manage traversal, leading to cleaner and more readable code, while stack based DFS uses a stack data structure to manage nodes. I used recursive DFS because I am more comfortable with it and it comes to me more naturally.
...
The suggest_ucs method performs a uniform cost search by using a heap that prioritizes nodes based on negative frequency (to simulate a max-heap). It uses a counter as a unique tie-breaker to ensure heap elements are comparable when frequencies are equal, incrementing the counter each time a node is pushed onto the heap. The method traverses the tree starting from the prefix node, collecting words in the order of highest frequency.

### Your output

![Output for UCS](ucs.png)



## Experimental
- BFS results are ordered by word length (shortest first).
- This approach is helpful when users are likely to be typing shorter, more common words, and is my personal favorite because it most resembles existing autosuggest features.

- DFS explores one branch of the tree deeply before backtracking (depth first).
- This may result in longer words being suggested before shorter ones.
- Can quickly find longer or less common words in a particular branch.

- UCS prioritizes words based on frequency.
- This is effective for suggesting commonly used words to the user.

Prefixes I used apart from "th":
