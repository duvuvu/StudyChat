...
- Put the tree diagram for `test.txt` here
  ![build_tree](https://github.com/user-attachments/assets/dcae9e54-df3e-4fa5-ae18-9dd5c7101bc5)

### Code analysis

- Put the intuition of your code here
* The function takes a document and builds a trie. The document is initially split into individual words. Then for each word, it iterates through each character of the word, checking if there exists a node for the character. If not, a new node is created. The function also tracks the frequency of each character at each node. If it doesn't exist it is initalized to 0. If it does exist it is incremented by 1. Once all the characters of a word are added, last node is marked as the end of the word.
...
- Put the intuition of your code here
* The function initially traverses through the trie to locate the node representing the last character of the prefix. If any character isn't present, then an empty list is returned (No words with that prefix exist). Once the prefix node is found, then a BFS is performed using a FIFO queue, starting from the node of the last character in the prefix. For each node, if it marks the end of the word, the corresponding word is added to the `suggestions` list. This algorithm ensures all possible completions of the prefix are explored level by level.
...
- Put the intuition of your code here
* DFS search is implemented in this function. The function intially traverses the trie to locate the node that represents the last character of the prefix. If a character is not found in the prefix, then an empty list is returned (No words with that prefix exists). The function utilizes a LIFO queue (stack). It starts from the node of the last character of the prefix. If a node marks the end of the word it is then added to the `suggestions` list.
...
* Recursive DFS relies on function calls to manage the traversal. Meanwhile the stack-based DFS used an explicit stack to track nodes. I think Stack-based DFS offers more control over the traversal process, making it easier to debug the search logic. In my function, I used a stack-based approach, as I felt I would have a more manageable way of exploring the trie.
...
- Put the intuition of your code here
* This function utilizes the Uniform Cost Search algorithm. It starts by locating the Trie node for the last character of the prefix. If any character is missing, an empty list is returned (no words contain that prefix). Using a priority queue, the function explores nodes based on the cumulative path cost, which is adjusted by the inverse frequency of characters to prioritize common characters. Words are added to the `suggestions` list as their end nodes are reached. This process continues until all possible nodes are explored.
...
* The main differences in the suggestions generated by BFS, DFS, and UCS come from the order in which the nodes are explored. BFS explores nodes level by level, which means shorter words are prioritized first. Thats why words like "the" and "thee" appear first in the list. DFS, explores deeper paths first. THis results in longer words (compared to BDS) like "thag" and "that" being first in the list. Finally, UCS prioritizes nodes based on character frequency. This leads to more common words like "the" and "thou" being suggested earlier, as frequently used characters are prioritized. 
