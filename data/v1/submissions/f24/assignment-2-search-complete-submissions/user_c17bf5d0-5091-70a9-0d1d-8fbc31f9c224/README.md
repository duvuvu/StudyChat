...
![screenshot](treeDiagram.png)

### Code analysis

- Builds a trie data structure from the input document by iterating through each word and character, creating nodes for new characters, and updating the transition frequencies for existing characters. The end of each word is marked in the trie, and the frequency of occurrences of each word is tracked.

## `BFS`

### Code analysis

- Implements a breadth-first search (BFS) algorithm to find all words in a trie that begin with a given prefix. It traverses the trie from the prefix's last character, collecting valid words by exploring all possible child nodes and appending them to a queue for processing.
...
- Implements a stack-based depth-first search (DFS) approach to find all words in a trie that start with a specified prefix. It explores each path from the last character of the prefix, collecting valid words as it traverses the trie by utilizing a stack for processing the child nodes.
...
- In a recursive depth-first search (DFS), the function calls itself for each child node, utilizing the call stack to keep track of the current path and state, which can result in cleaner and more straightforward code. However, this can lead to issues with stack overflow for very deep recursions. On the other hand, stack-based DFS explicitly manages its own stack, allowing for more control over iteration and memory usage while avoiding recursion depth limitations.
- In this implementation of `suggest_dfs`, I used a stack-based DFS approach, which allows for flexibility in traversing the trie without the risk of reaching the recursion limit, making it more suitable for handling deeper structures efficiently.
...
- Implements a uniform cost search (UCS) algorithm to suggest words in a trie that match a given prefix, prioritizing suggestions based the frequency of characters following the prefix. It uses a priority queue (min-heap) to explore nodes, where each node is assigned a path cost by summing the inverse frequencies of character transitions, ensuring that shorter and more common word completions are suggested earlier.
...
- Breadth-First Search (BFS): The suggestions are ordered based on their proximity to the prefix, resulting in a list that starts with the most immediate continuations of "th." This reflects a layer-by-layer exploration of the trie, with the output growing in length and capturing variations that are closely related to the prefix early in the suggestions.

- Depth-First Search (DFS): The output is somewhat similar to that of BFS, but the order of the words shifts based on the traversal path taken. DFS explores one branch fully before moving on to others. While still maintaining relevant words, the suggestions appear in a different sequence, emphasizing the exploration of certain branches of the trie before others.

- Uniform-Cost Search (UCS): The suggestions differ significantly as UCS prioritizes words based on their frequency, starting with the most frequently used words associated with the prefix "th." This method not only considers proximity but also the path cost of each node, resulting in a list that does not adhere to a purely alphabetical or depth-based order. The presence of "the", "thou" at the top reflects their higher frequency compared to others, while less frequent words appear later in the list, providing users with contextually relevant options that better align with typical usage patterns.
