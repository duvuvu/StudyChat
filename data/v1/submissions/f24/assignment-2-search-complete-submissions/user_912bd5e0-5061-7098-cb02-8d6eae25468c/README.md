...
def fizzbuzz(n):
    for i in range(1, n + 1):
        if i % 15 == 0:
            ...
            prihttps://github.com/<redacted>/assignment-2-search-complete-<redacted>
        else:
            print(i)
...
- This function constructs a (prefix tree) by breaking down each word in the document into characters. It builds a hierarchical structure where each node represents a character. After each word is recorded, the suffix of that word is marked as true, this helps in cases where a smaller word is in a larger word for example "the" is in "there" as after the, there are more children. The frequency of the number of times a node is passed through while adding a new word is stored.
...
- The code first finds the node at the last letter of the prefix and makes it the root node. Using a queue(FIFO) we add new children nodes when we explore a new node. once the node is explored, the node is removed from the queue. As we go explore a new node we add the char of that node to the prefix. If a node does not have any children then whatever prefix is created is appended to suggestions and then the next node in the layer is explores. If a node has a suffix that is true and has no children then the prefix is added to suggestions. when the queue gets empty then we return the suggestion list

### Your output

- Put the output you got for the prefixes provided here: li, b, glo

li: ![alt text](bfs3.png). ['lit', 'liter']
b: ![alt text](bfs1.png) ['bet', 'bruh', 'basic', 'bestie']
glo: ![alt text](bfs2.png) ['glow', 'glowed']
f: ![alt text](bfs4.png) ['fr', 'fam', 'fire', 'flex', 'facts', 'finna']
...
- The code first finds the node at the last letter of the prefix and makes it the root node. Using recursion we explore all the nodes. What we do is we first explore the first child of the node, and we recursivly call the function on the first child, to find its first child. We keep recursivly exploring nodes until we reach a node with no children. we then add that prefix created onto suggestions. then we check the next child on the last child called, recursivly until the entire graph is explored. If a node has a suffix that is true and has no children then the prefix is added to suggestions.
...
li: ![alt text](dfs1.png) ['lit', 'liter']
b: ![alt text](dfs2.png) ['bet', 'bestie', 'bruh', 'basic']
glo: ![alt text](dfs3.png) ['glow', 'glowed']

### Recursive DFS vs Stack-based DFS
-  Using recursion we explore all the nodes. What we do is we first explore the first child of the node, and we recursivly call the function on the first child, to find its first child. We keep recursivly exploring nodes until we reach a node with no children. Each recursive call dives deeper into the tree. Stack-based DFS uses a stack to manage the traversal. Nodes are pushed onto the stack as the algorithm dives deeper, and popped off as it backtracks. As we explore a node, its children are added to the front of the stack.
...
- The code first finds the node at the last letter of the prefix and makes it the root node. Using a heap we add new children nodes when we explore a new node. Every time a new node is explored a path cost is calculated by adding the inverse of the frequencies stored in the node. The new node is added to the heap according to it's path cost. As we go explore a new node we add the char of that node to the prefix. If a node does not have any children then whatever prefix is created is appended to suggestions and then the next node in the layer is explores. If a node has a suffix that is true and has no children then the prefix is added to suggestions. when the heap gets empty then we return the suggestion list

### Your output

- Put the output you got for the prefixes provided here: li, b, glow

li: ![alt text](ucs1.png) ['lit', 'liter']
b: ![alt text](ucs2.png) ['bet', 'bruh', 'basic', 'bestie']
f: ![alt text](ucs3.png) ['fr', 'fam', 'facts', 'flex', 'fire', 'finna']

## Experimental
- BFS explores all nodes layer by layer. This method tends to generate shorter word completions first. DFS dives deep into each branch of the tree before backtracking and exploring other branches. This method can generate longer word completions earlier in the list. UCS prioritizes nodes based on their cost. This method tends to generate more common word completions first.
