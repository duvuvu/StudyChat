...
![TreeDiagram](https://github.com/<redacted>/assignment-2-search-complete-<redacted>/blob/main/TreeDiagram.png)

### Code analysis

- Put the intuition of your code here
- My intuition was if a word has a prefix already used by another word, I can skip past the prefix and then create a subtree starting there. When the prefix ends, or if there is no common prefix at all, I can create new nodes to represent the new letters. This function is also where I kept track of the parent of each node, its edge counter, and the letter assigned to it to help with the search functions. 
...
- Put the intuition of your code here
- Theoretically, the nodes in a BFS search are explored through the First-In-First-Out method, and thus I decided to keep track of the explored nodes using a Queue. Whenever an 'END' node makes it to the front of the queue, I know that I have found a word and can then traverse up the tree using my 'parent' property to get the complete word.
...
- Put the intuition of your code here
- The nodes in a DFS search are explored through the Last-In-First-Out method, and thus I decided to keep track of the explored nodes using a Python List (since it acts as a Stack as long as you always explore the last element of the list). Whenever an 'END' node is the final element in the list, I know that I have found a word and can then traverse up the tree using my 'parent' property to get the complete word.
...
- I decided to use the Stack-based DFS since it ensured that I would end up exploring every node in the tree, in the right order. Intuitively, this implementation seemed a lot more clear because I could not think of how, for a recursive algorithm, I could go back up the tree to the last node with an unexplored child (I had not come up with the idea of using a 'parent' property at that point).
...
- Put the intuition of your code here
- This algorithm made most sense to implement with the help of a Priority Queue. This is because the Priority Queue allowed me to choose what node to remove from the frontier based on the total cost from the root to that specific node. It also allowed for nodes to be automatically sorted, based on the total cost, when added to the frontier.
...
- BFS seemed to have the most noticeable difference, as it always generated words based on their lengths - in ascending order (shortest words first).
- The order of the DFS search grouped together all the words that have the same prefixes (for instance, thou, though, and thought, all start with thou).
- UCS does seem to follow a pattern of shortest-to-longest words, however, there are exceptions due to longer words that have more common prefixes.
