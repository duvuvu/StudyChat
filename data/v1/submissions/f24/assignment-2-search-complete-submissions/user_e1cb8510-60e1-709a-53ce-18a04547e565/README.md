...
- Put the tree diagram for `test.txt` here
    ...
    ![tree diagram](test.txt.jpg)

### Code analysis

- Put the intuition of your code here

This function splits the document into its words and then creates a node for each char of the word. When adding a new node, it checks if the node is already in the tree as a child node. When a word is completed, it changes the nodes end_word property to true to mark that it is a finished word. The function also keeps track of each prefix and updates the frequency of each prefix as it is seen.
...
The function first traverses the tree to find the node of the last letter of the prefix. It creates a queue to store the nodes as the tree is searched. It gets the next node by using the popleft() function to follow FIFO. If the current node is the end of a word, it adds it to the result. It then adds all the children of the node with the new prefix created from the next char. It repeats until the queue is empty.

### Your output

- Put the output you got for the prefixes provided here

output: the, thee, thou, that, thag, there, their, though, thought, through
...
The function first traverses the tree to get to the last letter of the prefix. It creates a queue to store the nodes to be searched. It gets the next node by pop() to follow LIFO. If the current node is the end of a word, it adds it to the result. It adds all of the children with the new prefix and repeats until the queue is empty
...
Recursive DFS expands recursively, exploring the left-most leaf until the bottom of the tree is reached. Stack-based DFS explores the right-most leaf, following the LIFO model until the bottom of the tree is reached. I used the stack-based DFS and explored the most recent nodes put in the queue first.
...
The function first traverses the tree to find the node of the last letter of the prefix. It creates a priority queue that contains the path cost of the prefix, the prefix, and the node itself. It pops the element with the lowest path cost. If that node is the end of a word, it adds the word to the results. It then adds all the child nodes to the queue with the new prefix and the path cost of the new prefix. It repeats until the priority queue is empty.

### Your output

- Put the output you got for the prefixes provided here

output: the, thou, though, thag, that, thee, their, there, thought, through

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 

The suggestions generated with BFS started with the smallest words first. The suggestions generated with DFS showed all the words starting with a certain prefix and moving down the tree instead of across. The suggestions generated with UCS first showed words that were the most common prefixes, i.e., "the" is shown first because "the" is a prefix for 4 out of the 10 words.
