...
### Tree diagram
![plot](./prefix_tree.png)

### Code analysis

Iterates over every word in the string `document`. For each word, traverse existing nodes in the tree starting at the root corresponding to the letters in that word. If the node for that letter does not exist, create a new node for it.
...
First, finds the last node in the prefix through a traversal helper function. Then, start a queue with this node. Loop over the queue until empty, removing the first node and adding all of its children to the end of the queue. If this node is the end of the word, add the word to the suggestions list.
...
Similar to BFS, find the last node in the prefix with the traversal helper function. Then, start a stack with this node. Loop over the stack until empty, removing the last node and adding all of its children. If this node is the end of the word, add the word to the suggestions list.
...
A recursive DFS immediately searches the first child that it discovers, since it immediately calls the DFS function on that child. A stack-based DFS will append the node's children to the end of the stack in order, meaning that the last child will be at the top of the stack. The result is that the children are processed backwards with a stack.
...
UCS follows a similar structure to BFS, except it uses a priority queue instead of a normal queue. The priority queue now stores two things, the node itself and the cost to get there. The priority queue sorts first by the total cost of the path, then the individual cost for that node.

In order to assign costs/weights to each node, the `build_tree` function needed modification. After assembling the tree, I do a recursive DFS from the root to count the number of complete words underneath each node. Then, I assign the weight for that node to be the inverse of that.
...
## Experimental
BFS always gave the suggestions in word length order.
The behavior for DFS is dependent on the order of the words in `genZ.txt`. Because of the dict data structure, nodes are processed in reverse order of appearance. For example, "through" is the first word since "r" is the last letter option to be discovered at depth 3.
Uniform cost search loosely prefers shorter words over longer words, since shorter words consist of less weights to add. However, it also considers the commonality of every prefix as well.
