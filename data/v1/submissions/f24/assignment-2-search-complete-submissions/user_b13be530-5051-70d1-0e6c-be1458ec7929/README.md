...
<img src = "https://github.com/<redacted>/assignment-2-search-complete-<redacted>/blob/main/test.txt%20tree%20diagram.jpg">

### Code analysis

- Put the intuition of your code here

Builds the trie by splitting the "document" into words and adding each word to the trie character by character, starting from "self.root". For each character, if it doesn't exist in the current node’s children, a new "Node" is created. The frequency of the next character is updated in "node.freq[next_char]" if there's a following character. After inserting the word, "node.is_end_of_word" is set to "True" to mark the end of the word in the trie.
...
Starts at "self.root" and traverses the trie using a "deque" named "queue", which stores the current node and prefix ("(node, prefix)"). For each node, it checks if "current_node.is_end_of_word" to add valid words to the "suggestions" list. It processes each child node by appending "(next_node, current_prefix + char)" to the queue, ensuring words are found level by level.
...
Starts from the node matching the last character of the prefix, using a stack named "stack" to hold "(node, prefix)" pairs. It pops "current_node" and "current_prefix" from the stack and checks if "current_node.is_end_of_word" to add words to "suggestions". For each child, it pushes "(next_node, current_prefix + char)" onto the stack, exploring the trie deeply before backtracking.
...
Uses a priority queue ("pq") with tuples "(cost, current_prefix, current_node)", starting from the node corresponding to the last character of the prefix. It pops the lowest-cost node from "pq" using "heapq.heappop", adding to "suggestions" if "current_node.is_end_of_word". For each child, it calculates the "new_cost" using "1 / current_node.freq[char]" (or 1 if no frequency), and pushes "(new_cost, current_prefix + char, next_node)" into the priority queue, ensuring frequent words are prioritized.
...
BFS gives priority to shorter, lexicographically closer words, DFS dives deep into the trie to find longer words first, and UCS balances word frequency, suggesting common words early on.
