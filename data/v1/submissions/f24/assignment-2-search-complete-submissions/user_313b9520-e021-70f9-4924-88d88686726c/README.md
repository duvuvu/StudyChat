...
Insert words from a provided document into a tree, and each character of a word is stored as a node. The last character is marked as the end of the word.
...
Traverses the tree from the node that correspond to the last character of prefix using BFS. Return the list of complete words ordered by word length.
### Your output
...
Traverse the tree from the node corresponding to the last character of the prefix using DFS.  Return the list of complete words.
...
Here I use stack-based DFS in suggest_dfs. Recursive DFS uses funtion calls to traverse the nodes in the deeper layers. Stack-based DFS uses a stack to explore nodes. Recursive DFS is more intuitive than stack-based DFS, but it may cause stack overflow. 
...
Traverse the tree from the node using UCS. Find the node that has lower path cost by calculating the character frequency in the node. Return the complete word suggestion.
...
prefix: th
output: the, thou, thee, thag, that, though, their, there, thought, through

## Experimental
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS. 

BFS returns the suggestions in order of increasing word length. DFS returns the longer words first, but not always. UCS prioritizes words with more common character sequences.
