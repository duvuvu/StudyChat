...
![alt text](https://github.com/<redacted>/assignment-2-search-complete-<redacted>/blob/main/IMG_0472.jpg?raw=true)

### Code analysis

- Put the intuition of your code here

Builds a tree structure by breaking each word in a document into characters and placing them as nodes. Starting from the root, it adds each character sequentially as a child node, ensuring that common prefixes share the same path in the tree.
...
For a given prefix, the algorithm uses a queue to traverse the tree from the node where the prefix ends, finding all possible word completions. It systematically explores all nodes at the current depth before moving to deeper levels, ensuring that every layer of the tree is checked for valid completions. This approach guarantees that all potential words with the specified prefix are considered.
...
For a given prefix, the algorithm uses a stack to find all possible word combinations by extending from the node where the prefix ends. It explores one branch at a time, starting from the rightmost child due to the last-in, first-out (LIFO) nature of the stack, and backtracks as needed to traverse each subtree. This ensures that all nodes are visited systematically, and valid word completions are added to the word bank as they are encountered.
...
Recursive DFS intuitively follows the natural structure of a tree by diving deep into each branch and using the call stack to backtrack, while stack-based DFS manually simulates this process using an explicit stack. I used the stack-based DFS here to avoid potential recursion depth limits and maintain better control over the traversal.
...
The UCS algorithm systematically explores all possible word completions starting from a given prefix by using a priority queue to select paths by the lower cumulative costs, reflecting the inverse frequency of the characters following the prefix. More frequently occuring character sequences are suggested first, providing a list of ordered autocomplete suggestions.
...
BFS generates shorter completions first, exploring all words of similar lengths before going deeper in the tree. DFS tends to favor longer completions by fully exploring one branch before backtracking, leading to a less predictable order. UCS prioritizes suggestions based on path costs, favoring frequent character sequences, resulting in contextually relevant suggestions. Each method’s approach significantly influences the variety and relevance of autocomplete suggestions.
