...
### Tree diagram
![IMG_CC1BCD8D1F68-1](https://github.com/user-attachments/assets/1ccf7a4a-bb07-45a2-925c-97d894e91acd)


### Code analysis

The build_tree function builds a node tree representation of words, iterating through each word in the inputted document and for each newly encountered character in a word, creating a node; the end of a word is also noted for certain nodes with the is_word property. Afterwards, a recursive helper method (calculate_frequencies) is used to traverse the subtree of each node and calculate edge costs by keeping track of how many words exist in the node's (prefix) subtree: 1 / number of words in subtree.
...
The suggest_bfs function utilizes breadth-first search (BFS) with a queue, first navigating to the last node of the given prefix, and then using BFS (popping nodes out of the queue in order of addition; exploring the closest nodes layer by layer) to find words in the subtree. When a node with the is_word property (value true) is found, the word is added to the list of suggestions.
...
The suggest_dfs function utilizes depth-first search (DFS) with recursion, first navigating to the last node of the given prefix, and then calling the recursive helper method (dfs_recursive), which uses DFS (recursively with the call stack; exploring as far as possible in a given subtree) to find words in the subtree. When a node with the is_word property (value true) is found, the word is added to the list of suggestions.

### Your output

['there', 'their', 'thee', 'the', 'thought', 'though', 'thou', 'that', 'thag', 'through']

### Recursive DFS vs Stack-based DFS

Recursive DFS was used in the suggest_dfs function mainly for the purpose of ease of tree traversal without need for maintaining and managing a stack (which has potential to be less concise in terms of lines of code). To me, recursion is more intuitive and it follows logically from the tree data structure. 

## `UCS`

### Code analysis

The suggest_ucs function utilizes uniform cost search (UCS) with a priority queue implemented with a heap (min), first navigating to the last node of the given prefix, and then using UCS to find words in the subtree via lowest cost paths (exploring least expensive paths). When a node with the is_word property (value true) is found, the word is added to the list of suggestions.
...
## Experimental
BFS suggested shorter words first before providing longer words, while DFS suggested words in a variety of lengths, including longer ones towards the beginning unlike BFS, however there was no "logic" behind suggestion order. UCS suggested words with more common sequences of letters, creating more relevant, purposeful output that differed from both BFS and DFS.
