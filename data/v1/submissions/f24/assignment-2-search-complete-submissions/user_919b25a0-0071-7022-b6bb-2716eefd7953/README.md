...
Yes
HEUIAHFKLDSJFKLJDSKLFJDSKLFJDKLSFJKDLSJFKLDSJFKLDSJFKLDSJ

## `build_tree`

### Tree diagram
- Put the tree diagram for `test.txt` here
![diagram] (https://github.com/<redacted>/assignment-2-search-complete-<redacted>/raw/main/IMG_1016.jpg)
...
build_tree Method:
This method constructs the prefix tree by splitting the input document into words, creating nodes for each character if they do not already exist, and incrementing the frequency for each character as it is added. The last character of each word is marked as the end of a word, allowing for efficient word retrieval during suggestions.
...
suggest_bfs Method:
The BFS method searches through the prefix tree using a queue to find all valid word completions for a given prefix, appending complete words to the suggestions list when reaching nodes marked as the end of a word. By exploring the tree level by level, it efficiently collects possible completions in a breadth-first manner.
...
suggest_dfs Method:
This method performs a depth-first search on the prefix tree by recursively exploring child nodes, accumulating valid word completions along the way. It leverages a helper function to build a list of suggestions for the specified prefix by visiting all descendant nodes of the starting point.

_dfs Helper Method:
The `_dfs` method recursively traverses the tree to find all words that can be formed from the given starting node, adding complete words to the suggestions when the end of a word is reached. This approach allows for a clean and maintainable way to explore the tree structure deeply.
...
I used the recursive DFS, instead of stack-based DFS. Basically, intuitively the recursive DFS runs DFS on each child node of the root, and continues down the descendants. This makes sense because of how recursive algorithms work-- they would deeply search one node before returning to the top and continuing, even if they happen "simultaneously." With a stack, it makes a lot of sense when doing it by hand on paper, since the stack allows you to basically keep track and run DFS on each child node just like the recursive one, but you would do it one at a time, and then continue down that path until you have nothing left, then continue on to the next child. (If there are any children of the children you basically always continue and backtrack.)
...
- Put the intuition of your code here
suggest_ucs Method:
Using the UCS algorithm, this method attempts to find word completions for a given prefix by prioritizing nodes based on their inverse frequency, allowing for more relevant suggestions. It maintains a priority queue that sorts potential completions by cost, ensuring that more frequently occurring characters are considered first during the search.
...
Interestingly it seems like the shortest word, the, was always first for all three algorithms. The longest word, through, was last for all three algorithms. This makes sense to me because of course the shortest word would end up returning as a proper word first, since we always reach 'e' first from 'th'.
With 'the' as the prefix, for example, we have the, there, their, thee, for DFS, and then the, thee, their, there for UCS, and the, thee, there, their for BFS. Interestingly DFS was the only one that went out of order for word length. But that makes sense based on the fact that both BFS and UCS go step by step instead of searching deep first. 
