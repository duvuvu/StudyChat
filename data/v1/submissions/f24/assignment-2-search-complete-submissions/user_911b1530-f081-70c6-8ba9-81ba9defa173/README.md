...
- Put the intuition of your code here:
    ...
    For each word in the document, the method starts at the root and iterates through its characters. If a character is missing from the dictionary node.children, a new node is created for it. The current node position is then updated to the newly created or existing node corresponding to that character.
...
- Put the intuition of your code here:
    ...
    The suggest_bfs method begins by setting the current node to the root and traversing the trie to find the node corresponding to the last character of the provided prefix. It then uses a queue to explore this node and another queue to keep track of suffixes, starting with an empty string. As it processes each node, if it reaches a node that marks the end of a word, it adds the complete word (prefix + suffix) to the suggestions list. It continues by iterating over the children of the current node in alphabetical order, adding each child node and the corresponding updated suffix to the queues for further exploration.
...
- Put the intuition of your code here:
    ...
    The suggest_dfs method starts by setting the current node to the root and traversing the trie to locate the node corresponding to the last character of the provided prefix. It then uses a stack to explore this node, initializing another stack to track suffixes, beginning with an empty string. As it processes each node, if it encounters a node that marks the end of a word, it adds the complete word (prefix + suffix) to the suggestions list. The method continues by traversing the children of the current node in reverse alphabetical order, pushing each child node and the corresponding updated suffix onto the stacks for further depth-first exploration.
...
- Explain your intuition in recursive DFS VS stack-based DFS, and which one you used here:
    ...
    In recursive DFS, the function calls itself for each child node, maintaining an implicit call stack that handles backtracking automatically. This approach is concise but can lead to stack overflow for deep trees due to recursion limits.

    In contrast, stack-based DFS uses an explicit stack to manage nodes, allowing for greater control over the traversal process. It avoids the risk of stack overflow and can be more efficient in terms of memory, especially for deep or wide trees.

    In the suggest_dfs method, I used a stack-based DFS. This approach explicitly manages the stack for nodes and suffixes, enabling controlled traversal through the trie without the limitations of recursion depth.
...
- Put the intuition of your code here:
    The suggest_ucs method begins by setting the current node to the root and traversing the trie to reach the node corresponding to the last character of the prefix. It then uses a priority queue to explore nodes based on cost, where the cost is inversely related to the frequency of each character. The method extracts the node with the lowest cost, and if the node marks the end of a word, it adds the complete word (prefix + suffix) to the suggestions list. It continues by iterating through the children of the current node in alphabetical order, pushing each child node into the priority queue with the updated cost, ensuring that frequently occurring characters are prioritized.
...
- Explain here what differences did you see in the suggestions generated when you used BFS vs DFS vs UCS:
    ...
    When using BFS, suggestions are generated in strict lexicographical order because it explores each level of the trie completely before moving deeper. DFS, on the other hand, explores deeper nodes first, leading to suggestions that might be lexicographically ordered within subtrees but not globally. UCS prioritizes suggestions based on character frequency, so more frequent words appear earlier, but not necessarily in lexicographical order, as it balances between frequency and order.
