...
- yes, but I accidentally asked all my questions under Assignment 1 instead of Assignment 2

## `build_tree`

### Tree diagram
- Put the tree diagram for `test.txt` here: ./383Tree.pdf

### Code analysis

- Iterated through each word, and for each word iterated through each letter. If the letter is already in the tree, continue, if not, created a new node with the new letter. If node is the end of the word, set node's end attribute to true.
...
- Iterate to find the last letter of the prefix. Store each children and prefix into a queue of tuples, if the node's end attribute is true, add the tuple's word to suggestions. Repeat until queue is empty, then return suggestions. 
...
- I used stack-based DFS. Recursive DFS will have the same intuition as described above, with the only difference being instead of the last child added to be the first explored, recursive will just explore the first child it finds, potentially giving less control over the order of nodes explored. 
...
- Iterate to find the last letter of the prefix. Store tuples that include each children, its path cost from the root, and its prefix, into a heap/priority queue, if the node's end attribute is true, append the word to suggestions. Repeatedly pop from heap until heap is empty, then return suggestions.
...
## Experimental
- BFS suggested the shorter words first; DFS suggested longer words first/prefixes in groups (thr-, then tha-, tho-, th- etc.); UCS suggested words with similar spellings from shortest to longest and from more frequent use to less frequent use.  
