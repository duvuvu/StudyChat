...
![tree](https://github.com/user-attachments/assets/8e628c45-1510-49c2-85c6-7bedfb01d015)### Code analysis

- Put the intuition of your code here
Builds a tree by setting a node to a root and creating a new node if that character doesn't exist and looping through while there is another chracter in the word until setting a node to the end of word as true indicating that the node is the end of one of the words in the document then it does that for every word in the document constructing the tree.
### Your output
...
- Put the intuition of your code here
The code traverses to the node to find the last character on the inputted prefix and returns an empty array if the prefix can't be found. Stores all the possible suggestions in an array and goes through the bfs function while the queue isn't empty and if the node is marked as end of word adds it to the suggestion array then returns that array.
...
- Put the intuition of your code here
The code traverses to the node to find the last character on the inputted prefix and returns an empty array if the prefix can't be found. Stores all the possible suggestions in an array and implements the bfs algorithm by setting the stack to the root node and going through dfs and appending when a node is end of word until the stack is empty then returns the suggestion array.
...
While you can do recursive or stack-based I opted to do stack-based as I believe it is easier to comprehend the code adn follow along with the algorithms process in comparison to recursive.
...
- Put the intuition of your code here
The code traverses to the node to find the last character on the inputted prefix and returns an empty array if the prefix can't be found. Stores all the possible suggestions in an array and makes a priority queue to implement ucs storing words and costs. If the node is end of a word it's pushed onto the queue and has it's children looked at and whenever an end of word node is seen it's stored onto the queue and the suggestion array is in order starting with the most cost effective.
### Your output
...
The bfs had words ordered similar to their word length, the dfs suggestions were more grouped together with other words that started with the same letters, while ucs was ordered similarly to bfs by order length but ordered from the most cost effective to least.
