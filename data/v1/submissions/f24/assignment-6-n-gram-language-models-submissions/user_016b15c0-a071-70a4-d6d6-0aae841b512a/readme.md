...
- Created a list containing $n+1$ frequency tables represented as dictionaries with the $n$-gram as key and its corresponding frequency as value
- At index $i$ of the list is the frequency table for $i$-grams
- At index 0 is an edge case, where we have as key the empty sting and as value the size of the corpus (i.e. len(document))
...
- The formula used for sequence likelihood using the $n$-gram model, where our sequence (denoted as the vector $\vec{x}$) is of length at most $n$ was: 
    -   $P(\vec{x}) = P(x_1)P(x_2 \mid x_1)P(x_3 \mid x_1, x_2) \cdots P(x_n \mid x_{n-1}, \ldots, x_1)$
- We can use the following formula to evaluate conditional probabilities: $P(x_n \mid x_{n-1}, \ldots, x_{1}) = \frac{f(x_1x_2\cdots x_n)}{f(x_1x_2\cdots x_{n-1})}$ where $f$ is a function that returns the frequency of a specific word sequence. Since `calculate_probability` requires us to calculate a conditional probability, __this is formula used in the code__.

### Code analysis

- Using the formula above, we calculate the conditional probability of $P(\text{char} \mid \text{sequence})$ by obtaining the frequency of the sequence concatenated with the character and the frequency of the sequence. To ensure that the length of $\text{sequence + char}$ is at most $n$, we slice $\text{sequence}$ to be length $n-1$. If we divide by zero, we return a probability of 0, as this would imply the given sequence does not occur.
...
- The code will loop through every character in the vocabulary and record the character with the highest conditional probability given the previous sequence. After finding the character with the highest probability given the sequence, we return it.
...
- My answer: After experimenting with the "Alice's Adventures in Wonderland" corpus and "War and Peace" corpus, it was clear that "Alice's Adventures in Wonderland" worked much faster. The time it took for "Alice's Adventures in Wonderland" to work with $n=500$ was approximately the same as the time it took "War and Peace" to work with $n=30$. This is clearly due to the vast difference in size of each corpus. However, both worked at about the same. Testing with different sequences in both texts, it auto-completed with phrases that did appear in each text and chose the character with the highest frequency after the given sequence (I verified this using `ctrl + f` on each text document). So the $n$-gram model is effective at determining common pre-existing phrases given the beginning of such a phrase.
