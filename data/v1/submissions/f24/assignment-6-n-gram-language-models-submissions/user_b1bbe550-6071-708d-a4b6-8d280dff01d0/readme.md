...
- The function returns an array of n dictionaries, where each dictionary stores the n-gram mapped to its frequency in the corpus. Loop through the corpus, keep track of how many times a sequence has been seen, starting with unigram mapping to n-gram mapping.
...
- ***Write the formula for sequence likelihood as described in section 2***:
   - $$P(x_{t} \mid x_{1...t-1}) = \frac{f(x_{1})}{size(C)} \cdot \frac{f(x_{1}, x_{2})}{f(x_{1})}\cdot ... \cdot \frac{f(x_{1}, x_{2},...,x_{t-1},x_{t})}{f(x_{1}, x_{2},...,x_{t-1})}$$

### Code analysis

- The probability that a given character comes after a sepcific sequence will be the product of all the probabilities of previous sequences multiplied by the probability of the sequence + character over the probability of the sequence like in Bayes. In our implementation, this would translate to the product of the frequency of each next character added on to the sequence given the previous sequence, multipled by the given sequence + character over the frequency of the sequence in the corpus.
...
1. $P(X_3=a \mid X_1=a, X_2=a )$
   - $$P(a)P(a \mid a)P(a \mid aa) \newline = \frac{f(a)}{size(C)} \cdot \frac{f(a, a)}{f(a)} \cdot \frac{f(a, a, a)}{f(a, a)} \newline = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = \frac{1}{20}$$
2. $P(X_3=b \mid X_1=a, X_2=a)$
   - $$P(a)P(a \mid a)P(b \mid aa) \newline = \frac{f(a)}{size(C)} \cdot \frac{f(a, a)}{f(a)} \cdot \frac{f(a, a, b)}{f(a, a)} \newline = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = \frac{1}{10}$$
3. $P(X_3=c \mid X_1=a, X_2=a)$
   - $$P(a)P(a \mid a)P(c \mid aa) \newline = \frac{f(a)}{size(C)} \cdot \frac{f(a, a)}{f(a)} \cdot \frac{f(a, a, c)}{f(a, a)} \newline = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = \frac{1}{20}$$
...
- Returns the most likely character that follows after a given sequence. Loops through each character in the vocabulary and applies previous function, calculate_probability, while keeping track of the highest probability and its associated character.
...
- When  working with War and Peace, n = 10 table calculation becomes too time consuming , but with Alice, n = 10 response is generated much more quickly. I tested with the string 'assembled ' and passed in k = 10. I wonder if this has to do with the vocabulary used in a children's book versus Tolstoy's War and Peace? Perhaps the language is more diverse in War and Peace than Alice.
<hr>
