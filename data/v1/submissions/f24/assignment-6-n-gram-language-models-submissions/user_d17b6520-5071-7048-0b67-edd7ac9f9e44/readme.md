...
- ***Put the intuition of your code here***
    Since we need to create n tables, my initial thought was to iterate till we reach n tables in the list.
    The code to generate our tables are identical and differ only by the number of characters we use in the sequence.
    Thus, I use a helper function that will take the document and obtain the lines and chars to generate each frequency table. Since the index starts at 0 for python, the first table with n = 1 is stored at index 0--I keep this in mind for my other functions.
...
- ***Put the intuition of your code here***
My intuition for the function calculate_probability was to follow the joint probability formula given in the read me.
Where $P(X_1=x_1, X_2=x_2, X_3=x_3) = P(X_1=x_1) * P( X_2=x_2 \mid X_1=x_1) * P(X_3=x_3 \mid X_1=x_1, X_2=x_2)$.
Following this formula we can plug in the conditional probabilities using the estimation formula given: $P(X_3=x_3 \mid X_1=x_1, X_2=x_2) = \frac{f(X_1=x_1, X_2=x_2, X_3=x_3)}{f(X_1=x_1, X_2=x_2)}$.
If we plug in all the frequencies into the first equation, they all cancel each other out, and we are left with the frequency of the target divided by the number of characters from the text file (referenced as size(C) in the readme).

With this knowledge, I turn the given sequence into a tuple and use it to obtain the value from the table at the index of the length of the target - 1. Then I add up all the values in the first table (n = 1) to obtain the number of characters from the text file which was seen in the first function (but we do not have access to anymore).
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
    $= P(a) * P(a \mid a) * P(a \mid a, a)$
    $= (11 / 20) * (5 / 10) * (1 / 4)$
    $= .06875$

2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
    $= P(a) * P(a \mid a) * P(b \mid a, a)$
    $= (11 / 20) * (5 / 10) * (2 / 4)$
    $= .1375$
...
- ***Put the intuition of your code here***
    Given a set of chars that could be potential characters that could appear next in the sequence, we need to identify the probability of each char. My intuition was to iterate through the set and compare the probability of the current character to the current maximum probability. I continue to compare the values till the last element and the character with the highest value is returned.
...
Using this command: print(predict_next_char('a', create_frequency_tables("Alice's Adventures in Wonderland.txt", 10), ('t', 'l', 'n')))
I tested the n value 20 at first which only took a second. As I incremented by 20, it slowly took longer and longer. It wasn't until n = 100 that the table calculation became too time consuming (even n = 80 took a few seconds). With more tables to generate it took longer to compute--though the term "longer" would be different in terms of one's personal preferences.

But in terms of making the correct prediction, no matter what n value was used the correct prediction was given. Though this could be due to the fact that my probability function was coded to find the frequency of the sequence divided by the number of characters in the text.

In terms of the two different files, Alice's Adventure in Wonderland was much faster than war and peace. When n was set to 3, war and peace still ran within one second or two. But the amount I could increment with war and peace was much smaller than Alice's Adventure in Wonderland which I was able to increment by 20 and not notice an extreme change in time.

With war and peace, incrementing n to 10 took just as long as when n = 80 in Alice's Adventure in Wonderland. Though I didn't measure the exact times, incrementing war and peace to 20 by far took longer than incrementing Alice's Adventure in Wonderland to n = 100. This could have been due to war and peace containing more text, which can significantly increase the time it takes to generate all the tables.
