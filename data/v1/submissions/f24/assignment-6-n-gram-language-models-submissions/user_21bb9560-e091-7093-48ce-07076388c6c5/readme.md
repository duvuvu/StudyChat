...
- It uses nested `defaultdict` structures to handle missing keys automatically.
- Iterates through the document to extract sequences of increasing lengths (`k`) and the character that follows them.
- Populates frequency counts for each sequence in its respective table (`tables[k-1]` for k-grams).
- The result is a set of frequency tables for 1-gram, 2-gram, ..., n-gram.
...
1. ***What is your vocabulary in this case***
   - ${aab, aba, bab, abc, bca, cac, acc, cca, caa, aaa, aac, acb, cba, baa}$
...
### Formula
For \(n = 3\), the formula for computing the joint probability is as follows:
$P(X_1=x_1, X_2=x_2, X_3=x_3, X_4=x_4) = P(x_1) \cdot P(x_2 \mid x_1) \cdot P(x_3 \mid x_1, x_2) \cdot P(x_4 \mid x_2, x_3)$

Using frequency tables, this expands to:
$P(X_1=x_1, X_2=x_2, X_3=x_3, X_4=x_4) = \frac{f(x_1)}{\text{size}(C)} \cdot \frac{f(x_1, x_2)}{f(x_1)} \cdot \frac{f(x_1, x_2, x_3)}{f(x_1, x_2)} \cdot \frac{f(x_2, x_3, x_4)}{f(x_2, x_3)}$

### Code analysis

- Trims the input sequence to match the maximum n-gram size if necessary.
- Checks the tables in descending order of specificity, starting with the longest matching n-gram and falling back to shorter ones.
- Adds a small constant (`+0.01`) to both the numerator (specific character frequency) and denominator (total frequency for the sequence) to ensure robustness.
- If no match is found, it assigns a low uniform probability to prevent issues.
...
$P(X_1=a, X_2=a, X_3=a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = \frac{1}{20}$
...
$P(X_1=a, X_2=a, X_3=b) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = \frac{1}{10}$
...
$P(X_1=a, X_2=a, X_3=c) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = \frac{1}{20}$
...
- Calls `calculate_probability` for each character in the vocabulary to determine its likelihood of occurring next.
- Collects these probabilities in a dictionary.
- Selects the character with the highest probability using `max`.
...
## Experiment
- The two corpus files differ significantly in size, with one containing about **66,000 lines** and the other about **1,700 lines**. This disparity affects the performance and feasibility of calculating n-gram probabilities. For smaller values of `n` (e.g., `n = 2` or `n = 3`), both corpora perform well, as the computational cost and table size remain manageable.
- For **warandpeace**, calculations are manageable up to `n = 15`, taking approximately **50 seconds**, but they terminate for `n = 20` due to the exponential growth of n-grams. In contrast, the smaller **Alice's Adventures in Wonderland** corpus handles much larger `n` values efficiently. For example, `n = 128` takes about **18 seconds**, while `n = 200` completes in **30 seconds**. However, the calculation terminated at `n = 230. 
- My experimenting results showed that the smaller corpus is better suited for exploring higher `n` values due to reduced computational overhead. However, the larger corpus is more effective for meaningful n-gram probabilities at lower `n` values, as it mitigates data sparsity issues. 
