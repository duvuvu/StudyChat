...
No, the website was not working when I was attempting to use it while doing the assignment

## `create_frequency_tables(document, n)`

### Code analysis

The function create_frequency_tables(document, n) takes a training document, in this case either "Alice in wonderland" or "War and Peace", then generates a n-gram frequency tables ranging from 1 to n. This function helps us to track how often an n-gram appears in a document, and allows us to predict future probabilities.
...
### Formula
$P(X_{n+1} \mid X_1, X_2, \ldots, X_n) = \frac{C(X_1, X_2, \ldots, X_n, X_{n+1}) + 1}{C(X_1, X_2, \ldots, X_n) + |V|}$


Where: 

$C(X_1, X_2, \ldots, X_n, X_{n+1})$ represents the count of the (n+1)-gram sequence.

$C(X_1, X_2, \ldots, X_n)$ represents the count of the n-gram sequence.

And $|V|$ represents the size of the vocabulary.

### Code analysis

The function calculate_probability(sequence, char, tables) calculates the conditional probability of the next character in the sequence "next_char" given the previous sequence, "sequence". Function uses the frequency tables created in create_frequency_tables(document, n). The function also uses laplace smoothing in order to ensure every character has a non-zero value for probability.
...
1. $P(X_3=a \mid X_1=a, X_2=a )$
   - $P(X_3=a \mid X_1=a, X_2=a ) = \frac{C(aaa) + 1}{C(aa) + |V|}$
   - $P(X_3=a \mid X_1=a, X_2=a ) = \frac{2 + 1}{3 + 3} = \frac{1}{2}$
2. $P(X_3=b \mid X_1=a, X_2=a)$
   - $P(X_3=b \mid X_1=a, X_2=a) = \frac{C(aab) + 1}{C(aa) + |V|}$
   - $P(X_3=b \mid X_1=a, X_2=a) = \frac{1 + 1}{3 + 3} = \frac{1}{3}$
3. $P(X_3=c \mid X_1=a, X_2=a)$
   - $P(X_3=c \mid X_1=a, X_2=a) = \frac{C(aac) + 1}{C(aa) +|V|}$
   - $P(X_3=c \mid X_1=a, X_2=a) = \frac{0 + 1}{3 + 3} = \frac{1}{6}$
...
The function predict_next_char(sequence, tables, vocabulary) chooses the next character in the sequence based off of the calculated probabilities in calculate_probability. The function iterates over the whole vocabulary and calculates the probability of each character given the current sequence. The character with the highest probability is then chosen.
...
Based off of the probability distribution calculated out above, the most probable character after "aa" would be "a" with a probability of 1/2.
...
We have the ability to choose between two training text files, Alice in Wonderland and War and Peace. Based on my extensive experimentation, I have found that the Alice in Wonderland file typically returns more varying sequences of text. Once the n value reaches a value of around 400, the table calculation becomes very time consuming. for example, when i ran n=400, sequence=alice, k=1200, the script took about 2 minutes total to run. this number seems to increase drastically as well around n=500.  
