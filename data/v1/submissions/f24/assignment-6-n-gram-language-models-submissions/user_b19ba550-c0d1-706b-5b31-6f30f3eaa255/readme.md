...
- ***First, I created an outer loop ranging from 1 to (n + 1), so that I can map each of the frequencies of the n-grams found to the corresponding n-frequency table. Within this loop, I create a freqTable to store the frequencies for n, then construct each n-gram from the document and store it in a hashmap. If it is already in the hashmap, then its counter is incremented by 1, otherwise the n-gram is initialized as a key with value 1.***
...
2. ***Write down your probability table 3***:
    - You got this!
        ...
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $aab = P(b \mid aa)$ | $\frac{2}{5}$ |
        | $aba = P(a \mid ab)$ | $\frac{1}{3}$ |
        | $bab = P(b \mid ba)$ | $\frac{1}{2}$ |
        | $abc = P(c \mid ab)$ | $\frac{2}{3}$ |
        | $bca = P(a \mid bc)$ | $\frac{2}{2}$ |
        | $cac = P(c \mid ca)$ | $\frac{1}{3}$ |
        | $acc = P(c \mid ac)$ | $\frac{1}{2}$ |
        | $cca = P(a \mid cc)$ | $\frac{1}{1}$ |
        | $caa = P(a \mid ca)$ | $\frac{2}{3}$ |
        | $aaa = P(a \mid aa)$ | $\frac{1}{5}$ |
        | $aac = P(c \mid aa)$ | $\frac{1}{5}$ |
        | $acb = P(b \mid ac)$ | $\frac{1}{2}$ |
        | $cba = P(a \mid cb)$ | $\frac{1}{1}$ |
        | $baa = P(a \mid ba)$ | $\frac{1}{2}$ |
...
- ***Write the formula for sequence likelihood as described in section 2***
    ...
    - For example say that sequence consists of letters $X_1, X_2, ... , X_n$
    - $P(char | sequence) = P(X_1) * P(X_2 | X_1) * ... * P(X_n | X_1, X_2, ... X_(n-1)) * P(char | X_1, ... , X_n)

### Code analysis

- ***Initially, the function sets up a variable `seqProb` to accumulate the probabilities of each character in the sequence. It then iterates through each character in the input `sequence`, getting the relevant context of preceding characters limited by the max n-gram size. For each position in the sequence, it forms an n-gram that consists of this context and the current character, checking that the length of this n-gram does not exceed the available frequency tables. By retrieving the joint frequency of the n-gram and the frequency of the context from the respective tables, the function calculates the conditional probability for each character in the sequence. If the context frequency is zero, it appropriately sets the probability to zero to avoid undefined behavior. The overall sequence probability is updated only when the individual probabilities are non-zero, since if there is a 0, the end probability will be 0 regardless.***
- ***After calculating the total probability for the sequence, the function evaluates the probability of `char` occurring right after the sequence using similar principles. It constructs an n-gram combining the last characters of the sequence and `char`, again retrieving the necessary frequencies from the tables to compute \( P(char | sequence) \).***
...
2. $P(X_1=a, X_2=a, X_3=b)$
   ...
   - $\frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = \frac{2}{20}$
...
- ***The only possible next characters are one of those which are included in the vocabulary. So I have variables to keep track of the max_prob and best_char, and iterate through each of the characters in the vocabulary and call the calculateProbability function on each. If the probability of a character is greater than the max, then it gets saved into the max_prob and best_char variables. At the end, the character with the greatest probability is outputted.***
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Based on the probability distribution obtained above the most likely character next in the sequence after "aa" would be "b". So the most likely completed sequence would be "aab".*
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
    ...
    - I've found that since each of the corpus files have vastly varying levels of vocabulary and grammar along with very different styles. The Alice's Adventures in Wonderland text tends to use more informal, shorter language, so the frequency tables are filled up more with those. Therefore, the higher values of n tended to have odd behavior when using this text. The War and Peace novel is nearly 50x larger than that text, has more complex language. This corpus worked far better than the other, however even with this, excessively large values of n or initial sequences. 
    - What I noticed is that when a large initial sequence is inputted along with a larger value of n, it fails to calculate the probability of the next character since the sequence never existed in the training corpus. This leads to it outputting the same character over and over again since it cannot properly find the most probable character. Also I found that if the value of n is approximately greater than 35, the table calculation takes >1 min which is significantly more time consuming than the lower n values.
<hr> 
