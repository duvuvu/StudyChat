...
It initializes a list of `Counter` objects to store the frequencies for each n-gram size, where each `Counter` automatically handles counting of sequences. The function iterates through the document using nested loops; the outer loop tracks the starting position, while the inner loop extracts n-grams of increasing lengths from 1 to `n`, ensuring that they remain within the document's bounds. For each n-gram, its count is updated in the respective `Counter`. Finally, the function returns the list of frequency tables, which can be utilized for calculating probabilities and making predictions 
...
1. ***What is your vocabulary in this case***
   The vocabulary in this case is {a,b,c}
...
- P(x1, x2, ..., xn) = [f(x1) / Total Unigrams] * [f(x1, x2) / f(x1)] * [f(x1, x2, x3) / f(x1, x2)] * ... * [f(x1, x2, ..., xn) / f(x1, x2, ..., xn-1)]
...
The function starts by checking if the input sequence is empty, in which case it calculates the probability of just the character based on its unigram frequency.
For a non-empty sequence, it initializes a probability value and calculates the unigram probability for the first character. Then, it iteratively assesses each subsequent character in the sequence, slicing the sequence into manageable parts (subsequences) to retrieve their frequencies from the provided tables.
It computes the conditional probability for each character based on its preceding context, multiplying these probabilities. Finally, it evaluates the probability of the specified character following the entire sequence. If any counts are missing during the calculations, the function returns zero to signify that the observed sequence is unlikely based on the model. 
...
- P(X1 = a) = 11/20 (from Table 1)
- P(X2 = a | X1 = a) = 5/11 (from Table 2)
- P(X3 = a | X2 = a, X1 = a) = 1/5 (from Table 3)

Now, multiplying these together:

P(X1 = a, X2 = a, X3 = a) = P(X1 = a) * P(X2 = a | X1 = a) * P(X3 = a | X2 = a, X1 = a)

Substitute the values:

P(X1 = a, X2 = a, X3 = a) = (11/20) * (5/11) * (1/5) = 1/20
...
- P(X1 = a) = 11/20 (from Table 1)
- P(X2 = a | X1 = a) = 5/11 (from Table 2)
- P(X3 = b | X2 = a, X1 = a) = 2/5 (from Table 3)

Now, multiplying these together:

P(X1 = a, X2 = a, X3 = b) = (11/20) * (5/11) * (2/5) = 2/20 = 1/10
...
- P(X1 = a) = 11/20 (from Table 1)
- P(X2 = a | X1 = a) = 5/11 (from Table 2)
- P(X3 = c | X2 = a, X1 = a) = 1/5 (from Table 3)

Now, multiplying these together:

P(X1 = a, X2 = a, X3 = c) = (11/20) * (5/11) * (1/5) = 1/20

Summary of Results:

- P(X1 = a, X2 = a, X3 = a) = 1/20
- P(X1 = a, X2 = a, X3 = b) = 1/10
- P(X1 = a, X2 = a, X3 = c) = 1/20
...
The function starts by iterating through each possible character in the provided vocabulary. For each character, it calculates the conditional probability of that character appearing after the current sequence using the `calculate_probability` function. This probability reflects how frequently the character has followed the sequence in the training data.
After computing the probabilities for all characters, the function sorts them in descending order so that the character with the highest likelihood appears first. Finally, it returns the character with the maximum probability as the predicted next character. If the vocabulary is empty, the function handles this by returning an empty string.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Based on the calculations that we did, the next character in the sequence should be 'b'. It has the highest probability compared to the other characters in the vocabulary.
...
I experimented with values of n from 1 to 10 using "Alice's Adventures in Wonderland" and "War and Peace" as training texts. For smaller values of n (like 1 or 2), the model didn’t have enough context, so its predictions were often not very accurate and very generalized. As n increased to 3 or 4, the predictions became much better, as the model could use more information from previous characters. With larger values, especially between 5 and 10, the larger text ("War and Peace") provided good context, which made predictions accurate. 

However, the time and memory needed to build and use the tables increased significantly. By the time n reached 10, the tables were extremely large, and computation became very slow. Overall, n = 3 to n = 5 worked well for balancing prediction quality and computational effort, while values beyond n = 5 became too slow.
