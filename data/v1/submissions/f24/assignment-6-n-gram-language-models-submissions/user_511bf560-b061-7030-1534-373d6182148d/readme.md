...
The function generates frequency tables for an n-gram model based on character occurrences in a given text document. It initializes a list of frequency tables using a list comprehension, where each table corresponds to a different level of dependency. It iterates through each character in the document, along with preceding characters 'min(n, document_length)'. It checks whether there are enough characters available before the current position to form a valid n-gram.
...
<img width="345" alt="Screenshot 2024-11-20 at 6 30 59 PM" src="https://github.com/user-attachments/assets/fe11eb30-7fea-4b25-b46f-ea020ee43b39">


2. ***Write down your probability table 3***:
<img width="416" alt="Screenshot 2024-11-20 at 6 31 07 PM" src="https://github.com/user-attachments/assets/a29d86eb-dc16-43a6-99bf-5333cffaf2d0">
...
- $P(X_1 = x_1, X_2 = x_2,...,X_n = x_n) = P(X_1 = x_1)\cdot \prod_{i=2}^{n} P(X_i = x_i | X_1 = x_1, ...., X_{i-1} = x_{i-1})$

### Code analysis

- ***Put the intuition of your code here***
This function computes the probability of observing a specific character 'char' following a given sequence of characters 'sequence'. If the sequence is empty, it defaults to the unigram case (where character occurrences are considered independently of context). 

For non-empty sequences, the function identifies the appropriate frequency table based on the length of the context provided by the sequence. It then extracts the last characters of the sequence (context) and computes the total occurrences of all characters for that context.  It then retrieves the count of the specified character given that context. It returns the probability as the ratio of the character count for the specific context to the total count of characters observable in that context.
### Your Calculations
...
1. $P(X_3=a \mid X_1=a, X_2=a )$
   - *Show your work*
   - ![IMG_3391](https://github.com/user-attachments/assets/9f9678c9-0eb9-4976-9f2d-5feaf0400307)

2. $P(X_3=b \mid X_1=a, X_2=a)$
   - *Show your work*
   - ![IMG_3392](https://github.com/user-attachments/assets/c10880f7-ded6-4ff1-abae-e72304dc36be)

3. $P(X_3=c \mid X_1=a, X_2=a)$
   - *Show your work*
   - ![IMG_3394](https://github.com/user-attachments/assets/f5af182e-3b7d-4247-88f0-93ba880be241)
...
- ***Put the intuition of your code here***
The function first extracts last `n-1` characters, relevant for prediction. It iterates through each character in the provided vocabulary and calculates the probability of that character following the context using the `calculate_probability` function (results in a dictionary of probabilities mapping each character to its likelihood of being the next character).

Next, it identifies and returns the character with the highest computed probability using the `max` function, which retrieves the key from the probabilities dictionary that has the maximum value.
...
I found that lower values of n, particularly n = 3, often yielded the most efficient performance in terms of both speed and predictive accuracy. With n greater than 8, the time needed significantly increased.
<hr>
