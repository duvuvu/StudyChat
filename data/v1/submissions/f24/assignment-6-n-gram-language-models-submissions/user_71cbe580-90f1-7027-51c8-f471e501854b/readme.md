...
## 383GPT
Did you use 383GPT at all for this assignment (yes/no)? No, I did not.

## `create_frequency_tables(document, n)`

### Code analysis

- I implemented the frequency tables as a list of Counters, which are a specialized form of dictionaries with a default value of zero. I iterated through the characters in the document and for each character, iterated through all the possible sequence lengths (1-n), identifying each substring and incrementing the count in the frequency table for each.
...
1. ***What is your vocabulary in this case***
   - Our vocabulary is the set containing the characters a, b, and c. 
...
### Formula
- $P(X_1 = x_1, X_2 = x_2, ... X_k = x_k | X_1 = x_1, X_2 = x_2, ... X_{k-1} = x_{k-1}) = \frac{f(x_{(k-n+1)}...x_k)}{f(x_{(k-n+1)}...x_{k-1})}$

### Code analysis

- Since most of the terms end up cancelling out once we use our Bayesian probability formula, we just need to divide the frequency of the last n-1 characters of the sequence with the character added on to the end by the frequency of the last n-1 characters of the sequence by itself. I made a helper function, seq_freq, to help me navigate the frequency tables more easily so that with any sequence it would give me the frequency of the last n characters or of the entire sequence if it was short enough. Then I just called that helper function with sequence+char and sequence and divided.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1 = a, X_2 = a, X_3 = a) = P(X_3 = a \mid X_1 = a, X_2 = a) \cdot P(X_1 = a, X_2 = a) = P(X_3 = a \mid X_1 = a, X_2 = a) \cdot P(X_2 = a \mid X_1 = a) \cdot P(X_1 = a) = \frac{1}{5} \cdot \frac{5}{11} \cdot \frac{11}{20} = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_1 = a, X_2 = a, X_3 = b) = P(X_3 = b \mid X_1 = a, X_2 = a) \cdot P(X_1 = a, X_2 = a) = P(X_3 = b \mid X_1 = a, X_2 = a) \cdot P(X_2 = a \mid X_1 = a) \cdot P(X_1 = a) = \frac{2}{5} \cdot \frac{5}{11} \cdot \frac{11}{20} = \frac{2}{20} = \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_1 = a, X_2 = a, X_3 = c) = P(X_3 = c \mid X_1 = a, X_2 = a) \cdot P(X_1 = a, X_2 = a) = P(X_3 = c \mid X_1 = a, X_2 = a) \cdot P(X_2 = a \mid X_1 = a) \cdot P(X_1 = a) = \frac{1}{5} \cdot \frac{5}{11} \cdot \frac{11}{20} = \frac{1}{20}$
...
- I set a maximum likelihood of any subsequent character to be 0 to start. Then I iterated through the vocabulary once, updating the maximum probability and storing the character whenever I found a new highest likelihood next character. At the end, I returned the character that had given that highest probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Since $P(b \mid aa)$ is higher than $P(a \mid aa)$ and $P(c \mid aa)$, the most likely next character should be b. Indeed, when we look at the original sequence, we see that aab occurs twice as often as the others.
...
- The War and Peace corpus took far longer since the book is so much longer than Alice in Wonderland, and so creating the frequency tables was much more tedious. Increasing the value of n too much can make it very inefficient, since the storage for the frequency tables increases by a factor of the vocabulary size every time n goes up by one, and the time increases as well. I found that increasing to n=20 made it take an unreasonably long amount of time for War and Peace and essentially just quote one specific section of War and Peace or Alice in Wonderland since there aren't that many times that 20 characters repeat in the same order throughout a book.  However, too short values of n like n=2 or n=4 often generated nonsense, either where the sequence wasn't actually words or where it just repeated some common word like "the" many times.
