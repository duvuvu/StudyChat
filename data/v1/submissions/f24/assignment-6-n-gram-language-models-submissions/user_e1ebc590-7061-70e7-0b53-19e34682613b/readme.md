...
The overall objective is to count the occurences for every unique sequence, and the data structure that does this best is a dictionary. For every sequence length i, I loop through the document and count repetitions of each unique sequence of length i as they show up. I keep a dictionary for each table and return them all in the list `tables`.

Using a defaultdict gives an advantage when both writing and reading to the dict by making all calls to non-existent entries default to 0.
...
Let $m$ be the length of the sequence. Let $f$ notate the frequency of a sequence. The formula to get the true probability:

$$P( x_1,\cdots,x_i) = \prod_{i=1}^m\frac{f(x_1, \cdots, x_i)}{f(x_1, \cdots, x_{i-1})}$$

The practical n-gram formula with a non-infinite $n$:
$$P( x_1,\cdots,x_i) = \prod_{i=1}^m\frac{f(x_{\max(1, i-n)}, \cdots, x_i)}{f(x_{\max(1, i-n)}, \cdots, x_{i-1})}$$

We essentially chain rule as far back as we can access instead of chain ruling all the way to the start.

### Code analysis

The general idea is to calculate the product from the n-gram formula using the sliding window technique. There is a left and a right pointer, and each step will calculate two frequencies:
1) for the subsequence bounded by both pointers (numerator)
2) for the subsequence bounded by the left pointer and the right pointer - 1 (denominator).

The pointers are updated such that they traverse over the entire sequence and the length of the subsequence never exceeds $n$.

If the subsequence is a single character, the denominator is the document length since this particular term is written as $P(x_1) = \frac{f(x_1)}{\text{num chars}}$.

If any denominator is $0$, the probability of finding the overall sequence is $0$ because every denominator is a subsequence of the overall sequence.
...
<!-- 1. $P(X_1=a, X_2=a, X_3=a) = P(X_1=a) \frac{P(X_1=a, X_2=a)}{P(X_1=a)} \frac{P(X_1=a, X_2=a, X_3=a)}{P(X_1=a, X_2=a)}$ -->
1. $P(X_1=a, X_2=a, X_3=a) = P(X_1=a) P(X_2=a | X_1=a) P(X_3=a|X_1=a, X_2=a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5}=\frac{1}{20}$


2. $P(X_1=a, X_2=a, X_3=b) = P(X_1=a) P(X_2=a | X_1=a) P(X_3=b|X_1=a, X_2=a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5}=\frac{2}{20}$
3. $P(X_1=a, X_2=a, X_3=c) = P(X_1=a) P(X_2=a | X_1=a) P(X_3=c|X_1=a, X_2=a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5}=\frac{1}{20}$
   - All probabilities are in the table
...
The for loop calculates the probability for all possible next characters given the sequence and find which one is most likely. The array picks a random default character beforehand in case the sequence never appears in the corpus.
...
Choose $b$, since it has the highest probability of $\frac{2}{20}$.
...
warandpeace.txt was much more time consuming for any $n$ than Alices' Adventures in Wonderland.txt for all $n$. This is likely because it is 66 times longer. Increasing $n$ greatly increased the amount of time spent making tables for warandpeace. It took around a minute at $n=50$. With Alice, I went up to $n=1000$ in order to take a comperable amount of time generating.
<hr>
