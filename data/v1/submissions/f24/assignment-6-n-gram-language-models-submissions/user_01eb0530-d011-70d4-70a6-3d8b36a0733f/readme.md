...
- ***In essence the overall intuition of the code is as follows. The create_frequency_tables function constructs n frequency tables, returning them as a list. Each of the table records the counts/frequencies of unigrams, bigrams, ..., n-grams present in a document. By capturing the frequencies of these sequences, calculation of sequence probabilities is made easier. As for implementation, create_frequency_tables iterates from 1 to n, identifying all of the sequences less than or equal to n and incrementing the corresponding count in the frequency table being constructed.***
...
2. ***Write down your probability table 3***:
   - You got this!
        ...
        | $P(a \mid b, b)$ | $0$ | bb is not a sequence in the document
        | $P(b \mid b, b)$ | $0$ | bb is not a sequence in the document
        | $P(c \mid b, b)$ | $0$ | bb is not a sequence in the document
...
General formula: $$P(X_1=x_1, X_2=x_2, X_3=x_3) = P(x_1) \cdot P(x_1 \mid x_2) \cdot P(x_3 \mid x_1, x_2) = \frac{f(x_1)}{size(C)} \cdot \frac{f(x_1,x_2)}{f(x_1)} \cdot \frac{f(x_1,x_2,x_3)}{f(x_1,x_2)}$$

Formula for this problem (where x_3 could be a, b, or c): $$P(X_1=a, X_2=a, X_3=x_3) = P(a) \cdot P(a \mid a) \cdot P(x_3 \mid a, a) = \frac{f(a)}{size(C)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,x_3)}{f(a,a)}$$

### Code analysis

- ***The calculate_probability function utilizes the frequency values found in the frequency tables produced by create_frequency_tables to carry out the probability calculations. The main functionality of calculate_probability is as follows, though there are other measures in place to handle "edge" cases. The probabilities are computed by extracting the counts from the frequency tables and performing necessary multiplication and division per the discussed probability formulas.***
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $$P(X_1=a, X_2=a, X_3=a) = P(a) \cdot P(a \mid a) \cdot P(a \mid a, a) = \frac{f(a)}{size(C)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,a)}{f(a,a)} = \frac{11}{20} \cdot \frac{1}{2} \cdot \frac{1}{4} = \frac{11}{160} = 0.06875$$ 
2. $P(X_1=a, X_2=a, X_3=b)$
   - $$P(X_1=a, X_2=a, X_3=b) = P(a) \cdot P(a \mid a) \cdot P(b \mid a, a) = \frac{f(a)}{size(C)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,b)}{f(a,a)} = \frac{11}{20} \cdot \frac{1}{2} \cdot \frac{1}{2} = \frac{11}{80} = 0.1375$$ 
3. $P(X_1=a, X_2=a, X_3=c)$
   - $$P(X_1=a, X_2=a, X_3=c) = P(a) \cdot P(a \mid a) \cdot P(c \mid a, a) = \frac{f(a)}{size(C)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,c)}{f(a,a)} = \frac{11}{20} \cdot \frac{1}{2} \cdot \frac{1}{4} = \frac{11}{160} = 0.06875$$
...
- ***The predict_next_char function utilizes the probabilities calculated in create_frequency_tables, identifying the most likely next character given a particular sequence. predict_next_char employs a loop to find a maximum/highest probability value, which corresponds with the most likely next character.***
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Using joint probabilities, the next character in the sequence would be b because "aab" is the most probable sequence. Using conditional probability, b is also the most likely next character.
...
Overall, it seemed that "warandpeace.txt" and "Alice's Adventures in Wonderland.txt" worked better due to their larger size than a smaller corpus such as the sequence "aababcaccaaacbaabcaac." With a larger corpus, there are more probabilities to work with and more complex relationship, which can be captured and in turn predicted. Higher n values seemed to yield more accurate predictions, giving coherent and logical phrases from a larger corpus. However, larger n values came with the cost of added time for computation; it was slower to output a prediction. On the other hand, with a small corpus, such a short sequence of letters, predictions turned inaccurate even with a larger n value if larger predictions (sequences longer in length than the corpus itself) were made. As for how high a value of n can be run before the table calculation becomes too time consuming, it is important to note that this question is subjective. In the modern world, prediction should be nearly instantaneous (milliseconds) in order to be implemented in practical applications. With n = 10 on "warandpeace.txt", it takes several seconds to produce output. For n = 20, it takes nearly 30 seconds. This time cost only grows for larger values of n. Realistically, n = 2, 3, and 4 are more practical in this context.
