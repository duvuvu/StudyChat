...
In the  of a language model, each node in the network corresponds to a character in the sequence, and edges represent the conditional dependencies between them.
...
In our  we are interested in computing the next character (the character at position 4) given the characters at the previous positions (characters at position 1, 2, and 3). Applying the definition of conditional distributions we can see this is
...
My create frequency tables take in the document, and n then starts by creating empty objects for each n-gram table. Then we loop through the document for each char i, and from that char, we get each ngram of lengths 1 to n. If we don't already have it in our n-gram table, we create the object and make it zero, then after no matter what we add 1 because we've encountered it in the document. This will go through the document and for every i, get the grams up to length n, then move to the next position. Then it will return the frequency tables of objects with their counts.
...
1. ***What is your vocabulary in this ***
   - a, b, and c
...
The calculate probability function will take in the variables and then check if the sequence is greater than the length of our tables, meaning we don't have an n-gram table to calculate it, and if so returns 0. If the length is 0, it means that it's only a single character so we just have to divide the frequency of a character by the total count of all chars. If it is between 0 and len(sequence), then we simply get the frequency of the original sequence as our denominator, and then the sequence with the new char as our numerator and return it.
...
Substituting values from our frequency tables:

From the unigram table:
$P(X_1 = a) = P(a) = 0.55$

From the bigram table:
$P(X_2 = a \mid X_1 = a) = P(a \mid a) = \frac{5}{11} \approx 0.4545$

For the conditional probability:
$P(X_3 = a \mid X_1 = a, X_2 = a) = P(a \mid a,a) = \frac{1}{5} = 0.2$

Now, we can compute the overall probability:
$P(X_1 = a, X_2 = a, X_3 = a) = 0.55 \cdot 0.4545 \cdot 0.2$

Calculating this:
$P(X_1 = a, X_2 = a, X_3 = a) \approx 0.55 \cdot 0.4545 \cdot 0.2 \approx 0.050$

Thus, the probability $P(X_1 = a, X_2 = a, X_3 = a)$ is approximately $0.050$ .
...
Now for the conditional probability of the bigram:
   $P(X_3 = b \mid X_1 = a, X_2 = a) = P(b \mid a,a) = \frac{2}{5} = 0.4$

Now, we can compute the overall probability:
    $P(X_1 = a, X_2 = a, X_3 = b) = 0.55 \cdot 0.4545 \cdot 0.4$

Calculating this:
    $P(X_1 = a, X_2 = a, X_3 = b) \approx 0.55 \cdot 0.4545 \cdot 0.4 \approx 0.100$

Thus, the probability $P(X_1 = a, X_2 = a, X_3 = b)$ is approximately $0.100$.
...
For the conditional probability:
$P(X_3 = c \mid X_1 = a, X_2 = a) = P(c \mid a,a) = \frac{1}{5} = 0.2$

Now, we can compute the overall probability:
$P(X_1 = a, X_2 = a, X_3 = c) = 0.55 \cdot 0.4545 \cdot 0.2$

Calculating this:
$P(X_1 = a, X_2 = a, X_3 = c) \approx 0.55 \cdot 0.4545 \cdot 0.2 \approx 0.050$
Thus, the probability $P(X_1 = a, X_2 = a, X_3 = c)$ is approximately $0.050$.


## `predict_next_char(sequence, tables, vocabulary)`

### Code analysis
The predict char uses the calculate probability function to find the highest value probability for the next char given our sequence. We simply iterate over all possible chars in our vocab, test their probability, and if it is higher than our max we update max and char. If there is none we just return ''.
...
The best character for any given sequence would be the one with the highest probability that we encounter. Given these probabilities above, the next character predicted in the sequence  "aa" is "b" because 0.2 is the greatest probability.
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
    ...
    In my experiments with the provided corpus files using varying values of n, I observed that the corpus for Alice's Adventures in Wonderland performed significantly better than War and Peace. This can be attributed to the shorter length of Alice's Adventures in Wonderland, which results in a more manageable dataset that can be processed quickly. Conversely, the larger size of the War and Peace text caused the program to become increasingly time-consuming when n exceeded approximately 10 or 15. For instance, when I attempted to run the model with n = 25 on the *War and Peace* text, it took about 3 minutes to complete the calculations. This indicates that, as n increases, the complexity and time required for n-gram table calculations grow significantly, leading to exponentially larger tables as well as longer processing times. Consequently, shorter corpora yield more efficient performance, allowing for quicker predictions and better usability of the model.

<hr>


Please don't hesitate to reach out to us in  of any questions (no question is dumb), and come meet us during office hours XD!
Happy coding!
