...
- The intuition of my code for this function was that a dictionary is the best way to store a 'table'. Moreover, I can use a double for-loop where each outer-loop takes care of the work for each table (a total of n tables), and the inner loop finds all the sequences in the document of length m, where m is the current outer-loop iteration. As we loop through the document and find sequences, each time we come across a sequence that we haven't seen before it can be added in the respective table with a frequency of 1. On the other hand, if the sequence has been seen before, we can increment the key(frequency) for that particular sequence in the current table. Finally, since we must return a list of tables, before resetting each table in the outer-loop, we can just append it to self-created list, so we'll return a list of dictionaries.
...
- $\frac{f(x_1, x_2, ...., x_n)}{size(C)} \cdot \prod_{i=1}^{m-n} \frac{f(x_{i+1}, x_{i+2}, ...., x_{i+n})}{f(x_{i+1}, x_{i+2}, ...., x_{i+n-1})}$

### Code analysis

- We know that $P(char \mid sequence) = \frac{P(sequence +  char)}{P(sequence)}$. Expanding out the probability calculations and actually doing them by hand made me realize that a lot of the terms in the numerator and denominator cancel out. The only term that does not cancel out is the last term in the numerator as it does not have anything in common with P(sequence). This last term is the $P(char \mid$ last n-1 characters in sequence). We can calculate this using the tables we have since we have n tables. For the code, we have a base case for when the sequence length is 0, because then we must divide by the length of the document itself. Then for the main cases, we can handle the probability calculations described above for when n is greater than or equal to the sequence length, and separately for when it is less than. This is because the calculations change a little when we don't have access to the tables for larger length strings, due to the implementation and logic of N-gram models.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(a \mid aa) = P(aaa)/P(aa)$
   - $P(aaa) = P(a) * P(a \mid a) * P(a \mid aa)$
   - 11/20 x 5/11 x 1/5
   - 0.05
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(b \mid aa) = P(aab)/P(aa)$
   - $P(aab) = P(a) * P(a \mid a) * P(b \mid aa)$
   - 11/20 x 5/11 x 2/5
   - 0.10
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(c \mid aa) = P(aac)/P(aa)$
   - $P(aac) = P(a) * P(a \mid a) * P(c \mid aa)$
   - 11/20 x 5/11 x 1/5
   - 0.05
...
- Having implemented the last function, this one is now quite straightforward. We know that to predict the next char, we have to take the max probability value (which is calculated in the last function) for each character in the vocabulary. Whichever char corresponds to the largest probability, is the next char for our sequence. So for the code, we just need a loop that calls on calculate_probability for each char in the vocabulary.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - $P(a \mid aababcaccaaacbaabcaa) => P(a \mid aa) = 1/5$
  - $P(b \mid aababcaccaaacbaabcaa) => P(b \mid aa) = 2/5$
  - $P(c \mid aababcaccaaacbaabcaa) => P(c \mid aa) = 1/5$
  - Therefore, 'b' would be next in the sequence
...
Answer: After a thorough experimentation process (which I must say, was quite fun: this project was great and very practical), it seems that the 'War and Peace' corpus works better than 'Alice in Wonderland'. I believe this is because War and Peace is much longer and thus there is more data to work with. For War and Peace, n = 30 is where the code execution started to take quite long for me (considering my laptop is almost 9 years old and on its death bed). As for Alice in Wonderland, even values like n = 300 could get executed in under a minute.
I was mostly experimenting with War and Peace, and found that n = 15-25 was ideal as it outputted some pretty great results and paragraphs/sentences that made a good amount of sense. Values under n = 10 outputted text that had quite a bit of repetition.
Moreover, inputting sequences that don't exist in the document resulted in different results since we had to set the probability to 0 for them.

I had a lot of fun with the experimentation process and am even going to now input some kaggle datasets/texts that have lyrics of some of my favorite artists to then try and generate lyrics on my own. Similarly, I have found datasets for tweets by famous celebrities.
