...
For this I was not familiar with python a ton so I had to do a lot of looking up how to make frequency tables and things and ended up using a dictionary. Then after consulting gpt it suggested splitting the document into a more readable format. After that what I had to do was I split it into two different methods, one for if n = 1 because it had to be formatted differently since it would be the probability of just the letter and have nothing following after it. Then the other ways I needed the prefix which would be used as the sequence later and the next char which would end up being used for the char later. There I had it so it was able to adjust based on the given size n both in the length of the prefixes and of the overall list
...
$P(\odot)$ = $P(A)$ x $P(B|A)$ x $P(C|B,A)$

### Code analysis

- ***Put the intuition of your code here***

For this part of the code i first intialized a bunch of variables, then I used the idea that the probability was just the number of times one of the chars occured after a certain seqeuence over the total number of times the sequence appeared (which would have to be the number of times each char appeared after the sequence which would be just the overall times they appeared) Then I just calculated that based on the variables given. I also had to retroactively fix the fact that I had to make sure not to divide by 0
### Your Calculations
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   5/20 x 5/11 x 1/5 = 25/1100 = 1/44 = 0.023
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*
   5/20 x 5/11 x 2/5 = 50/1100 = 1/22 = 0.045
...
- ***Put the intuition of your code here***
For this all I did was calculate the probability for each character in the vocabulary based on the given sequence, comparing it to each other to save whichever ended up being the greatest value overall. I then just retruned that value that I ended up recieving 
...
After experimenting enough, I realized that I made a mistake and was getting completely off answers by having the next char be i + k instead of i + k - 1, after fixing this I found a much better result with real written answers as my responses. Overall the Alice text works really well after testing it and I found somewhere in the roughly 180-200 range is when my program will start terminating for taking too long and time consuming. Overall I get pretty intersting and understandable sentences no matter what I was using as a source. I started by switch to Alice because I was more familiar and it was completing simple words I know appear a lot like finishing alice, wonderland and chesire. I then moved on to larger n values and k values and was able to get readable and understandable answers
