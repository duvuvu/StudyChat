...
* The function `create_frequency_tables` builds a list of frequency tables that count how often sequences of characters appear in a document (n-grams). The way this function achieves this is by first going through the document and removing unwanted characters. It then counts sequences of 1 characters, 2 characters, until n characters. By building these tables, it will help the model understand specific patterns in the document.
...
1. ***What is your vocabulary in this case***
   - The vocabulary in this case would be {a,b,c}.
...
- Lets assume the character sequence is: S = s1,s2,s3,...,sn. The formula used would be:
- $P(S) = P(s_1) × P(s_2 | s_1) × P(s_3 | s_1, s_2) × … × P(s_n | s_n-2, s_n-1)$


### Code analysis

- ***Put the intuition of your code here***
* The function `calculate_probability` finds how likely a character is to appear after a specific sequence of characters. It checks how often the sequence plus the character appears in the frequency tables and divides that by how often the sequence alone appears. If the sequence isn’t in the table, it returns zero.
...
1. $P(X_1=a, X_2=a, X_3=a )$
   1. Count of `aaa` =1.
   2. Total occurrences after `aa` = 4.
   * $P(X_1=a, X_2=a, X_3=a )$ = 1/4 = `0.25`.

2. $P(X_1=a, X_2=a, X_3=b)$
   1. Count of `aab` = 2.
   2. Total occurrences after `aa` = 4.
   * $P(X_1=a, X_2=a, X_3=b)$ = 2/4 = `0.5`.

3. $P(X_1=a, X_2=a, X_3=c)$
    1. Count of `aac` = 1.
    2. Total occurrences after `aa` = 4.
    * $P(X_1=a, X_2=a, X_3=c)$ = 1/4 = `0.25`.
...
- ***Put the intuition of your code here***
* The function `predict_next_char` identifies the most likely next character in a sequence by calculating probabilities for each character in the vocabulary using the function `calculate_probability`. It finds the character with the highest probability based on the context of the last characters in the sequence. If no character has a high probability, then the function will return an empty string.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - Based on the previous answers we can see that the next character that has the highest probability is `b` with a score of `0.5`
...
1. n-grams (n): 5
2. Initial Sequence: "the"
3. Length of completiton (k): 50

I initially tested with the parameters above. In the case of the text file `warandpeace.txt` the results are pretty poor and the outputs just seem to be repeating the same two words over and over again (`the`, `said`). In the case of the text file `Alice's Adventures in Wonderland.txt` the results are really good. From this we can see that the `Alice's Adventures in Wonderland.txt` seems to be a better fit for my code with the initial parameters.

I then tested it with the parameters:
1. n-grams (n): 20
2. Initial Sequence: "the"
3. Length of completiton (k): 70

In this case, both text documents produced good results. I could see that in the `warandpeace.txt` only when the n-grams (n) was increased to 15 or more, would it produce results that could be considered good. However, when running the `warandpeace.txt` file with these parameters it would take a lot more time to run than the `Alice's Adventures in Wonderland.txt`. 

When I used the parameters: 
1. n-grams (n): 50
2. Initial Sequence: "the"
3. Length of completiton (k): 700

In this case I felt like it took too long for the `warandtext.txt` file. It took almost 4 minutes (When I timed it). On the contrary the file `Alice's Adventures in Wonderland.txt` took around 5-10 seconds to complete this. Both files still produced exceptional results.
