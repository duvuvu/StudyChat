...
- ***Put the intuition of your code here***
    - First, create a list, then with a for loop from 1 to n+1, we start creating an n-gram table by creating a dictionary. We then use another for loop to iterate through the document string, to find each n-gram, and either add it to the n-gram table or increase its frequency in the table if the specific n-gram already exists. After iterating through the string, the dictionary is appended to the list of tables, which is then returned at the end of the function.
...
- ***Write the formula for sequence likelihood as described in section 2***
    - $P(X_1=a, X_2=a, X_3=a) = P(a) \cdot P(a \mid a) \cdot P(a \mid a, a) = \frac{f(a)}{size(doc)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,a)}{f(a,a)}$
    - $P(X_1=a, X_2=a, X_3=b) = P(a) \cdot P(a \mid a) \cdot P(b \mid a, a) = \frac{f(a)}{size(doc)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,b)}{f(a,a)}$
    - $P(X_1=a, X_2=a, X_3=c) = P(a) \cdot P(a \mid a) \cdot P(c \mid a, a) = \frac{f(a)}{size(doc)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,c)}{f(a,a)}$

### Code analysis

- ***Put the intuition of your code here***
    - First, we check to make sure that the sequence is less than or equal to n-1. If it isn't, we only consider the last n-1 characters of the sequence. Then, we find the respective n-gram table using the length of the considered part of the sequence plus one for char. If the considered sequence + char is in that table, that new sequence's frequency is our numerator (If not, then we return 0). Then, we add up the frequencies of each individual character from the 1-gram to find our denominator, then return (numerator/denominator).
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*: $P(X_1=a, X_2=a, X_3=a) = \frac{f(a)}{size(doc)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,a)}{f(a,a)} = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*: $P(X_1=a, X_2=a, X_3=b) = \frac{f(a)}{size(doc)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,b)}{f(a,a)} = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work*: $P(X_1=a, X_2=a, X_3=c) = \frac{f(a)}{size(doc)} \cdot \frac{f(a,a)}{f(a)} \cdot \frac{f(a,a,c)}{f(a,a)} = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = \frac{1}{20}$
...
- ***Put the intuition of your code here***
    - This function uses a for loop to iterate through the vocabulary, calling calculate_probability with each iteration to find the character with the highest probability of coming next in the sequence. If there is a tie between two probabilities, it will randomly pick one of the two characters, and if the probability of every possible character is 0, then it will randomly choose a character from the vocabulary.
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
    ...
    - "Alice's Adventures in Wonderland.txt" seems to run much faster than "warandpeace.txt", as when running Alice's Adventures with n=10, it completed all of the table calculations within a few seconds, but running war and peace with n=10, it took around 30-45 seconds to complete all of the table calculations. Even with a much larger n value, such as n=100, Alice's Adventures still completed all of the table calculations in less time than it took for war and peace to complete with n=10.
