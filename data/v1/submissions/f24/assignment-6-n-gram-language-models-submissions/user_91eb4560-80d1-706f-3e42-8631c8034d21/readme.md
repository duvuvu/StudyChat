...
- ***this create_frequency_tables function iterates through 'document' to count the occurrences of n-grams. Here, each n0gram represents a sequencey of 'n' characters, which are stored in the frequency table for n values from 1 to n. ***
...
1. ***What is your vocabulary in this case***
   - All the unique combinations, so Vocaubulary = {a, b, c}
...
In this:
- \( f(x_1, x_2, \dots, x_t) \) is the frequency of the n-gram \( (x_1, x_2, \dots, x_t) \).
- \( f(x_1, x_2, \dots, x_{t-1}) \) is the frequency of the \((t-1)\)-gram \( (x_1, x_2, \dots, x_{t-1}) \).


### Code analysis

- ***This function gets the probability of 'char' following a sequence using the relevant frequency tables. It uses the formula above to caclulate probabilities from the frequency list.
***
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - *Show your work*
   \[
P(aaa) = P(X_3 = a \mid X_1 = a, X_2 = a) \cdot P(X_1 = a, X_2 = a)
\]
\[
P(a \mid aa) = \frac{2}{4} = 0.5
\]
For \( P(X_1 = a, X_2 = a) \)
\[
P(X_2 = a \mid X_1 = a) = \frac{4}{11}
\]
Thus:
\[
P(aaa) = 0.5 \times \frac{4}{11} = \frac{2}{11}
\]
...
2. $P(X_1=a, X_2=a, X_3=b)$
   ...
   \[
P(aab) = P(X_3 = b \mid X_1 = a, X_2 = a) \cdot P(X_1 = a, X_2 = a)
\]
\[
P(b \mid aa) = \frac{1}{4} = 0.25
\]
...
Hence:
\[
P(aab) = 0.25 \times \frac{4}{11} = \frac{1}{11}
\]

3. $P(X_1=a, X_2=a, X_3=c)$
 
\[
P(aac) = P(X_3 = c \mid X_1 = a, X_2 = a) \cdot P(X_1 = a, X_2 = a)
\]
...
Hence:
\[
P(aac) = 0.25 \times \frac{4}{11} = \frac{1}{11}
\]
...
- ***The function iterates over the possible characters in vocabulary and uses the calculate_probability function to compute the probability of each character being the next character in the sequence. For this, it uses the highest probability as the next predicted charcater. Essentiallyto usse the n-gram probability tables to find the likeliest next character based on  conditional probabilities.
***
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *- \(P(X_3 = a \mid X_1 = a, X_2 = a) = 0.1818\)
- \(P(X_3 = b \mid X_1 = a, X_2 = a) = 0.0909\)
- \(P(X_3 = c \mid X_1 = a, X_2 = a) = 0.0909\), hence 'a' would be the next character*
...
When experimenting with the "Alice's Adventures in Wonderland" corpus, I found that the model performed well even with higher values of \(n\), such as \(n=4\) and \(n=5\). Due to the smaller size of the corpus, the computation time remained better for all tested values of \(n\). The predictions improved with higher values of \(n\), as there was more context was considered in the n-grams. The time taken for calculations increased  as \(n\) grew. This corpus allowed for efficient table creation and prediction, even when using larger n-grams, and computational performance was not a bottleneck at \(n=5\).
