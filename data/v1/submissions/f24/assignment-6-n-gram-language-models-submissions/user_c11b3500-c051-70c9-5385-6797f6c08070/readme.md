...
## 383GPT
Did you use 383GPT at all for this assignment (yes/no)? YES absolutely

## `create_frequency_tables(document, n)`

### Code analysis

- For each value of n passed into the function, a countVectorizer is used to discover all the different n-grams in the corpus and their respective frequencies (of the current i of n). This list is then transformed to an array table of frequencies, frequency_table. The n-gram labels are then extracted into the variable, ngram_labels. Finally the dictionary object for that value of n, is created using the ngram_labels and frequency_table. This is appended to the master table and returned once all values of n-grams have been calculated.
...
1. ***What is your vocabulary in this case***
   - {aab, aba, bab, abc, bca, cac, acc, cca, caa, aaa, aac, acb, cba, baa, aab} 
...
### Formula
- $P(w_1, w-2, ... w_m) = Multiplicative product for i from 1 to m of P(w_i | w_{i+m-1}, ... w_{i-1})$

### Code analysis

- I used the chain rule. Calculate each n-gram probability from 0 to n-1 grams of the sequence with the appended character, and multiply them together to get the final probability of observing that char as the next in the sequence. Since terms like the frequency of the previous n-1 gram, are reused, I just keep updating variables in each loop iteration so I didn't have to search the dictionary more than I needed to. 
...
1. $P(X_1=a, X_2=a, X_3=a )$
   - 1/20 = P(a)*P(a|a)*P(a|aa) = (11/20)*(5/11)*(1/5)
2. $P(X_1=a, X_2=a, X_3=b)$
   - 1/10 = P(a)*P(a|a)*P(b|aa) = (11/20)*(5/11)*(2/5)
3. $P(X_1=a, X_2=a, X_3=c)$
   - 1/20 = P(a)*P(a|a)*P(c|aa) = (11/20)*(5/11)*(1/5) 
...
- Initally set the max probability to -inf and then iterate through each char in vocab, updating max when there is a new highest probability. With each char in vocab, call calculate_probability to get it's probability of being the next character. At the end return the character associate with the highest probability. 
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - The next character would be 'b', since its probability is 2/5 which is greater than the probabilty for a or c. 
...
After experimenting with the provided corpus files using varying values of n, I observed notable differences in performance and efficiency across the datasets. Some corpora, particularly those with a rich and diverse vocabulary, managed to yield insightful results even with higher values of n, allowing for more comprehensive analyses of word combinations. Conversely, corpora with limited vocabulary or simpler structures tended to show diminishing returns as n increased, leading to less informative tables. In terms of computational efficiency, I found that values of n above 5 often resulted in significant increases in processing time, particularly for larger corpora. Ultimately, while higher values of n can enhance the depth of analysis, they also demand more computational resources, underscoring the need for a balanced approach based on the specific characteristics of each corpus.
