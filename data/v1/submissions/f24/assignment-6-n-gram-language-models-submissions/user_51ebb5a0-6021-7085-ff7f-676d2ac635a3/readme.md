...
First, I created an empty dictionary to store the n frequency tables. I iterate through the document and keep track of 1 to n letters that come after the starting point to correspond it to which n-gram frequency table to look at. If the table already has the sequence, then it adds another instance. If it doesn't, it initializes the sequence to 1. I use a tuple instead of a string to make the sequence we are working with more clear and to easily access the next letter.
...
- ***Write the formula for sequence likelihood as described in section 2***
    ...
    $P(X_1=x_1, X_2=x_2, X_3=x_3, X_4=x_4) = P(x_1) \times P(x_1 \mid x_2) \times P(x_3 \mid x_1, x_2) \times P(x_4 \mid x_1, x_2, x_3)$

### Code analysis

- ***Put the intuition of your code here***

    Given a sequence and a character, we add the character to the end of the sequence and see if it exists in our tables. If the sequence was longer than n, we would have to shift over to just look at the n - 1 last letters. If it does exist, then we add one more instance. If it doesn't we initialize the sequence to 1 in the table. Each number in the table represents how many times the sequence appears. The numerator will be how many times the sequence and the character appear and the denominator is how many times just the sequence appears
...
1. $P(X_1=a, X_2=a, X_3=a)$
   ...
   $P(X_1=a, X_2=a, X_3=a) = P(X_1) \times P(X_2 \mid X_1) \times P(X_3 \mid X_1, X_2) = \frac{11}{20}\times\frac{5}{11}\times\frac{1}{5} = \frac {1}{20}$


2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work*

   $P(X_1=a, X_2=a, X_3=b) = P(X_1) \times P(X_2 \mid X_1) \times P(X_3 \mid X_1, X_2) = \frac{11}{20}\times\frac{5}{11}\times\frac{2}{5} = \frac {1}{10}$


3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* 

    $P(X_1=a, X_2=a, X_3=c) = P(X_1) \times P(X_2 \mid X_1) \times P(X_3 \mid X_1, X_2) = \frac{11}{20}\times\frac{5}{19}\times\frac{1}{5} = \frac {1}{20}$
...
- ***Put the intuition of your code here***
    ...
    For each letter in the vocabulary, it calculates the probability of it showing up. It then takes the highest probable character and returns it. If the n value is equal to 1, it just chooses the most common character, that being " ", or an empty space. I'm not sure if I was supposed to keep all of the characters, those including spaces and special characters, but the assignment is a sentence completer, so it made sense that I would need all of those characters.
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
    ...
    I tried a n of 25, and that took a long time to run. Granted, I am impatient, so objectively it may have not been super long, but it felt like 5+ minutes of it running. Different n values could have very different results. If n was 1, then it would just choose the most common character, that being " ", or an empty space. The more n values, the better the results would be. It seems like the warandpiece.txt corpus gave better responses.
