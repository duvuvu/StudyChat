...
def main():
    ...
    '''
    char = input("Enter the next character to predict probability for: ")
    probability = calculate_probability(list(initial_sequence), char, tables)
    print(f"Probability of '{char}' given the sequence '{initial_sequence}': {probability}")
    '''
    vocabulary = set([char for (char,) in tables[0]])
