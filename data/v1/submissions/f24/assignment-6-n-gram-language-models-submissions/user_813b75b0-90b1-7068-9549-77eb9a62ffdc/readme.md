...
- We first create a list to hold all of the frequency tables, and then for each frequency table $k=1$ until $k=n$ we have to add frequencies for subsequences of size $k$. This effectively means iterating through the given document $n$ times, each time increasing the subsequence window step size by 1.
...
2. ***Write down your probability table 3***:
   - You got this!
        ...
        | $P(a \mid a, c)$ | $\frac{0}{3}=0$ |
        | $P(a \mid b, a)$ | $\frac{1}{3}$ |
        | $P(a \mid b, b)$ | $\frac{0}{0}=0$ |
        | $P(a \mid b, c)$ | $\frac{2}{1}$ |
        | $P(a \mid c, a)$ | $\frac{2}{2}$ |
        | $P(a \mid c, b)$ | $\frac{1}{2}$ |
        | $P(a \mid c, c)$ | $\frac{1}{1}$ |
        | $P(b \mid a, a)$ | $\frac{2}{5}$ |
        | $P(b \mid a, b)$ | $\frac{0}{2}=0$ |
        | $P(b \mid a, c)$ | $\frac{1}{3}$ |
        | $P(b \mid b, a)$ | $\frac{1}{3}$ |
        | $P(b \mid b, b)$ | $\frac{0}{0}=0$ |
        | $P(b \mid b, c)$ | $\frac{0}{1}=0$ |
        | $P(b \mid c, a)$ | $\frac{0}{2}=0$ |
        | $P(b \mid c, b)$ | $\frac{0}{2}=0$ |
        | $P(b \mid c, c)$ | $\frac{0}{1}=0$ |
        | $P(c \mid a, a)$ | $\frac{1}{5}$ |
        | $P(c \mid a, b)$ | $\frac{2}{2}$ |
        | $P(c \mid a, c)$ | $\frac{1}{3}$ |
        | $P(c \mid b, a)$ | $\frac{0}{3}=0$ |
        | $P(c \mid b, b)$ | $\frac{0}{0}=0$ |
        | $P(c \mid b, c)$ | $\frac{0}{1}=0$ |
        | $P(c \mid c, a)$ | $\frac{1}{2}$ |
        | $P(c \mid c, b)$ | $\frac{0}{2}=0$ |
        | $P(c \mid c, c)$ | $\frac{0}{1}=0$ |
...
### Formula
Where $n > 1$ (excluding unigram):
- $P(x_1,...,x_s) = P(x_1) * \prod_{i=2}^{n-1} P(x_i \mid x_1, ..., x_{i-1}) * \prod^{s}_{i=n} P(x_i \mid x_{i-(n-1)},...,x_{i-1})
    \\ = \frac{f(x_1)}{size(C)} * \prod_{i=2}^{n-1} \frac{f(x_1,...,x_i)}{f(x_1, ...,x_{i-1})} * \prod^{s}_{i=n} \frac{f(x_{i-(n-1)},...,x_i)}{f(x_{i-(n-1)},...,x_{i-1})}$

### Code analysis

- To calculate probability of the character following the sequence, we concatenate the character to the sequence and then determine our $n$-gram model (by getting the length of `tables`). Then, we get other values we need to compute the formula such as the frequency of the first character and the size of the collection. Then, if the model is unigram ($n=1$) we call a helper that implements such formula ($\prod_{1}^{len(full\_sequence)} P(x_i)$). Otherwise, we carry on computing the probability of the `sequence + char` using our $n$-gram formula (stated/defined above).
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $= P(X_1) * P(X_2 \mid X_1) * P(x_3 \mid X_{1},X_{2})$
   - $= P(a) * P(a \mid a) * P(a \mid a, a)$
   - $= \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{55}{1100} = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $= P(a) * P(a \mid a) * P(b \mid a, a)$
   - $= \frac{11}{20} * \frac{5}{11} * \frac{2}{5} = \frac{110}{1100} = \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $= P(a) * P(a \mid a) * P(c \mid a, a)$
   - $= \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{55}{1100} = \frac{1}{20}$
...
- For all the characters in our vocabulary, we compute the probability of that character following the given sequence, keeping a running maximum probability stored in a variable, as well as the corresponding max probability character. This max probability character is then returned after looking at the entire vocabulary. If there is no maximum probability character (i.e. `best_char == None` at the end of iteration), then we return a random character from the vocabulary.
...
Upon experimenting, I found that `warandpeace.txt` often came up with more "accurate" or "sensical" autocompletions overall. I figure this is due to its larger size as compared to `Alice's Adventures in Wonderland.txt`, giving the model a more robust training sample upon which to judge what characters are supposed to come next. However, this model also resulted in longer compute times, once again likely due to its larger size. Regarding the time consumption of different values of $n$, around $n>100$ for $k = 1$ is when it becomes slow for autocomplete. If we're talking more about theoretical time-complexity, I ran it at $n = 1000$ for $k=1$ and it completed not quickly but not *too* slowly. However, at $n=10,000$ it caused my IDE and OS to freeze up (likely due to the large `tables` data structure being built in memory). But these are very unreasonable and unrealistic numbers to be using within the scope of this assignment.
<hr>
