...
For this, I build a nested defaultdict that can store a dictionary of integers as its values. I create `n` of these dictionaries to store the frequency of each character sequence up to `n`. I iterate through each character in the document updating the frequency count for that character for each of the n-gram from 1-n. It produces a list of frequency tables, which are then used for calculating sequence probabilities.
...
- ***Write the formula for sequence likelihood as described in section 2***
    - The probability of the next character given a sequence is calculated as:
        $$P(x_{t+1} \mid x_1, x_2, \dots, x_t) = \frac{P(x_1, x_2, \dots, x_t, x_{t+1})}{P(x_1, x_2, \dots, x_t)}$$

### Code analysis

- ***Put the intuition of your code here***
    - The function `calculate_probability` computes the likelihood of a character following a given sequence using the frequency tables. It first determines the appropriate table based on the sequence length, retrieves the frequency of the sequence followed by the character, and divides it by the total frequency of the sequence to get the probability.
...
- ***Put the intuition of your code here***
    - The function `predict_next_char` predicts the most likely next character by calculating the probability of each possible character in the vocabulary following the given sequence. It uses the `calculate_probability` function to determine these probabilities and selects the character with the highest probability.
...
I experimented with various starting sequence, different values of n-grams and length of completion. In my experimentation if you give a complete word ('the' in my case) as the starting sequence it usually resulted in the same output meaning the temperature of the model was almost 0. For different values of n-grams in the text Alice in the wonderland, I started light with 5-10 and it was quick in the predictions, however when I scaled it to around 100 it took quite some time to give results but still workable. When I pushed to 200 it was super slow and took exactly 9.8 seconds (I timed it!) which is hard to work with. 

For the warandpeace corpus which was significantly larger than Alice in the wonderland corpus, even a n-gram of 50 took 28.7 seconds which is very slow. Given that this corpus is almost 60 times bigger this made sense as the table creation and calculate probability had a exponentially larger search_space.
I think the sentence formed from the waranpeace corpus made more sense given the same starting point. 
For the starting text 'the'

Alice - `the project gutenberg literary archive foundation is`
Warandpeace - `the same time he felt that the deeper the ground sank`
Also giving small n-gram of 5 resulted in a repeated words being predicted like `the said the said the said the said` This is probably because of a positive feedback loop where having the n-gram smaller than a average word length would in most cases result in this scenario. 
<hr>
