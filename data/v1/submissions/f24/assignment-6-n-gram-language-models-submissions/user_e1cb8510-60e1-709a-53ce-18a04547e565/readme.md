...
- This part of the code creates a list of n dictionaries. There are 2 variables, ngram and j that are used to iterate through the document and add the frequencies to the dictionary. The ngram variable keeps track of how long of a sequence to count according to the value of the n-gram table, i.e., when ngram = 2, sequences of 2 characters are added to the bigram table. The j variable corresponds to the character of the document to start the sequence. It also ensures that the code does not go out of bounds to get a character that exceeds the length of the document. As the code iterates, the character sequences are added to their respective n-gram tables. If the sequence already exists in the dictionary, the value is incremented. 
...
- The code first gets the value of n which is the size of the 'tables' parameter. The value of n is needed in the case that the sequence length is larger than n. If the sequence length is larger, consider the last n-1 characters. The last n-1 characters are needed for the sequence in order to ensure that there is an n-gram table for the sequence + char. Get the frequencies of the initial sequence and the sequence with the additional char using the proper indices of the tables parameter. If either sequence doesn't exist in the table, it will return 0 as the frequency. The probability is calculated by dividing the frequency of the sequence with the additonal char by the frequency of the initial sequence.
...
3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* 
    ...
    = $(\frac{11}{20}) \cdot (\frac{100}{11}) \cdot (\frac{1}{5})$
    = $\frac{1}{20}$
...
- This function creates a dictionary to hold the probability for each possible next char.
For each char in the vocabulary, the function creates a key corresponding to that char in a dictionary with a corresponding value that is the return value of calculate_probability(sequence, char, tables) where 'char' is the current char key.
After getting the probability for each char in the vocabulary, the function returns the char that has the max probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  ...
  - The next char in the sequence should be 'b'
...
By varying the corpus files and the varying n values i've noticed that the model works much better with higher n values. Higher n values give the model a longer sequence to work off of and provide a more relevant word. In addition, the two corpuses perform differently based on the context of the sequence I plug in. For example, when prompted with n=10, initial sequence = 'que' and k=2, the Alice in Wonderland text responds with 'queen' whereas the War and Peace text responds with 'quest'. The War and Peace text is much longer than the Alice in Wonderland text so it takes much longer to calculate a result with larger n values. An n value of 20 took around 20 seconds to produce an output and jumping to an n value of 30 terminated. 
