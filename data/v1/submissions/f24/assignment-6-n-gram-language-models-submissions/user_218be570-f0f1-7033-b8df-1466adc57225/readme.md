...
- For each k in range [1, n], iteratively extract contiguous k-length substrings from the document and store the occurrence of each unique substring in a dictionary. The dictionary for each k value is stored in an array which is returned.  
...
$P(X_1 = x_1, X_2 = x_2, \dots, X_T = x_T) = P(x_1) \cdot P(x_2 \mid x_1) \cdot P(x_3 \mid x_1, x_2) \cdots P(x_n \mid x_1, \dots, x_{n-1}) \cdot \prod_{t=n+1}^{T} P(x_t \mid x_{t-n+1}, \dots, x_{t-1})$ 

$= \prod_{t=1}^{n-1} P(x_t \mid x_1, \dots, x_{t-1}) \cdot \prod_{t=n}^{T} P(x_t \mid x_{t-n+1}, \dots, x_{t-1})$



### Code analysis

- The probability value is calculated as the frequency of the updated sequence (sequence followed by the character) divided by the frequency of the original sequence. If the prior sequence is empty, return the frequency of the character divided by the sum of all frequencies in the initial table.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(X_1 = a) \cdot P(X_2=a | X_1=a) \cdot P(X_3 = a | X_1 = a, X_2 = a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(X_1 = a) \cdot P(X_2=a | X_1=a) \cdot P(X_3 = b | X_1 = a, X_2 = a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = \frac{1}{10}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(X_1 = a) \cdot P(X_2=a | X_1=a) \cdot P(X_3 = c | X_1 = a, X_2 = a) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = \frac{1}{20}$
...
- For each character in the vocabulary, calculate its probability of appearing next in the sequence, and choose the character with the highest probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - b would be next in the sequence because since aab has the highest sequence probability (1/10) compared to aaa (1/20) and aac (1/20).
...
With the War and Peace text, I tested with a standard initial sequence 'he spoke' and k = 20 to see the impact of n. When n = 2, the output was 'he spoke the the the the the ', which is a clear indication that bigram is too simple for it to generate coherent sentences and truly capture the word dependencies. When n = 10, the output was 'he spoke of the countess mary', which is an improvement from the previous one. When n = 20, the calculation got slightly more timeconsuming. The output was 'he spoke of the execution he', which still makes sense, but the final 'he' threw the sentence off a little. As I continued to increase n to 50, it got too time consuming and not practical.  


Doing the same with Alice's Adventures in Wonderland text, below were my findings: 

n=2: 'he spoke the the the the the'
n=10: 'he spoke. "i must be off, an'
n=20: 'he spoke, "either you or you'
n=30: 'he spoke, "either you or you'
n=40: 'he spoke, "either you or you'
n=50: 'he spoke. "i must go and get'
n=60: 'he spoke, "either you or you'


One main observation with this text is that the calculations were much quicker, because the size of the corpus is significantly shorter. Even when n=100, it wasn't noticeably time consuming, whereas with the War and Peace text, at around n=20, I could notice the delay in output. However, because the text is shorter, the outputs were also not making much sense and cycled between responses with different n values, and the quality of the output was more refined when using war and peace text. 
