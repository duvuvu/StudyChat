...
Create a list of dictionarys that has key of length 1 to n+1 of the combination of vocabulary.
Count frequency for each key in the corpus and assign as value.
...
- ***Write the formula for sequence likelihood as described in section 2***
 ...
 $$P(x_1, x_2, x_3) = P(x_1) \cdot P(x_2 \mid x_1) \cdot P(x_3 \mid x_1, x_2) = \frac{f(x_1)}{size(C)} \cdot \frac{f(x_1,x_2)}{f(x_1)} \cdot \frac{f(x_1, x_2, x_3)}{f(x_1, x_2)} $$

### Code analysis

- ***Put the intuition of your code here***

Calculate the multiplication of the first n-terms by dividing the count of the
first n characters of the sequence in the table with C (the length of corpus). 
Calculate the rest of terms by using a sliding window to find the substrings of the
sequence and read the table to find numerator and denominator and multiply the
first n terms's result with numerator / denominator.
...
1. $P(X_1=a, X_2=a, X_3=a )$
   - $$P(X_1=a) \cdot P(X_2=a \mid X_1=a) \cdot P(X_3=a \mid X_1=a, X_2=a) = \frac{f(X_1=a)}{size(C)} \cdot \frac{f(X_1=a,X_2=a)}{f(X_1=a)} \cdot \frac{f(X_1=a, X_2=a,X_3=a)}{f(X_1=a, X_2=a)}$$
   - $$= \frac{11}{20} \cdot \frac{4}{11} \cdot \frac{1}{4}$$
   - $$= \frac{1}{20}$$


2. $P(X_3=b \mid X_1=a, X_2=a)$
    - $$P(X_1=a) \cdot P(X_2=a \mid X_1=a) \cdot P(X_3=b \mid X_1=a, X_2=a) = \frac{f(X_1=a)}{size(C)} \cdot \frac{f(X_1=a,X_2=a)}{f(X_1=a)} \cdot \frac{f(X_1=a, X_2=a,X_3=b)}{f(X_1=a, X_2=a)}$$
    - $$= \frac{11}{20} \cdot \frac{4}{11} \cdot \frac{2}{4}$$
    - $$= \frac{1}{10}$$


3. $P(X_3=c \mid X_1=a, X_2=a)$
    - $$P(X_1=a) \cdot P(X_2=a \mid X_1=a) \cdot P(X_3=c \mid X_1=a, X_2=a) = \frac{f(X_1=a)}{size(C)} \cdot \frac{f(X_1=a,X_2=a)}{f(X_1=a)} \cdot \frac{f(X_1=a, X_2=a,X_3=c)}{f(X_1=a, X_2=a)}$$
    - $$= \frac{11}{20} \cdot \frac{4}{11} \cdot \frac{1}{4}$$
    - $$= \frac{1}{20}$$
...
Calculate probability for each character appended after the sequence
via the calculate probability function and pick the character with
...
For corpus \"Alice\'s Adventures in Wonderland.txt\", when n = 1 and 2, the 
calculation is fast; when n = 3, the calculation is in acceptable range;
when n >= 4, the calculation become too time consuming. 

For corpus \"warandpeace.txt\", when n = 1, the calculation is fast; when n = 2,
the calculation is in acceptable range; when n >= 3, the calculation become
too time consuming. 

\"Alice\'s Adventures in Wonderland.txt\" is better because it gives a
similar result as what \"warandpeace.txt\" offers when the other inputs are
the same and it is faster to calculate. Not only that, \"Alice\'s Adventures in Wonderland.txt\" is easier to output results based on higher n, which is more accurate. For example, \"Alice\'s Adventures in Wonderland.txt\" with n=3, sequence="hum", completion=3 gives "humble" when \"warandpeace.txt\" with n=2, sequence="hum", completion=3 gives "hume t", using relatively the same amount of time. It's clear that "humble" is an English word when "hume t" is not.

<hr>

## I want to note that the entire assignment was done by myself, <redacted> <redacted>. I accidently used my personal git email to commit so that's why it's showing a different github account.
