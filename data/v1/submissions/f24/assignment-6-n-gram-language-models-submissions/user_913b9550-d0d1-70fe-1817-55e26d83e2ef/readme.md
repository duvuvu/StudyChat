...
For each number of characters 1 to n, I am creating a dictionary of all combinations of characters of those lengths length found in the document, and how many times they show up, then storing those dictionaries in a list.

### Compute Probability Tables

**Note:** _Probability tables_ are different from _frequency_ tables**

- Assume that your training document is (for simplicity) `"aababcaccaaacbaabcaa"`, and the sequence given to you is `"aa"`. Given n = 2, do the following:
...
$$Sequence Likelihood = \frac{f(x_1)}{size(C)} \cdot \frac{f(x_1...x_{n}))}{f(x_1...x_{n-1})} \cdot \frac{f(x_{2}...x_{n+1})}{f(x_{2}...x_{n})} \cdot ... \cdot \frac{f(x_{t-(n-2)}...x_{t+1})}{f(x_{t-(n-2)}...x_{t})}$$


### Code analysis

- ***Put the intuition of your code here*** \
My code first sets up some variables to be easier to calculate certain things. It then checks if the sequence fits entirely within the n-gram table we are given. If so, we can fetch and calculate the frequency directly. Otherwise, we repeatedly multiply a current probability that starts as f(x_1)/size(C) by the terms of f(x_start, x_start+n)/f(x_start, x_start+n-1), until we reach the end.
...
1. $P( X_1=a, X_2=a, X_3=a )$
   ...
   $\frac{11}{20}*\frac{5}{10}*\frac{1}{4}= \frac{11}{160}$ \
    Values from the above tables.
        
2. $P(X_1=a, X_2=a, X_3=b)$
   - *Show your work* \
   $P( X_1=a, X_2=a, X_3=b ) =$ \
   $P( X_1=a) * P( X_2=a \mid X_1=a)*P( X_3=b \mid X_1=a, X_2=a)$ = \
   $\frac{11}{20}*\frac{5}{10}*\frac{2}{4}= \frac{11}{80}$ \
    Values from the above tables.
3. $P(X_1=a, X_2=a, X_3=c)$
   - *Show your work* \
   $P( X_1=a, X_2=a, X_3=c ) =$ \
   $P( X_1=a) * P( X_2=a \mid X_1=a)*P( X_3=c \mid X_1=a, X_2=a)$ = \
   $\frac{11}{20}*\frac{5}{10}*\frac{1}{4}= \frac{11}{160}$ \
    Values from the above tables.
...
My code calls calculate_probability for each character in the vocabulary, while storing the maximum probability found so far, and the associated character. After all characters have been checked, the one with the highest probability is chosen.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  ...
  b. \
  given the starting two characters "aa", b has the highest probability attached. (of "aaa", "aab", and "aac": aab has the highest probability)
...
With very low values of n, the issue of context comes into play, as in the middle of a prediction, the model loses the context of previous iterations, resulting in nonsensical predictions - even changing in the middle of a word for very small ns. 
Alice in Wonderland doesn't necessarily "work better", but it computes much faster, likely since it is much shorter than War and Peace.
N can be pretty high. For War and Peace, I ran up to 100, but if I was more patient, I could have gone higher. Similar for Alice in Wonderland, which I went up to 1000, but would have stopped at around 500 if I had to choose a cutoff point.
Overall, these algorithms seem very constrained to the single training document, but that makes sense. With a larger training document, it would possibly have more accuracy or be useful in more cases, but it would also take much longer to train, especially for higher n values
<hr>
