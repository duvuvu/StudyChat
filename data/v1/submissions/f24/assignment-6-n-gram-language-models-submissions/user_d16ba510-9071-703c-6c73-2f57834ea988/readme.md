...
The intuition behind this code is to create a model that learns language patterns by observing which characters frequently follow others in a given text. By counting character sequences of different lengths (n-grams), the model builds an understanding of common character transitions and uses these patterns to predict the most likely next character in a sequence.

This code builds frequency tables to capture character sequence patterns from a document, enabling probabilistic next-character predictions. The create_frequency_tables function counts occurrences of unigrams, bigrams, trigrams, etc., to build tables for n-grams up to a specified length. Using these tables, the model calculates the probability of each character following a given sequence, based on conditional probabilities derived from observed counts. For a sequence like "ab", the model estimates the probability of each possible next character and selects the most likely one. This approach allows for effective autocomplete by predicting characters based on learned text patterns.
...
1. ***Write down your probability table 2***:
   - as in:
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid a)$ | $\frac{f(a,a)}{f(a)}$ = $\frac{5}{11}$|
        | $P(b \mid a)$ | $\frac{f(a,b)}{f(a)}$ = $\frac{3}{11}$|
        | $P(c \mid a)$ | $\frac{f(a,c)}{f(a)}$ = $\frac{2}{11}$|
        | $P(a \mid b)$ | $\frac{f(b,a)}{f(b)}$ = $\frac{2}{4}$|
        | $P(b \mid b)$ | $\frac{f(b,b)}{f(b)}$ = $\frac{0}{4}$|
        | $P(c \mid b)$ | $\frac{f(b,c)}{f(b)}$ = $\frac{2}{4}$|
        | $P(a \mid c)$ | $\frac{f(c,a)}{f(c)}$ = $\frac{3}{5}$|
        | $P(b \mid c)$ | $\frac{f(c,b)}{f(c)}$ = $\frac{1}{5}$|
        | $P(c \mid c)$ | $\frac{f(c,c)}{f(c)}$ = $\frac{1}{5}$|
...
2. ***Write down your probability table 3***:
   - as in:
        | $P(\odot)$ | Probability value |  
        | ------ | ----------------- |
        | $P(a \mid aa)$ | $\frac{f(aa,a)}{f(aa)}$ = $\frac{1}{5}$|
        | $P(b \mid aa)$ | $\frac{f(aa,b)}{f(aa)}$ = $\frac{2}{5}$|
        | $P(c \mid aa)$ | $\frac{f(aa,c)}{f(aa)}$ = $\frac{1}{5}$|
        | $P(a \mid ab)$ | $\frac{f(ab,a)}{f(ab)}$ = $\frac{1}{3}$|
        | $P(b \mid ab)$ | $\frac{f(ab,b)}{f(ab)}$ = $\frac{0}{3}$|
        | $P(c \mid ab)$ | $\frac{f(ab,c)}{f(ab)}$ = $\frac{2}{3}$|
        | $P(a \mid ac)$ | $\frac{f(ac,a)}{f(ac)}$ = $\frac{0}{2}$|
        | $P(b \mid ac)$ | $\frac{f(ac,b)}{f(ac)}$ = $\frac{1}{2}$|
        | $P(c \mid ac)$ | $\frac{f(ac,c)}{f(ac)}$ = $\frac{1}{2}$|
        | $P(a \mid ba)$ | $\frac{f(ba,a)}{f(ba)}$ = $\frac{1}{2}$|
        | $P(b \mid ba)$ | $\frac{f(ba,b)}{f(ba)}$ = $\frac{1}{2}$|
        | $P(c \mid ba)$ | $\frac{f(ba,c)}{f(ba)}$ = $\frac{0}{2}$|
        | $P(a \mid bb)$ | $\frac{f(bb,a)}{f(bb)}$ = $\frac{0}{0}$|
        | $P(b \mid bb)$ | $\frac{f(bb,b)}{f(bb)}$ = $\frac{0}{0}$|
        | $P(c \mid bb)$ | $\frac{f(bb,c)}{f(bb)}$ = $\frac{0}{0}$|
        | $P(a \mid bc)$ | $\frac{f(bc,a)}{f(bc)}$ = $\frac{2}{2}$|
        | $P(b \mid bc)$ | $\frac{f(bc,b)}{f(bc)}$ = $\frac{0}{2}$|
        | $P(c \mid bc)$ | $\frac{f(bc,c)}{f(bc)}$ = $\frac{0}{2}$|
        | $P(a \mid ca)$ | $\frac{f(ca,a)}{f(ca)}$ = $\frac{2}{3}$|
        | $P(b \mid ca)$ | $\frac{f(ca,b)}{f(ca)}$ = $\frac{0}{3}$|
        | $P(c \mid ca)$ | $\frac{f(ca,c)}{f(ca)}$ = $\frac{1}{3}$|
        | $P(a \mid cb)$ | $\frac{f(cb,a)}{f(cb)}$ = $\frac{1}{1}$|
        | $P(b \mid cb)$ | $\frac{f(cb,b)}{f(cb)}$ = $\frac{0}{1}$|
        | $P(c \mid cb)$ | $\frac{f(cb,c)}{f(cb)}$ = $\frac{0}{1}$|
        | $P(a \mid cc)$ | $\frac{f(cc,a)}{f(cc)}$ = $\frac{1}{1}$|
        | $P(b \mid cc)$ | $\frac{f(cc,b)}{f(cc)}$ = $\frac{0}{1}$|
        | $P(c \mid cc)$ | $\frac{f(cc,c)}{f(cc)}$ = $\frac{0}{1}$|
...
### Formula
- $$
P(X_1 = x_1, X_2 = x_2, X_3 = x_3, X_4 = x_4) = \frac{f(x_1)}{\text{size}(C)} \cdot \frac{f(x_1, x_2)}{f(x_1)} \cdot \frac{f(x_1, x_2, x_3)}{f(x_1, x_2)}  \cdot \frac{f(x_1, x_2, x_3, x_4)}{f(x_1, x_2, x_3)}
$$

$$
P(X_1 = x_1, X_2 = x_2, ....., X_n = x_n) = \frac{f(x_1)}{\text{size}(C)} \cdot \frac{f(x_1, x_2)}{f(x_1)} \cdot \frac{f(x_1, x_2, x_3)}{f(x_1, x_2)} \.... \frac{f(x_1, x_2,....., x_n)}{f(x_1, x_2,...., x_{n-1})}
$$


### Code analysis

This algorithm calculates the probability of a sequence using n-gram frequency tables, based on the chain rule of probability. It models the sequence probability as the product of conditional probabilities for each character, given its preceding context.

Unigram (first character): The probability of the first character is computed as its frequency divided by the total unigram frequencies.

Higher-order n-grams: For each subsequent character, it calculates the conditional probability using the formula above.

It multiplies these values to build the overall sequence probability.

If any n-gram or prefix is unseen, the probability becomes zero. This approach balances context-dependence with computational efficiency by leveraging frequency tables of varying n-gram lengths.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - To calculate this probability, we consider the following:
     ...
     $P(X_{3} = a \mid X_{1} = a, X_{2} = a)$ = $\frac{f(a)}{\text{size}(C)} \cdot \frac{f(a, a)}{f(a)} \cdot \frac{f(a, a, a)}{f(a, a)}$  = $\frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - To calculate this probability, we consider the following:
   
     $P(X_{3} = b \mid X_{1} = a, X_{2} = a)$ = $\frac{f(a)}{\text{size}(C)} \cdot \frac{f(a, a)}{f(a)} \cdot \frac{f(a, a, b)}{f(a, a)}$  = $\frac{2}{20}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - To calculate this probability, we consider the following:
   
     $P(X_{3} = c \mid X_{1} = a, X_{2} = a)$ = $\frac{f(a)}{\text{size}(C)} \cdot \frac{f(a, a)}{f(a)} \cdot \frac{f(a, a, c)}{f(a, a)}$  = $\frac{1}{20}$
...
The predict_next_char function identifies the most likely next character in a sequence based on n-gram probabilities. Using a given vocabulary, it evaluates each possible next character by appending it to the sequence and calculating the probability of the extended sequence using the calculate_probability function.

Key Steps:
Iterate through the vocabulary: Each character is tested as a potential next character.

Evaluate probability: The function calculates the sequence's probability for each character, considering how well it fits the existing sequence (via frequency tables).
Select the best match: The character with the highest probability is chosen as the prediction.

The intuition is to leverage n-gram statistics to predict the next character by assessing how likely it is based on historical patterns in the document. This function essentially picks the character that maintains the strongest continuity with the observed data.
...
The effectiveness of the n-gram approach depends heavily on the size and nature of the corpus. For smaller corpora or those with repetitive patterns, lower values of 
n (e.g., n = 2 or n = 3) often work well, as the tables are easier to compute and provide sufficient context without overfitting to specific patterns. Larger, more diverse corpora benefit from higher n-values (e.g., n = 4 or n = 5), as they capture longer dependencies and richer contextual information. However, increasing 
n drastically inflates the computational cost and memory requirements, as the frequency tables grow exponentially with n.

In experiments, simple corpora with repetitive or structured patterns (e.g., "abababab") perform well with low n, while complex, less predictable corpora require higher n. The practical upper limit for n depends on your system's computational power and the corpus size but typically becomes infeasible around n = 6 to 8 for large datasets.
