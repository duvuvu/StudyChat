...
The purpose of create_frequency_tables is to create n tables where each table stores the frequencies of character sequences of length 1 to n. Each table represents the frequency of seeing a certain character following a specific sequence of preceding characters (context) in the training document. For example:

Table 1 records frequencies of each individual character.
Table 2 records frequencies for bigrams.
Table 3 records frequencies for trigrams, and so on.
This helps us capture the conditional dependencies needed to predict the next character.

### Compute Probability Tables

**Note:** _Probability tables_ are different from _frequency_ tables**

Assume that your training document is `"aababcaccaaacbaabcaa"`, and the sequence given to you is `"aa"`. Given `n = 3`, do the following:

1. **Vocabulary Extraction:** Extract the unique characters from the document to determine the vocabulary.
   - **Vocabulary:** `{a, b, c}`

2. **Write Down Probability Tables**

#### Probability Table 1 (Unigram)
...
#### Probability Table 3 (Trigram)
| Trigram    | Probability         |
|------------|---------------------|
| \( P(a \mid a, a) \) | \( \frac{1}{3} \) |
| \( P(b \mid a, a) \) | \( \frac{1}{3} \) |
| \( P(c \mid a, a) \) | \( \frac{1}{3} \) |
| \( P(a \mid a, b) \) | \( \frac{1}{3} \) |
| \( P(c \mid a, b) \) | \( \frac{2}{3} \) |
| \( P(a \mid b, a) \) | \( \frac{1}{3} \) |
| \( P(a \mid c, a) \) | \( \frac{1}{4} \) |
| \( P(c \mid c, a) \) | \( \frac{3}{4} \) |

These tables capture the different levels of conditional dependencies in the text, which are crucial for computing probabilities and predicting the next character in a sequence.

### Formula for Sequence Likelihood

Using the chain rule, the probability of the next character in an N-gram model is calculated as:
\[ P(X_t = x_t \mid X_{t-1} = x_{t-1}, \dots, X_1 = x_1) = \frac{f(x_1, x_2, \dots, x_t)}{f(x_1, x_2, \dots, x_{t-1})} \]

Where:
- \( f(x_1, x_2, \dots, x_t) \) is the frequency of the full sequence including the next character.
- \( f(x_1, x_2, \dots, x_{t-1}) \) is the frequency of the preceding sequence.

### Code Analysis

1. **`create_frequency_tables(document, n)`**
   - **Purpose:** This function constructs `n` tables where each table stores frequencies of character sequences of length 1 to `n`. Each table represents the frequency of seeing a certain character following a specific sequence of preceding characters (context) in the training document.

2. **`calculate_probability(sequence, char, tables)`**
   - **Purpose:** This function calculates the probability of observing a character following a given sequence by looking up the relevant frequency table.

3. **`predict_next_char(sequence, tables, vocabulary)`**
   - **Purpose:** This function calculates the probability of each possible next character in the vocabulary and selects the one with the highest probability as the predicted next character.
...
- **Sequence:** `"aa"`
- **Vocabulary:** `{a, b, c}`
- **Frequency Table (example values based on provided data):**
  - \( f(a, a) = 3 \)
  - \( f(a, a, a) = 1 \)
  - \( f(a, a, b) = 1 \)
  - \( f(a, a, c) = 1 \)
...
#### 1. Calculate \( P(X_3 = a \mid X_1 = a, X_2 = a) \)

To find the probability of seeing `a` after the sequence `"aa"`, we use:
\[
P(X_3 = a \mid X_1 = a, X_2 = a) = \frac{f(a, a, a)}{f(a, a)}
\]

**Steps:**
1. **Identify the counts:**
   - \( f(a, a, a) = 1 \): The trigram `"aaa"` appears once in the document.
   - \( f(a, a) = 3 \): The bigram `"aa"` appears three times in the document.

2. **Calculate the probability:**
   \[
   P(X_3 = a \mid X_1 = a, X_2 = a) = \frac{f(a, a, a)}{f(a, a)} = \frac{1}{3} \approx 0.333
   \]

#### 2. Calculate \( P(X_3 = b \mid X_1 = a, X_2 = a) \)

To find the probability of seeing `b` after the sequence `"aa"`, we use:
\[
P(X_3 = b \mid X_1 = a, X_2 = a) = \frac{f(a, a, b)}{f(a, a)}
\]

**Steps:**
1. **Identify the counts:**
   - \( f(a, a, b) = 1 \): The trigram `"aab"` appears once in the document.
   - \( f(a, a) = 3 \): The bigram `"aa"` appears three times in the document.

2. **Calculate the probability:**
   \[
   P(X_3 = b \mid X_1 = a, X_2 = a) = \frac{f(a, a, b)}{f(a, a)} = \frac{1}{3} \approx 0.333
   \]

#### 3. Calculate \( P(X_3 = c \mid X_1 = a, X_2 = a) \)

To find the probability of seeing `c` after the sequence `"aa"`, we use:
\[
...
**Steps:**
1. **Identify the counts:**
   - \( f(a, a, c) = 1 \): The trigram `"aac"` appears once in the document.
   - \( f(a, a) = 3 \): The bigram `"aa"` appears three times in the document.

2. **Calculate the probability:**
   \[
   P(X_3 = c \mid X_1 = a, X_2 = a) = \frac{f(a, a, c)}{f(a, a)} = \frac{1}{3} \approx 0.333
   \]

### Summary of Results

- \( P(X_3 = a \mid X_1 = a, X_2 = a) = 0.333 \)
- \( P(X_3 = b \mid X_1 = a, X_2 = a) = 0.333 \)
- \( P(X_3 = c \mid X_1 = a, X_2 = a) = 0.333 \)

Since each character (`a`, `b`, and `c`) has an equal probability of appearing after `"aa"`, any of these characters 
could be chosen as the next character in the sequence.


## Experimentation

Experiment with different corpora and values of `n`:
- **Corpus Choice:** A larger and diverse text corpus improves model accuracy by capturing a broader range of N-grams.
- **Optimal `n` Value:** Higher values of `n` increase context sensitivity, but beyond `n = 4`, the model may become computationally intensive. Values of `n = 3` or `n = 4` often balance accuracy and performance effectively.
- **Performance Trade-off:** As `n` grows, table sizes increase exponentially, making computations slower. This trade-off is crucial for real-time applications like autocomplete.
