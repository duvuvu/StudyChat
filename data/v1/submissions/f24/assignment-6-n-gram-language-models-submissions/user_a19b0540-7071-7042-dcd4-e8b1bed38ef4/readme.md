...
- The function iterates through the document for each character-level n-gram of size 1 to n. If the n-gram is in the table for the given size, then it increments the frequency. Otherwise, it adds an entry to the table and sets its frequency to 1. The function should correctly add each sequence of characters of sizes 1 through n to the table of dictionaries in the correct dictionary by size and update the frequency of the n-gram entry each time it is observed afterwards.
...
### Formula
- $P(X_1=x_1, X_2=x_2, ..., X_n=x_n) = \Pi^n_i P(X_i=x_i \vert X_1=x_1, X_2=x_2, ..., X_{i-1}=x_{i-1})$
### Code analysis

- The function returns the quotient of the frequency of the initial sequence concatenated with the character and the frequency of the initial sequence without the character. The function should correctly calculate the probability of the newly constructed sequence given the probability of the initial sequence by dividing the number of times they each occur in the document, as provided by the table of n-grams.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $= P(X_1=a) * P(X_2=a \vert X_1=a) * P(X_3=a \vert X_1=a, X_2=a) = \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{1}{20}$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $= P(X_1=a) * P(X_2=a \vert X_1=a) * P(X_3=b \vert X_1=a, X_2=a) = \frac{11}{20} * \frac{5}{11} * \frac{2}{5} = \frac{2}{20}$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $= P(X_1=a) * P(X_2=a \vert X_1=a) * P(X_3=c \vert X_1=a, X_2=a) = \frac{11}{20} * \frac{5}{11} * \frac{1}{5} = \frac{1}{20}$

P(X_1=a) * P(X_2=a \vert X_1=a) * P(X_3=a \vert X_1=a, X_2=a)

## `predict_next_char(sequence, tables, vocabulary)`

### Code analysis

- The function calculates the probability of each of possible sequence that can be constructed by concatenating the initial sequence with any character token in the vocabulary and then outputs the character with the highest probability.
...
Larger corpora require increasingly longer time and space to create n-gram tables, and smaller corpora produce less accurate results. Smaller corpora produce sparsity in the n-gram tables. Corpora within a certain sweet-spot range provide the best of both worlds: speed and fewer low frequency sequences. In addition to larger corpora taking longer time to calculate the table, larger values of n also take longer time to calculate the table. Values greater than 20 causes the table calculation to become too time consuming for the war and peace corpora, but values up to 100 are not too time consuming. The size of the corpus seems to play a much larger role in the table calculation than values of n. Regardless of settings, the results appear to be very specific and unable to generalize.
