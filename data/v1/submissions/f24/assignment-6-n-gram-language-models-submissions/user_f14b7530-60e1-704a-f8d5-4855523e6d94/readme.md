...
I made the frequency tables by finding how many accurancess of each sequence there are in the document.
I considered edge cases like if the sequence is empty(first char) and only included valid entries. 
...
- ***Put the intuition of your code here***
- I calculated the probability by finding the frequency of the next chrechter given a givensequence divided by the frequency of the sequence
### Your Calculations
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $$P(X_1=a, X_2=a, X_3=a) = \frac{f(x_1 = a)}{size(C)} \cdot \frac{f(x_1 = a,x_2 = a)}{f(x_1 = a)} \cdot \frac{f(x_2 = a,x_3 = a)}{f(x_2 = a)} = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{5}{10} = 0.125 $$
2. $P(X_1=a, X_2=a, X_3=b)$
   -  $$P(X_1=a, X_2=a, X_3=b) = \frac{f(x_1 = a)}{size(C)} \cdot \frac{f(x_1 = a,x_2 = a)}{f(x_1 = a)} \cdot \frac{f(x_2 = a,x_3 = b)}{f(x_2 = a)} = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{3}{10} = 0.075 $$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $$P(X_1=a, X_2=a, X_3=b) = \frac{f(x_1 = a)}{size(C)} \cdot \frac{f(x_1 = a,x_2 = a)}{f(x_1 = a)} \cdot \frac{f(x_2 = a,x_3 = b)}{f(x_2 = a)} = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{10} = 0.005 $$
...
- ***Put the intuition of your code here***
- I iterated through the vocabulary and kept the char with the highest probability to be the next charachter
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - *Your answer*
  - it is most reasonably that the next char will be a, as it has the highest probability
## Experiment
...
Using different corpus files and varying n values I found that there is no big difference in the so called quality of the output, most of the outputs seemed half random or taken directly from the source, i belive it is because the data set is still considerably small, although the model did provided full words and not giberish every time which was nice to see. using war and peace i maneged to set the n value to 11 with a short delay, but any higher value took too long in my opinion, it seemed like it was easyer to process alice's adventure so I could have used a higher n value of 250 with a resonable delay time.
