...
- I created an array of dictionaries where each dictionary is an n table. I then loop through all the text, using ngram = document[i:i + j] to splice the text. I then check whether that spliced string is in the dictionary or not and populate the dictionaries accordingly. 
...
- `P(c | S) = Count(S followed by c) / Count(S)`
- Super sorry it had to look like this, I'm not familiar with laTex and I tried asking gpt to render this correctly but it just wouldn't work. D: 
...
- I initialize the starting ratio which is 1. I then multiply that by the probaility of the first element in the sequnence occuring. Afterwards, I loop through the entire sequence + char and use string splicing to extract the specific substring I want so I can compute the specific probability for that substring. Once that probability is found, it is then multipled with the ratio to compute the new ratio which is the probability of substring x happening thus far. I continue this until we reach the end of the string where the ratio is then returned.
...
- (I'm aware of the changes to the assignment but I'm not familiar enough with git to pull and not familiar enough with laTex to format it correctly)
1. $P(X_3=a \mid X_1=a, X_2=a )$
   - *Show your work*

   $P(a) = \frac{11}{20}$

   $P(aa) = P(a \mid a) = \frac{5}{11}$

   $P(aaa) = P(a \mid aa) = \frac{1}{5}$

   $\frac{11}{20} \times \frac{5}{11} \times \frac{1}{5} = \frac{1}{20}$
...
2. $P(X_3=b \mid X_1=a, X_2=a)$
   ...
   $P(aab) = P(b \mid aa) = \frac{2}{5}$

   $\frac{11}{20} \times \frac{5}{11} \times \frac{2}{5} = \frac{1}{10}$
...
- We are given a vocabulary input which contains all of the possible next characters in our sequence and our job is to find the one with the highest probability. The solution is to just loop through all the characters in vocabulary and pass it into the previous calculate_probability function and keep track of the one with the highest return value. Once the loop terminates we just return the variable we used to track the highest probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  ...
  - When I inputted n = 3, doc = text.txt (which is just aababcaccaaacbaabcaa) and k = 5, I got that the next character in the sequence should be a.
...
- I ran the experiment multiple times with different n values and found that at an n value of 10 the table calculation becomes too time consuming, my mac actually ran out of application memory while running this. In terms of what corpus works better, I found that the Alice Adventures in Wonderland file ran a lot faster than the warandpeace file. 
