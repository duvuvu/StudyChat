...
This function generates frequency tables for an n-gram model based on a given text document and the specified size of n-grams. The function creates a list of `n` nested dictionaries (defaultdicts) that map characters to their frequencies, given certain contexts derived from the document.
For unigrams (n=1), it counts each character independently. For bigrams (n=2) and higher, it counts occurrences of a character in the context of the preceding n-1 characters, capturing the conditional probabilities needed for the n-gram model.
...
P(char|sequence) =(number of occurences of sequence) / (total number of sequences with same length)

### Code analysis

- ***Put the intuition of your code here***

 This function calculates the probability of a character following a given context or sequence of characters.
  It makes use of the n-gram frequency tables created previously to compute the likelihood of char given sequence. For the unigram case (when no preceding characters exist), it calculates the total occurrences of the character in the collection of frequencies. For n-grams, it computes the probability based on the context’s frequency relative to the total occurrences of any character in that same context, which enables it to model character dependencies explicitly.
...
This function predicts the most likely next character based on a given sequence using the frequency tables. It iterates over possible next characters, calculates the probability for each based on the context (using the previously defined function), and identifies the character with the highest probability. If the sequence is empty, it pulls from the whole vocabulary, ensuring that it can always make a prediction even with no context. This function effectively leverages the learned n-gram probabilities to make informed next-character predictions based on patterns observed in the training document.
...
For me, n = 3 and 4 worked the best. N = 1 and 2 led to very quick predictions however they were inaccurate. Beyond n = 6 it took a long time to get the output. N = 3 and 4 gave the best results overall combining speed and accuracy.
