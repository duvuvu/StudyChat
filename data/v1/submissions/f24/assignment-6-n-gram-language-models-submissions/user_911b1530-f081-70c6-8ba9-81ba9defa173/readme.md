...
- It creates tables that count how often different sequences of characters (called n-grams) appear in a document. It starts with single characters (1-grams), then moves to pairs of characters (2-grams), and so on, up to sequences of length n. For each sequence length, it goes through the text, collects all possible sequences, and keeps track of their counts in a dictionary. The result is a list of these dictionaries, one for each sequence length, which shows the frequency of each n-gram in the text.
...
$P(x_{n}|x_1, x_2, ..., x_{n-1}) = \frac{f(x_1, x_2, ..., x_n)}{f(x_1, x_2, ..., x_{n-1})} \times \frac{f(x_1, x_2, ..., x_{n-1})}{f(x_1, x_2, ..., x_{n-2})} \times ... \times \frac{f(x_1)}{c} = \frac{f(x_1, x_2, ..., x_n)}{c}$

Where c is the total number of characters in the corpus

### Code analysis

- This function calculates the probability of a character char appearing immediately after a given sequence of characters, based on frequency tables. It goes through each frequency table, counting all individual character frequencies (c) and checking if the specific combination of sequence followed by char exists in any table. If found, it retrieves its frequency (freq_char). Finally, the function returns the probability as the ratio of freq_char to the total count of characters (c). This helps estimate how likely a character is to follow a given sequence using the n-gram model.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(a|a, a) = \frac{f(a, a, a)}{f(a, a)} \times \frac{f(a, a)}{f(a)} \times \frac{f(a)}{c} = \frac{f(a, a, a)}{c} = 0.05$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(b|a, a) = \frac{f(a, a, b)}{f(a, a)} \times \frac{f(a, a)}{f(a)} \times \frac{f(a)}{c} = \frac{f(a, a, b)}{c} = 0.05$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(c|a, a) = \frac{f(a, a, c)}{f(a, a)} \times \frac{f(a, a)}{f(a)} \times \frac{f(a)}{c} = \frac{f(a, a, c)}{c} = 0.0$
...
- This function predicts the most likely next character to follow a given sequence of characters using frequency tables and a set of possible characters (vocabulary). It calculates the probability of each character in the vocabulary being the next character by calling calculate_probability(). As it evaluates each character, it keeps track of the one with the highest probability. Once all characters are checked, it returns the character with the highest probability as the predicted next character. This is useful for generating text or completing sequences based on patterns learned from the frequency tables.
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
    ...
    In my experiments with various corpus files and values of n ranging from 1 to 6, I observed notable differences in performance and computational feasibility. Simpler corpuses, such as children's literature, performed better with higher n values due to their repetitive patterns and limited vocabulary. In contrast, complex texts like technical manuals or chat logs were better suited to lower n values, as their diversity made larger n-grams less effective and harder to compute. Computationally, the table calculations became noticeably slower beyond n = 4, with significant delays at n = 6, particularly for larger corpuses. This indicates that while higher n values can enhance predictive accuracy, they are limited by the increasing resource demands, requiring a balance based on the text type and computational capacity.
