...
Yes, I used 383GPT for this assignment. I used it to understand the project and check if my code is correct.
...
- In this function, I iterate from i = 1 to n (both inclusive). Within each iteration, I create a dictionary to store the i-gram table, where the keys are i-grams of length i and values are the number of times the i-gram occurs in the document. The function returns a list of n frequency tables represented as dictionaries.
...
2. ***Write down your probability table 3***:
   - Probability Table 3: ($$P(x_3 \mid x_1, x_2)$$)
...
- ***Write the formula for sequence likelihood as described in section 2***
  $$P(X_1 = x_1, X_2 = x_2, ..., X_m = x_m) = P(x_1) \dot P(x_2 \mid x_1) \dot ... \dot P(x_m \mid x_{m - 1}, x_{m - 2}, ..., x_{m - n + 1})$$
  
  $$P(X_1 = x_1, X_2 = x_2, \dots, X_m = x_m) = \frac{f(x_1)}{\text{len}(C)} \cdot \frac{f(x_1, x_2)}{f(x_1)} \cdot \dots \cdot \frac{f(x_{m - n + 1}, \dots, x_m)}{f(x_{m - n + 1}, \dots, x_{m - 1})}$$


### Code analysis

- ***Put the intuition of your code here***
  In the calculate_probability function, I calculate the probability of getting the character given the sequence. I use the formula: $$P(char \mid sequence) = \frac{f(sequence, char)}{f(sequence)}$$. I have defined a separate helper function to find the n-gram probability. The function fetches the table corresponding to the sequence and sequence+char, finds their frequencies, and puts them into the formula to get the resulting probability, which is then returned.
...
![20241118_213349](https://github.com/user-attachments/assets/6bc85448-aef5-4d17-8afe-19a38f26cedd)
...
- ***Put the intuition of your code here***
  The predict_next_char function uses the 2 above functions and the calculate_target_probability helper function to find the most likely character after the sequence. It uses the n-grams formula, and inside the helper function, the common terms from the numerator and denominator are ignored since they cancel each other out. In the predict_next_char function, the length of the corpus is stored in the corpus_length variable by iterating through the values in the 1-gram table. For every character in the vocabulary, the probability of that character is calculated, and if it exceeds the maximum probability so far, the character is stored in the result. At the end, the character with maximum probability is returned.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - b, since it has the highest probability of 0.1
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
  ...
  I noticed that the program takes a long time to run for large values of n. When n = 10, my program took around 15 seconds to run, and when n = 20, it took around 30 seconds to run. When I set n to 50, the program became too time consuming.
  
  I also noticed that the program runs much faster on the Alice's Adventures in Wonderland file than the War and Peace file. This is probably because Alice's Adventures in Wonderland (1697 lines) is a much shorter book than War and Peace (66035 lines). In terms of accuracy, I didn't notice any significant difference between the 2 books. I tested my code using words that are common in the English language, such as "the" and "and", and I was able to predict the words correctly in both books.
