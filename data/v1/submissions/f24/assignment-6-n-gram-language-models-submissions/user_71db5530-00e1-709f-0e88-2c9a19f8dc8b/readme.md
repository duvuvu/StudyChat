...
First, it does some preprocessing to clean the input text, removing numbers and weird symbols and switch every alphabet to lowercase. Then, for n = 1 to n = N (the input max n), it would iterate through the cleaned text and count the appearances of all unique ngrams and create a list of count tables. After that, for n = N (the input max n) to n = 1, it calculates the probability of each ngram by dividing its count by the count of its previous n-1 gram. The frequency tables get returned.
...
$$
P(x_{i+1}|x_1,...,x_i) = \frac{Count(x_{i+1},x_i,...,x_1)}{Count(x_i,x_{i-1},...,x_1)}*\frac{Count(x_{i},x_{i-1},...,x_1)}{Count(x_{i-1},x_{i-2},...,x_1)}*...*\frac{Count(x_{1})}{Size(C)}
$$

### Code analysis

It concatenates the starting sequence with the new character. Then, given the length of this new combined sequence, it goes to the corresponding ngram table and finds the probability of this sequence. If it doesn't exist in the table, then it returns 0.
...
\\P(X_1=a, X_2=a, X_3=a) = 0.55 * 0.45 * 0.2
\\P(X_1=a, X_2=a, X_3=a) \approx 0.05
$$

2. $P(X_1=a, X_2=a, X_3=b)$

$$
P(X_1=a, X_2=a, X_3=a) = P(X_1 = a) P(X_2 = a|X_1 = a)P(X_3 = b|X_1 = a,X_2 = a)
\\P(X_1=a, X_2=a, X_3=a) = 0.55 * 0.45 * 0.4
\\P(X_1=a, X_2=a, X_3=a) \approx 0.01
$$

3. $P(X_1=a, X_2=a, X_3=c)$

$$
P(X_1=a, X_2=a, X_3=a) = P(X_1 = a) P(X_2 = a|X_1 = a)P(X_3 = c|X_1 = a,X_2 = a)
\\P(X_1=a, X_2=a, X_3=a) = 0.55 * 0.45 * 0.2
\\P(X_1=a, X_2=a, X_3=a) \approx 0.05
$$
...
First, it converts the input sequence into lowercase. Then, given all the vocabulary, it takes individual vocabs and concatenates them with the starting sequence to create a list of possible full sequences. It then finds the corresponding probabilities for each full sequence and returns the character that has the maximum probability.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
    - It should be b ('aab') as it has the highest probability.
...
- Experiment with the given corpus files and varying values of n. Do any corpus work better than others? How high of a value of n can you run before the table calculation becomes too time consuming? Write a short paragraph describing your findings.
    ...
    - I find that War And Peace to generally work better (subjectively) than Alice's Adventures in Wonderland. One of reasons is that there are a lot of noise in Alice's Adventures in Wonderland. The "Project Gutenberg" line was repeated too much in the relatively short text, which made the predictor predicting the noise too frequently. War And Peace has less noise and the result predicted was much cleaner. However, War And Peace took significantly more runtime than Alice's Adventures in Wonderland due to its large size. For War and Peace, the runtime began to significantly increase from n = 10 - 20, while for Alice's Adventures in Wonderland, it only began to increase significantly from n = 200 - 300.
<hr>
