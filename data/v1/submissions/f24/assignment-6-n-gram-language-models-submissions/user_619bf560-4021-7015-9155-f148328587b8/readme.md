...
- The intuition of my create_frequency_tables function is that we first find all of the probabilities for table[0], which will be for single characters (ie P(a)). We then find the probabilities for table[1], which will be for double characters (ie P(a,b)), and so on up to table[n-1] which will have sequences of length n.
...
### Formula
- $\frac{f(x_1, ..., x_n)}{size(c)} \cdot \prod_{i=1}^{m-n}\frac{f(x_{i+1}, ... x_{i+n})}{f(x_{i+1},...,x_{i+n-1})}$

 We can directly calculate the probability of a sequence of length n instead of chaining together the conditional probabilities, which is represented by the first term. Then we have to calculate the conditional probabilties for the remaining characters, which is done by the second term.

### Code analysis

- The `calculate_probability` function takes advantage of the fact that $P(char \mid sequence) = \frac{P(sequence, char)}{P(sequence)}$ will have common terms in both the numerator and denomiator. In fact, only the last term in the numerator will not be cancelled out. So depending on the length of the sequence, we either compute the last value of the first term from the formula above if $n >=$ length of sequence, or the last value of the second term if our tables aren't big enough (when $n <$ length of sequence). This essentially computes the probability of $char$ using as many of the last characters as possible in sequence.
...
1. $P(X_1=a, X_2=a, X_3=a)$
   - $P(a \mid aa) = \frac{P(aaa)}{P(aa)}$
   - $P(aaa) = P(aa)P(a \mid aa) = P(a)P(a\mid a)P(a \mid aa)$
   - $P(aaa) = \frac{11}{20}\cdot\frac{5}{11} \cdot \frac{1}{5} = 0.05$
2. $P(X_1=a, X_2=a, X_3=b)$
   - $P(b \mid aa) = \frac{P(aab)}{P(aa)}$
   - $P(aab) = P(aa) P(b \mid aa) = P(a) P(a \mid a) P(b \mid aa)$
   - $P(aab) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{2}{5} = 0.10$
3. $P(X_1=a, X_2=a, X_3=c)$
   - $P(c \mid aa) = \frac{P(aac)}{P(aa)}$
   - $P(aac) = P(aa) P(c \mid aa) = P(a) P(a \mid a) P(c \mid aa)$
   - $P(aac) = \frac{11}{20} \cdot \frac{5}{11} \cdot \frac{1}{5} = 0.05$
...
- The intuition for this function is that we simply calculate the probability of each possible character in the vocabulary using `calculate_probability`, and return the character that is most likely.
...
- **Based on the probability distribution obtained above for all the next possible characters, which character would be next in the sequence?**
  - $P(a \mid aababcaccaaacbaabcaa) \approx P(a \mid aa) = \frac{1}{5}$ from the table
  - $P(b \mid aababcaccaaacbaabcaa) \approx P(b \mid aa) = \frac{2}{5}$ from the table
  - $P(c \mid aababcaccaaacbaabcaa) \approx P(c \mid aa) = \frac{1}{5}$ from the table
  - **So the most likely next character is $b$**
